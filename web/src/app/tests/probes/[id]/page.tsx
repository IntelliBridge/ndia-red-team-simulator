        ...
        <>
          ...
          <PanelSection title="Details" eyebrow="catalog">
            <dl className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              <Fact label="Status">
                ...
                {probe.reason ? <span className="text-muted-foreground"> · {probe.reason}</span> : null}
              </Fact>
              ...
              <Fact label="In sets">{probe.sets.length ? probe.sets.join(", ") : "—"}</Fact>
          ...
          <PanelSection title="Findings from this test" eyebrow={`${related.length} recorded`}>
            ...
            {!findings.error && related.length === 0 && (
              <p className="text-sm text-muted-foreground">No finding recorded from this probe yet.</p>
            )}
            {related.length > 0 && (
