...
- **Alternatives rejected:** CIFAR-10 as the demo dataset (kept for CI); document classifier; synthetic FAQ assistant; the LLM-assistant domain (Phase B, via garak pointed at Pythia if added).
...
- **New dependencies / changed exclusions:** the `ml` extra (CPU torch, ART, SHAP, ONNX loading, sklearn / XGBoost). Exclusions unchanged: no poisoning pipeline, no operational or sensitive data, no targeting or weapons optimization.
