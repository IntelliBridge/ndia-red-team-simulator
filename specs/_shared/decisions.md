...
- **2026-09-08 (later) addendum, dataset publication:** the Phase A datasets were published as the public GitHub repository [IntelliBridge/ai-red-teaming-data](https://github.com/IntelliBridge/ai-red-teaming-data) (title "AI Red Teaming", commit `ff6a36b`, CC BY 4.0 for the repository's own contents, upstream licences kept per file): `data/military_vehicles.parquet` (81 MB, 9,444 JPEGs as bytes with `split`, `label` and `filename` columns, from HF `leibnitz-lab/military_vehicles` at `9f4ab217bda7a1dffbe0ff9c079fb2d0036cdf4a`, MIT), `data/malicious_urls.csv` (651,191 rows, `url` and `type`, CC0) and `data/malicious_urls_eval_split.csv` (128,224 rows, the seeded 20 percent holdout behind the URL classifier metrics), with `INDEX.csv` (size, sha256, source, licence per file), `MANIFEST.json` (dataset entries and a redaction record), `README.md` and `LICENSE`. No models and no CIFAR-10 are published. This is the way other teams obtain the data (spec 11.7). The public URL CSVs are redacted copies (D003 addendum). The first-domain choice and the dataset sources are unchanged.
...
- **2026-09-08 (later) addendum, redaction in the public URL copies:** in the public repository recorded under D001, credential-shaped query-parameter values (`password`, `token`, `guestaccesstoken`, `sid`, `key`, `access_token`, and one AWS-key-shaped string) are replaced with the literal `REDACTED` in 2,346 of 651,191 rows of `data/malicious_urls.csv` and 406 of 128,224 rows of `data/malicious_urls_eval_split.csv`. Row count, order and labels are unchanged, so row indices still match the upstream file. Public hashes: `a514e7cc8534d68030adcfb6d992414647f919a749761ca802d716454a56a1cb` (full) and `068c8effcaef088dab0494544709a9c8ddece94552dc2a1f59fb03ff6b1aabde` (eval split). The unredacted upstream hash stays `d83ce942075dd63ed4d11560cfdcd9d512caa3d680e292f22cab484e8f074d01` and remains the `dataset_revision` the private build records. The private redsim build still trains on the unredacted Kaggle file, so anyone re-deriving metrics from the public copy will see small differences confined to the 0.36 percent of redacted rows. Dataset access policy is otherwise unchanged: the build fetches from Kaggle with a token at build time and CI uses the committed sample.
