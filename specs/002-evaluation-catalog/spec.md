...
| US2 | deferred to Phase B | Independent Reviewer approval of a catalog version is not on the D8 demo-critical path and has no redsim counterpart today (`Target` has `verified: bool` but no author column, and `POST /v1/targets/{id}/verify` returns 501 because the ownership-verification engine was removed). Phase A substitute: bundled assets are approved at build time by the D3 decision and their manifest; uploads are admitted by rule with the upload event on the audit chain. When it lands, the natural home is `Target.verified` set by an `approver`-tier action whose audit event carries actor, decision and reason. |
...
| FR-003 | amended | The provenance fields survive but live in the asset manifest (bundled) or the upload metadata (uploaded), are surfaced as `TargetInfo.metadata`, and are copied into `Provenance.model_manifest` / `dataset` / `dataset_split` for every campaign. "Public-or-synthetic classification" becomes "open, unclassified, named license" per D3. Domain values are the `Domain` literal in `redsim/ml/schema.py`: `image` and `tabular` live, `llm` registered but not evaluated (Phase B). |
| FR-004 | superseded | D003 is RESOLVED (D11): bundled models plus ONNX / `state_dict` upload in Phase A, endpoints Phase B, pickles refused by default. The "allowlist" survives in three forms: the bundled asset manifest, the upload format allowlist enforced on the worker, and redsim's `target_allowlist` admission check in `redsim.safety.authorize`. |
...
| FR-011 | amended | The F008 event envelope is redsim's hash-chained `AuditEvent` (`redsim/audit/chain.py`, `AuditWriter.append(action, actor, target, detail)`), written before the DB mutation by `redsim/services/targets.py`; D4(c) requires the upload event on this chain. `detail` carries kind, blob key and sha256 — never model bytes, dataset rows or credentials; Phase B endpoint credentials go in `AuthProfile` (Fernet), never in `Target.value`. Authorization is the configured policy engine (static, OPA or Cedar) plus RLS. |
...
| Phase B LLM-track corpus (D008) | added 2026-09-08 (later) | Catalog entry for Phase B only: the jailbreak prompts of `idllresearch/malicious-gpt` (USENIX Security '24, commit `25be7cc16ab821c7f7908641d3e433ac81671181`), 200 records combined into `assets/cache/combined/malicious-gpt_jailbreak-prompts_combined.jsonl` (sha256 `98c9e7cf4447d86cb377c1db29f5a07b24a68ffc717b1e84d162f9515afb9b42`) with a README and a JSON Schema beside it. Dataset kind: prompt corpus. Role: probe material for garak-style probes through Pythia (D6), never a classifier dataset and never an MRI input. Licence: unresolved. The upstream repository declares none, so internal research use only until the authors clarify. No `Target` row, no manifest entry and no upload path in Phase A. When B1 lands the catalog entry records the commit, the sha256, the record count and the licence status, and the prompts themselves are never displayed as catalog content. |
...
**(c) Plan and task items that point at Replit monorepo paths.** The correct redsim locations are given; `plan.md` and `tasks.md` are not rewritten here and no task box is checked.

- `plan.md` Approach step 3 and the "Shared API integration" row (`lib/api-spec/openapi.yaml`, "existing generators") → redsim has no hand-maintained OpenAPI file and no generated client. FastAPI serves the schema from the routers registered in `redsim/api/app.py` (`app.include_router(..., prefix="/v1")`); the contract is the Pydantic request/response models in the router module; the web client is the hand-written `web/src/lib/api.ts`.
- `plan.md` "Catalog service" row (`artifacts/api-server/src/services/assurance/evaluation-catalog.ts`) → `redsim/services/targets.py` (extend) or a new `redsim/services/models.py`; framework detection by file signature belongs in `redsim/ml/loaders.py` (John's section 5; not yet present) and runs on the worker.
- `plan.md` "Catalog routes" row and Approach step 4 (`artifacts/api-server/src/routes/assurance/evaluation-catalog.ts`) → `redsim/api/v1/models.py` (new `/v1/models`, John's section 10) alongside the existing `redsim/api/v1/targets.py`, registered in `redsim/api/app.py`.
- `plan.md` "Persistence schema" row and Approach step 7 (`lib/db/src/schema/evaluation-catalog.ts`) → `redsim/db/models.py` (`Target`), with a new Alembic revision under `redsim/db/migrations/versions/` (latest is `0009_tenant_org_id_guard.py`) only if a column is added; the two new `kind` values need API-side validation, not a column change.
...
- `tasks.md` T004 (`lib/api-spec/openapi.yaml`, regeneration) → `redsim/api/v1/models.py` bodies and `web/src/lib/api.ts`, as above.
- `tasks.md` T006 (`lib/db/src/schema/evaluation-catalog.ts`) → `redsim/db/models.py` plus `redsim/db/migrations/versions/` if a column is added.
- `tasks.md` T007 (`artifacts/api-server/src/services/assurance/evaluation-catalog.ts`) → `redsim/services/targets.py` or `redsim/services/models.py`.
- `tasks.md` T008 (`artifacts/api-server/src/routes/assurance/evaluation-catalog.ts`) → `redsim/api/v1/models.py`, registered in `redsim/api/app.py`.
...
- `tasks.md` T001 names `specs/_shared/decisions.md`, which is correct; its content changes, not its path: D001 and D003 are RESOLVED there as of 2026-09-08 and D007 remains OPEN. Correspondingly, `plan.md` gates G1 (D001), G2 (D003) and G3 (D002) are closed by D11; G4 is satisfied by the redsim audit chain (FR-011 above); G5 (D007) stays open.
