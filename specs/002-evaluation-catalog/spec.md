...
| Phase B LLM-track corpora | added 2026-09-08 (later) | Catalog note for Phase B only: the LLM track uses the probe corpora garak ships under `garak/data` (in-the-wild jailbreak prompts, DAN templates, HarmBench, Do-Not-Answer, RealToxicityPrompts subsets and payload sets, spec 11.6). garak's probe classes and detectors load these files directly, so nothing is extracted, re-packaged or catalogued as a dataset of this tool. Licence: Apache-2.0 for the garak package, upstream terms per subset (HarmBench ships its own LICENSE). Role: probe material for garak probes through Pythia (D6), never a classifier dataset and never an MRI input. No `Target` row, no manifest entry and no upload path in Phase A. When B1 lands the catalog records the pinned garak version and the probe ids used, and the prompts themselves are never displayed as catalog content. |
