...
This feature is the feature-level layer beneath the canonical product spec, [docs/superpowers/specs/2026-09-08-adversarial-ml-redteam-spec.md](../../docs/superpowers/specs/2026-09-08-adversarial-ml-redteam-spec.md), which overrides the stack assumptions in this file wherever they conflict. Under decisions D1 (reuse the full aegis platform) and D10 (feature map), F001 "Project access" maps onto aegis components already present at the restored aegis head rather than onto new code: managed identity is Keycloak OIDC through NextAuth (`web/src/server/auth-options.ts`, `web/src/app/api/auth/[...nextauth]/route.ts`; realm in `deploy/keycloak/realm-export.json`), with an Aegis-signed API session cookie for browsers (`aegis/api/session_cookie.py`) and a JWKS-verified bearer JWT for CLI/CI (`aegis/api/auth.py`); membership travels in the IdP token claim `aegis_project_roles` (`{project_id: role}`) into `CurrentUser.project_memberships`, is mirrored in the `users` and `project_memberships` tables (`aegis/db/models.py`), and is exposed by `GET /v1/projects` and `GET /v1/projects/{slug}/membership` (`aegis/api/v1/projects.py`); server-side authorization is `ensure_project_access` (read: any membership) and `check(user, Action, project_id)` (write: role rank) in `aegis/api/policy.py`, delegated to the static/OPA/Cedar engine in `aegis/policy/engine.py`; tenant isolation is Postgres row-level security by `org_id` (migrations `0006_tenant_rls` and `0009_tenant_org_id_guard`; GUC set by `aegis/api/middleware/tenant.py` and `aegis/db/session.py`); accountable history is the hash-chained audit log (`aegis/audit/chain.py`, append-only since migration `0004_audit_append_only`). Decision D002 is RESOLVED by the product owner on 2026-09-08 as "aegis Keycloak OIDC + NextAuth, dev-token mode allowed for the demo" (recorded in [../_shared/decisions.md](../_shared/decisions.md) in its resolution format); D007 (named owner and reviewers) stays OPEN, so the Owner line below remains "Unassigned". Against John's milestones (S1 section 14), F001 is a prerequisite the platform already supplies rather than a milestone of its own: M0 (scaffold) adds the ML mutating actions to `Action`/`_ACTION_MIN_ROLE` and the rank-0 `viewer` role described under FR-003; M5 (UI) reuses `useRoles`/`RoleGated` gating on the new `/models` and findings screens; M7 (deploy) runs Keycloak on Fargate or dev mode, as S1 section 12 allows. F001 is not on the D8 demo-critical order inside Phase A; every row marked "deferred to Phase B" below is in-app membership or invitation management, which the demo performs in the Keycloak admin console instead. The aegis mechanisms are documented in [docs/architecture/auth.md](../../docs/architecture/auth.md) and [docs/architecture/multi-tenancy.md](../../docs/architecture/multi-tenancy.md). A concurrent code fix-up is making the restored platform import-clean; nothing in this section claims the ML vertical itself is implemented.

**Role mapping used in the table** (the product spec's access-matrix section is authoritative where it differs): Owner → `admin` (rank 4); Reviewer → `approver` (rank 3); Analyst → `remediator` (rank 2); Viewer → `scanner` (rank 1) in Phase A, per the shared baseline in `specs/_shared/architecture.md`: aegis has no read-only role, read access is granted to any active membership, and campaign start is registered at `remediator` (`Action.ATTACK_RUN`, `attack.run`) so that `scanner` stays read-mostly in the ML vertical, though it can still start a legacy `scan.start` scan; a strict rank-0 `viewer` role is a Phase B policy change. aegis ranks are linear (a higher rank holds every lower rank's permissions), so the shared matrix cells "Reviewer: No" for registering catalog entries, starting evaluations, and drafting findings become "Yes" under aegis; the "except own authored item" independent-approval rule is enforced by F006's author-is-not-approver check, not by rank.

| Requirement | Status after consolidation | Note |
| --- | --- | --- |
| US1 | amended | Project entry and per-role action visibility are the existing aegis flow: `/login` → Keycloak → `/projects` listing only member projects (`GET /v1/projects`), `useRoles`/`RoleGated` hiding controls, and `check()`/`ensure_project_access` denying server-side (covered by `tests/test_project_access_read.py`, `tests/test_projects_api.py`). The "suspended membership" scenario has no Phase A state (see FR-006): in Phase A a membership is present or absent. The Viewer scenario is exercised with a `scanner` membership (FR-003): evidence is readable and the management controls (`target.manage`, `attack.run`, `run.cancel`, `verify.replay`) are hidden by `RoleGated` and denied by `check()`; the residual gap is that `scanner` can start a legacy scan, which the ML pages do not offer. "Authentication unavailable" is aegis's existing behavior when Keycloak/JWKS is unreachable or `AEGIS_OIDC_JWKS_URL` is unset: the request fails (401/500) and no access is granted. |
| US2 | deferred to Phase B | aegis exposes no in-app role or status mutation endpoint, and `project_memberships` has no status or revision column. In Phase A, role changes are made in the Keycloak admin console (the `aegis_project_roles` claim); the last-owner boundary and stale-update handling arrive with the Phase B membership editor. |
| US3 | deferred to Phase B | aegis has no Invitation entity, delivery-acknowledgement contract, or acceptance route. Phase A onboarding is Keycloak-side: an administrator creates the user and assigns project roles, and the user signs in through the existing code flow. The roster view exists today at `/projects/[slug]/settings` and is readable by any member of the project. |
| FR-001 | amended | Managed identity is Keycloak OIDC; no custom passwords exist. Two bounded exceptions to "no locally minted tokens" are recorded: (1) dev-token mode, `Authorization: Bearer dev:<email>` when `AEGIS_AUTH_MODE=dev`, rejected outright when `AEGIS_ENV=prod`, permitted for the hackathon demo by D002/D11; (2) the Aegis-signed RS256 API session cookie minted by the NextAuth callback after a successful Keycloak login, and the HMAC worker service-account tokens, both session artifacts derived from an IdP login or a deploy-time signing key rather than passwords or invitation credentials. |
...
| FR-003 | amended | Roles are the aegis rank roles per the mapping above. Two departures from the shared matrix: linear rank gives Reviewer (`approver`) every Analyst (`remediator`) permission; and Viewer maps onto `scanner` in Phase A (the baseline mapping), which is read-mostly rather than read-only because `scan.start` sits at rank 1. A strict rank-0 `viewer` entry in `_ROLE_RANK` (`aegis/api/policy.py`), in the realm roles of `deploy/keycloak/realm-export.json`, and in any OPA/Cedar policy in use is a Phase B policy change; until then the ML vertical registers its own mutating actions (`attack.run`, `explain.run`, `harden.recommend`) at `remediator` so a `scanner` member cannot start a campaign. |
| FR-004 | superseded | The repository is no longer on Replit; it is the GitHub fork IntelliBridge/ndia-red-team-simulator. The rule survives in general form: repository collaboration and the existence of a Keycloak realm user neither create nor imply project membership; only an `aegis_project_roles` entry (or `project_memberships` row) does. |
| FR-005 | amended | Roster read is any-member in aegis (`GET /v1/projects/{slug}/membership`), not Owner-only; the product spec keeps aegis behavior. Role/status changes are Keycloak-side in Phase A, so their record is Keycloak's admin event log; any Phase B in-app mutation must append to the aegis audit chain (`AuditWriter.append`) before commit. The "F008 event envelope" is the aegis `AuditEvent` (actor, action, target, success, detail, project_id, run_id, ts, seq, prev_hash/this_hash). |
...
| FR-007 | deferred to Phase B | No last-owner guard exists in aegis, and there is no in-app transition to guard in Phase A. Phase A relies on the realm-import `admin` user keeping the `admin` role; the transactional guard ships with the Phase B membership editor. |
...
| FR-010 | amended | Accountability references already live in the append-only audit chain (migration `0004_audit_append_only`; `aegis audit verify`), so deleting a membership row cannot erase history. Membership soft-delete (keeping the row as "removed") is Phase B with FR-006. |
...
| FR-012 | amended | Sign-out clears the NextAuth session and the Aegis cookies (`/api/auth/signout-aegis`); JWT `exp` and cookie TTL (`AEGIS_API_SESSION_TTL_SECONDS`, default 900) are validated on every request. Because roles travel in the token claim and are not re-read from the database per request, suspension or removal in Keycloak takes effect at the next token/cookie expiry, not instantly; the product spec records this bound. Failures fail closed (401/403). |
| FR-013 | deferred to Phase B | No invitation entity or states exist in aegis. |
| FR-014 | deferred to Phase B | No delivery-acknowledgement contract exists in aegis; Keycloak's own user-creation and email-verification flows carry onboarding in Phase A. |
| FR-015 | deferred to Phase B | Identity verification is Keycloak's; atomic membership creation from an invitation has no aegis counterpart. |
...
**Key entities → aegis:** ManagedIdentity → `users` (sub, email, display_name) plus the request-scoped `CurrentUser`; Project → `projects` within `organizations` (the RLS boundary is the organization; membership is per project); Membership → `project_memberships` (no status or revision in Phase A); Invitation → no aegis table (Phase B); AccessDecision → the policy-engine `Decision` returned by `resolve_policy_engine().evaluate(...)`. Denied decisions are returned as 403 and are not persisted to the audit chain today; recording denied actions is a Phase B addition to F008.
...
- Approach step 3 and the "Shared API integration" row: `lib/api-spec/openapi.yaml` → the FastAPI routers are the contract, `aegis/api/v1/projects.py` (membership endpoints) and `aegis/api/auth.py` (principal resolution); the human-readable reference is `docs/api/v1.md`. A feature-local draft at `specs/001-project-access/contracts/project-access.openapi.yaml` may still be written, but nothing is "coordinated into" a shared YAML.
- Approach step 4 and the "Authorization middleware" row: `artifacts/api-server/src/services/assurance/project-access.ts` → `aegis/api/auth.py` (`get_current_user`), `aegis/api/policy.py` (`check`, `ensure_project_access`, `_ROLE_RANK`, `_ACTION_MIN_ROLE`), `aegis/policy/engine.py`, `aegis/api/middleware/tenant.py`.
- "Access routes" row: `artifacts/api-server/src/routes/assurance/project-access.ts` → `aegis/api/v1/projects.py` (registered in `aegis/api/app.py` under `/v1`).
- Approach step 6 and the "Persistence schema" row: `lib/db/src/schema/project-access.ts` → `aegis/db/models.py` (`User`, `ProjectMembership`, `Project`, `Organization`) plus a new Alembic revision in `aegis/db/migrations/versions/` after `0009_tenant_org_id_guard.py` for any Phase B status/revision columns.
- Approach step 5 and the "Membership UI" row: `artifacts/ai-assurance/src/features/project-access/` → the `@aegis/web` app under `web/`: `web/src/app/login/page.tsx`, `web/src/app/projects/page.tsx`, `web/src/app/projects/[slug]/settings/page.tsx`; gating in `web/src/hooks/useRoles.ts`, `web/src/hooks/useRequireAuth.ts`, `packages/design-system/src/components/role-gated.tsx`.
...
- Approach step 7: the "minimal agreed F008 event envelope" → `aegis/audit/chain.py` (`AuditWriter.append`; `PostgresAuditWriter` online, `JsonlAuditWriter` offline).
- Approach step 8: "generate clients and validators through the existing generation flow" → aegis has no generated client or validator layer; the web app calls the API through `web/src/lib/api.ts`. The step has no aegis counterpart.
...
- T005: `lib/api-spec/openapi.yaml` → `aegis/api/v1/projects.py` plus `docs/api/v1.md`; there is no regeneration step.
- T006: `artifacts/api-server/src/services/assurance/project-access.ts` → `aegis/api/auth.py`, `aegis/api/policy.py`, `aegis/policy/engine.py`.
- T007: `artifacts/api-server/src/routes/assurance/project-access.ts` → `aegis/api/v1/projects.py`.
...
- T010: `lib/db/src/schema/project-access.ts` → `aegis/db/models.py` plus a new revision in `aegis/db/migrations/versions/` (Phase B).
- T011, T014: "the F001 service/routes" → `aegis/api/v1/projects.py` (Phase B).
