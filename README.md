...
| Area | State |
|---|---|
| redsim platform (inherited from aegis): FastAPI `/v1` API (`redsim.api.app:create_app`), Celery workers (`redsim.workers.celery_app`), Postgres with Alembic migrations (`alembic.ini`, `redsim/db/migrations`), Redis, S3/MinIO blob store, Keycloak/NextAuth auth, RBAC and Postgres RLS, hash-chained audit log, per-task LLM routing and budgets, OTel observability and `redsim-log-ingest` | Restored from aegis and importable: `create_app()` builds and the Celery app loads. A code fix-up is bringing the test suite back to green after the pentest removal. Current counts are in [`CLAUDE.md`](CLAUDE.md). |
| Pentest domain (14 scanner adapters, Kali, CAI agents, GitHub remediation, ticketing, CI gate) | Deleted for good. The seams it filled fail explicitly rather than pretend: `GET /v1/scanners` returns an empty roster, `POST /v1/scans` rejects every scanner name with 400, `redsim scan --scanner X` exits 1 without writing a findings file, the web Start scan button is disabled behind a notice, target ownership verification answers 501, dependency re-scan verification reports its engine unavailable, and `redsim doctor` lists the adapter roster as information only. `POST /v1/scans` stays mounted for the ML adapters and is scheduled for review at milestone M0. |
| ML vertical `redsim/ml/`: evidence schema (`schema.py`), `Target` and `AttackAdapter` protocols (`targets/base.py`, `attacks/base.py`), `explain/` and `recommend/` packages | Contracts only. The packages `explain/` and `recommend/` exist and are empty. |
| ML vertical: model loaders, sandboxed loader subprocess, FGSM / PGD / HopSkipJump adapters, noise control, epsilon sweep, SHAP explainers, Model Robustness Index (MRI) scoring, recommendation rules, Celery tasks (`model.validate`, `attack.run`, `explain.run`, `harden.recommend`, ML branch of `verify.replay`), `/v1/models`, `/v1/attacks`, `/v1/datasets`, `/v1/defenses`, `/v1/ml/capabilities`, `/v1/artifacts/{id}`, campaign and compare routes, `redsim ml build-assets` CLI, dataset catalog | Not implemented. Nothing under `redsim/ml/` runs an attack today. |
| Pythia LLM transport (`redsim/llm/pythia.py`, `tests/test_llm_pythia.py`) | Exists and tested. It reads `REDSIM_LLM_MODEL` today. The spec renames that variable to `REDSIM_ML_LLM_MODEL` at M0. |
...
| Deployment: `deploy/docker-compose.yml` (postgres, redis, keycloak, minio, redsim-api, redsim-worker, redsim-worker-default, redsim-beat, redsim-web, redsim-log-ingest, optional opa / otel-collector / loki / jaeger / elasticsearch / kibana), `deploy/helm`, `deploy/Dockerfile.*` | Compose topology and Helm chart are inherited from aegis and are named `redsim-*` (`deploy/helm/redsim`). `deploy/Dockerfile.api` copies `redsim/` and `alembic.ini`, installs `.[api,worker,ml]` and runs `alembic upgrade head` before uvicorn. `deploy/Dockerfile.web` builds `@redsim/web`. `deploy-aws.yml` builds api, worker and web images and rolls whichever ECS services are configured. Not yet exercised end to end on the restored tree. |
...
`redsim/llm/pythia.py` falls back to an in-repo httpx client) and `garak`
(Phase B only).
...
| `REDSIM_ML_LLM_MODEL` | Canonical model id, `<vendor>/<model>` or `pythia/auto`. The code still reads `REDSIM_LLM_MODEL` until the M0 rename lands. |
...
inspecting proxy, so the Python client has to trust the system store: either
inject `truststore`, which is installed in the venv (`import truststore;
truststore.inject_into_ssl()`), or point `SSL_CERT_FILE` at a bundle that
contains the proxy root. Without one of those, httpx fails with
`CERTIFICATE_VERIFY_FAILED` even though `curl` succeeds, because curl uses the
macOS keychain. The Pythia access workstream (PR #11) adds the connectivity
check `python -m redsim.llm.pythia_check`, which lists `/v1/models` and runs
one chat completion. Use it to prove the gateway is reachable before debugging
the narrative code. Until it merges, the equivalent check is a `curl` against
`{PYTHIA_BASE_URL}/v1/models` with the bearer key.
...
shell. `make check` is the only gate today: `.github/workflows/redsim-ci.yml`
carries the redsim lint, mypy, pytest, coverage, SAST and web-build jobs, and
...
.venv/bin/python -m ruff check redsim tests     # lint
...
redsim/                 Python package (renamed from aegis on 2026-09-08)
  ...
  cli/                  `redsim` console script: audit verify, migrate, doctor, plugins, ...
...
| [`docs/architecture/overview.md`](docs/architecture/overview.md), [`auth.md`](docs/architecture/auth.md), [`audit-chain.md`](docs/architecture/audit-chain.md), [`multi-tenancy.md`](docs/architecture/multi-tenancy.md), [`observability.md`](docs/architecture/observability.md) | redsim platform architecture, inherited from aegis. Still accurate for the platform. |
...
| [`docs/adr/0002-registry-seam-and-runners.md`](docs/adr/0002-registry-seam-and-runners.md), [`docs/adr/0004-unified-effect-class-gate.md`](docs/adr/0004-unified-effect-class-gate.md) | The two seams the ML attack adapters reuse: the scanner registry that `redsim.ml.attacks` registers into (dispatch by name or capability, entry-point plugins, signing, sandbox) and the effect-class gate (`redsim/effects.py`) that classifies every adapter action as read, active or external. The other ADRs ([`0001`](docs/adr/0001-vendored-submodules.md), [`0005`](docs/adr/0005-worker-autoscaling-and-dr.md), [`0008`](docs/adr/0008-nix-reproducible-builds.md)) record platform decisions. |
