...
The platform's *role gate* — ``aegis.api.policy.check`` at every mutating
...
scope so ``aegis.policy`` stays importable by pure callers (see the
package docstring). The static role tables live in ``aegis.api.policy``
and are imported lazily inside ``StaticPolicyEngine`` only.

Backend selection mirrors ``aegis.storage.blobs.open_blob_store`` and
``aegis.audit.chain.resolve_writer``: read ``AEGIS_POLICY_ENGINE`` from
...
class StaticPolicyEngine:
    ...
    Reproduces ``aegis.api.policy.check`` exactly: a system principal is
    ...
    def evaluate(self, req: PolicyRequest) -> PolicyDecision:
        # Imported lazily so importing aegis.policy.engine does not drag in
        # FastAPI for pure (CLI / worker) callers.
        from aegis.api.policy import _ACTION_MIN_ROLE, _ROLE_RANK, Action
...
class OPAPolicyEngine:
    ...
    ``/v1/data/aegis/authz``) and reads OPA's ``result.allow`` (bool) and
    optional ``result.reason``. Fails closed on any error.
    """

    def __init__(self, base_url: str,
                 path: str = "/v1/data/aegis/authz") -> None:
...
_DEFAULT_OPA_PATH = "/v1/data/aegis/authz"
...
def _build_policy_engine(config: AegisConfig | None = None) -> PolicyEngine:
    """Construct the engine selected by ``AEGIS_POLICY_ENGINE`` / config.

    Resolution (env wins, falling back to ``config`` then defaults):

    - ``static`` (default) → :class:`StaticPolicyEngine`
    - ``opa`` → :class:`OPAPolicyEngine` over ``AEGIS_OPA_URL`` /
      ``AEGIS_OPA_PATH``
    - ``cedar`` → :class:`CedarPolicyEngine` over ``AEGIS_CEDAR_URL``
    """
    backend = os.environ.get("AEGIS_POLICY_ENGINE")
    if backend is None and config is not None:
        backend = config.policy_engine
    backend = (backend or "static").strip().lower()

    if backend == "opa":
        base_url = os.environ.get("AEGIS_OPA_URL") or (
            config.opa_url if config and config.opa_url else _DEFAULT_OPA_URL
        )
        path = os.environ.get("AEGIS_OPA_PATH") or (
            config.opa_path if config and config.opa_path else _DEFAULT_OPA_PATH
        )
        return OPAPolicyEngine(base_url, path)
    if backend == "cedar":
        base_url = os.environ.get("AEGIS_CEDAR_URL") or (
            config.cedar_url if config and config.cedar_url else _DEFAULT_CEDAR_URL
        )
        return CedarPolicyEngine(base_url)
    if backend != "static":
        logger.warning(
            "unknown AEGIS_POLICY_ENGINE=%r; falling back to static", backend
        )
    return StaticPolicyEngine()


def resolve_policy_engine(config: AegisConfig | None = None) -> PolicyEngine:
...
def reset_policy_engine() -> None:
    ...
    tests, and after changing ``AEGIS_POLICY_ENGINE`` at runtime.
    """
    global _ENGINE
    _ENGINE = None
