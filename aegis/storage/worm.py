...
The append-only audit chain (:mod:`aegis.audit.chain`) is tamper-*evident*:
...
(creds are NOT plumbed through ``AegisConfig``):

  - ``AEGIS_WORM_EXPORT``          enable flag (default off, ``"0"``)
  - ``AEGIS_WORM_BUCKET``          target bucket (default ``aegis-worm``)
  - ``AEGIS_WORM_RETENTION_DAYS``  Object Lock retention (default 2555 ≈ 7y)
  - ``AEGIS_WORM_LOCK_MODE``       ``COMPLIANCE`` | ``GOVERNANCE`` (default
                                   ``COMPLIANCE``)
  - ``AEGIS_WORM_INTERVAL``        beat export interval seconds (default
                                   86400)

The S3 client itself reuses ``AEGIS_S3_ENDPOINT/REGION/ACCESS_KEY_ID/
SECRET_ACCESS_KEY``.
...
from aegis.audit.chain import AuditWriter, verify_chain
from aegis.storage.blobs import BlobRef, BlobStore

logger = logging.getLogger("aegis.storage.worm")

_DEFAULT_BUCKET = "aegis-worm"
...
def worm_export_enabled() -> bool:
    """True when ``AEGIS_WORM_EXPORT`` is set to a truthy value."""
    return os.environ.get("AEGIS_WORM_EXPORT", "0").lower() in ("1", "true", "yes", "on")


def worm_export_interval() -> int:
    """Beat export interval (seconds) from ``AEGIS_WORM_INTERVAL``."""
    try:
        return int(os.environ.get("AEGIS_WORM_INTERVAL", str(_DEFAULT_INTERVAL_SECONDS)))
    except ValueError:
...
class WormArchive:
    ...
    ``store`` is any :class:`~aegis.storage.blobs.BlobStore`; in production
    ...
    def from_env(cls) -> "WormArchive":
        """Build a WORM archive from the ``AEGIS_WORM_*`` env, wrapping an
        ``S3BlobStore`` pointed at the WORM bucket (creds reused from the
        ``AEGIS_S3_*`` env)."""
        from aegis.storage.s3 import S3BlobStore

        bucket = os.environ.get("AEGIS_WORM_BUCKET", _DEFAULT_BUCKET)
        try:
            retention_days = int(os.environ.get(
                "AEGIS_WORM_RETENTION_DAYS", str(_DEFAULT_RETENTION_DAYS)))
        except ValueError:
            retention_days = _DEFAULT_RETENTION_DAYS
        lock_mode = os.environ.get("AEGIS_WORM_LOCK_MODE", _DEFAULT_LOCK_MODE)
        store = S3BlobStore(
            bucket=bucket,
