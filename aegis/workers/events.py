...
``aegis.api.ws`` serves ``/v1/runs/{run_id}/events`` by subscribing to the Redis
...
(``aegis.workers.bootstrap``) calls :func:`publish_job_event` on each job state
...
def _redis_client() -> Any:
    ...
    url = os.environ.get("AEGIS_BROKER_URL", "redis://localhost:6379/0")
