"""`aegis tenants verify` — on-demand tenant-isolation reconciliation.

Runs the same scan as the ``aegis.verify_tenant_integrity`` beat task
(``aegis.workers.tasks.tenant_reconcile``) but synchronously, against the
...
def cmd_tenants_verify(args: argparse.Namespace, config: AegisConfig) -> None:
    ...
    # CLI path, and resolving the engine eagerly would break ``aegis --help``.
    ...
    try:
        ...
        print("    Set AEGIS_DB_URL to a Postgres instance and run "
