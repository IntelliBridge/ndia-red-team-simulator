...
def _cmd_scan_api(args, _config):
    ...
    try:
        ...
        _main._err(f"scan rejected by API: {exc}")
        sys.exit(1)
    _main._info(f"Run ID: {result.get('run_id')}")
    _main._info(f"Job ID: {result.get('job_id')}")
    if result.get("status_url"):
        _main._info(f"Status: {result['status_url']}")
...
def cmd_scan(args, config) -> None:
    ...
    _main._info(f"Run ID: {state.run_id}")
    _main._info(f"Target: {target_url}")

    findings = []

    if getattr(args, "use_strix", False):
        _main._info("Launching Strix as a subprocess (this may take a while)")
        outcome = start_scan(
        ...
        if outcome.success:
            _main._info(f"Strix completed: {len(findings)} finding(s)")
        elif outcome.partial_success:
            _main._warn(f"Strix exited rc={outcome.return_code} but emitted "
                        f"{len(findings)} finding(s) — partial success")
        else:
            _main._err(f"Strix failed: {outcome.error or 'unknown error'}")
    ...
    if not findings and events_path is not None:
        if events_path.exists():
            _main._info(f"Loading Strix events from {events_path}")
            findings = load_strix_events(str(events_path), state.run_id)
        else:
            _main._warn(f"Strix events file not found: {events_path}")
    ...
    if not findings and getattr(args, "demo", False):
        ...
        if fixture.exists():
            _main._info(f"Loading demo fixture from {fixture}")
            with _main.open(fixture) as fh:
                raw = _main.json.load(fh)
            raw_list = raw if isinstance(raw, list) else [raw]
            findings = convert_strix_findings(raw_list, state.run_id)
        else:
            _main._warn("Demo fixture not found. No findings to process.")
    elif not findings:
        _main._warn("No Strix findings loaded. Provide --events, --repo with events.jsonl, or --demo.")

    state.save_findings(findings)
    _main._info(f"Saved {len(findings)} finding(s) to {state.findings_path}")

    # Summary
    if findings:
        print()
        print(f"{_main._BOLD}Scan Summary{_main._RESET}")
        ...
        for sev in ("critical", "high", "medium", "low"):
            ...
            if count:
                print(f"  {_main._colored_severity(sev)}: {count}")
