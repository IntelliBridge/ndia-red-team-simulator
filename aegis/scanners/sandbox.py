...
discovered via the ``aegis.scanners`` entry-point group (gated by
``AEGIS_PLUGINS=1`` + the ``AEGIS_PLUGINS_ALLOW`` allowlist, and optionally the
Ed25519 signature gate in :mod:`aegis.supply_chain.signing`) are *not*. This
...
``aegis.scanners.registry._sandbox_wrap``) — itself a structurally-conformant
:class:`~aegis.scanners.registry.ScannerAdapter` — whose ``scan()`` does not run
...
#. Spawns ``python -m aegis.scanners.sandbox_worker --entry-point <ep>`` with a
   **list argv** (never ``shell=True`` — the existing safe pattern shared by
   every CLI adapter, e.g. :func:`aegis.scanners.registry.run_cli_scan`).
...
from aegis.scanners.registry import _DEFAULT_TIMEOUT, ScanOptions, ScanResult
...
# Env var names (mirrored in aegis.config for discoverability/docs).
ENV_SANDBOX = "AEGIS_PLUGINS_SANDBOX"
ENV_NETWORK = "AEGIS_PLUGIN_SANDBOX_NETWORK"
ENV_CPU_SECONDS = "AEGIS_PLUGIN_SANDBOX_CPU_SECONDS"
ENV_MEMORY_MB = "AEGIS_PLUGIN_SANDBOX_MEMORY_MB"
ENV_FILESIZE_MB = "AEGIS_PLUGIN_SANDBOX_FILESIZE_MB"
...
# AEGIS_* (DB URL, worker-signing/Fernet keys, S3 creds), cloud tokens, etc. —
...
class SandboxConfig:
    ...
    def from_env(cls, *, timeout_s: int | None = None) -> SandboxConfig:
        """Build a config from ``AEGIS_PLUGIN_SANDBOX_*`` env vars + defaults.
...
def sandbox_enabled(config: object | None = None) -> bool:
    ...
    disable it with ``AEGIS_PLUGINS_SANDBOX=0`` (or the ``plugins_sandbox``
...
def _child_env(cfg: SandboxConfig) -> dict[str, str]:
    ...
    plugin code must never see the parent's secrets (``AEGIS_DB_URL``,
    ``AEGIS_WORKER_SIGNING_KEY``, ``AEGIS_AUTH_PROFILES_KEY``, S3/cloud creds,
    API tokens, …). We copy only ``_SAFE_ENV_KEYS`` (what the interpreter needs
    to start and import the one factory) and pin ``AEGIS_PLUGINS=0`` so the child
...
def run_scanner_sandboxed(
    ...
    *,
    ...
    argv = [sys.executable, "-m", "aegis.scanners.sandbox_worker",
...
def _parse_worker_output(
    ...
    *,
    ...
    findings = [AegisFinding.from_dict(f) for f in payload.get("findings", [])]
    return ScanResult(
        findings=findings,
