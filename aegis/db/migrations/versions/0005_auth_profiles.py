...
(``aegis.security_utils.secrets``) and is only decrypted server-side for
...
def upgrade() -> None:
    ...
    # Role separation (guarded; mirrors 0004 Layer 2): the aegis_app DML
        ...
        BEGIN
            IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'aegis_app') THEN
                GRANT SELECT, INSERT, UPDATE, DELETE
                    ON TABLE auth_profiles TO aegis_app;
            END IF;
        END
        $$;
    """))
