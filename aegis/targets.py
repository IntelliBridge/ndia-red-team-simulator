...
class TargetPack:
    ...
    container_prefix: ClassVar[str] = "aegis-target"
    ...
    def up_from_repo(self, repo_path: Path | str) -> TargetRuntime:
        ...
        image_tag = f"aegis-{self.name}:local"
    ...
    def rebuild(self, repo_path: Path | str) -> TargetRuntime:
        ...
        repo = Path(repo_path).resolve()
        image_tag = self.runtime.image_tag or f"aegis-{self.name}:local"
        _run(["docker", "build", "-t", image_tag, str(repo)])
...
class JuiceShopPack(TargetPack):
    ...
    container_prefix = "aegis-juice-shop"
    readiness_path = "/"
