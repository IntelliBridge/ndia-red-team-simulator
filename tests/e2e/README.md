...
| `test_ml_upload_reports.py` | a bundled campaign that yields no finding says so; two compatible campaigns compare `side_by_side` with no delta and no gain word. An ONNX export uploads to `available` with gradients and a pickled `.pt` is `415 pickle_refused` with a `success=False` `model.register` row. `report.md`, `report.json` and `report.html` carry the six sections and the scorecard sub-block, and the completion path renders `report.pdf` as well | 2, 15, 17, 24 |
