...
def test_task_context_preserves_verify_finding_mutations(
    orchestration_db: dict[str, Any],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    monkeypatch.setattr("redsim.db.session.init_engine", lambda: None)
    monkeypatch.setattr(
        "redsim.config.load_config",
        lambda: SimpleNamespace(output_dir="/tmp"),
    )
    monkeypatch.setattr("redsim.storage.open_blob_store", MagicMock())
    monkeypatch.setattr("redsim.state.PostgresRunState", MagicMock())
    monkeypatch.setattr("redsim.audit.chain.PostgresAuditWriter", MagicMock())
    monkeypatch.setattr("redsim.workers.bootstrap._publish", MagicMock())

    with task_context("job-verify-persistence") as ctx:
        finding = ctx.session.get(Finding, finding_id)
        assert finding is not None
        finding.status = "fixed"
        finding.validation_state = "poc_passed"
        finding.validated_at = record.completed_at

    with sessions() as session:
        persisted = session.get(Finding, finding_id)
        job = session.get(Job, "job-verify-persistence")
        assert persisted is not None
        assert job is not None
        assert persisted.status == "fixed"
        assert persisted.validation_state == "poc_passed"
        assert persisted.validated_at is not None
        assert persisted.validated_at.replace(tzinfo=None) == record.completed_at.replace(
            tzinfo=None,
        )
