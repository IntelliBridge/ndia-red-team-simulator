...
(`tests/ml/test_endpoint_contract.py` and `tests/ml/test_endpoint_egress.py`
on the torch-less lane, the registration and campaign paths in
`tests/ml/test_endpoint_routes.py`, `tests/ml/test_admission_phase_b.py` and
`tests/ml/test_tasks_phase_b.py`, and the end-to-end file
`tests/e2e/test_ml_endpoint.py`). `GET /v1/ml/capabilities` carries the
summary block under `endpoint_connector.contract`, read from
`contract_summary()`.
...
| Piece | Where | State |
|---|---|---|
| Request and response contract, `EndpointRegistration`, `encode_request`, `validate_response_bytes`, `contract_summary()` | `redsim/ml/targets/endpoint_contract.py` | built |
| Egress policy: URL rules, allowlist, private-address refusal, resolve-once session pin, `EgressPolicy` | `redsim/ml/endpoint_egress.py` | built |
| `EndpointTarget` (the `Target` protocol over ART `BlackBoxClassifier`), the worker-parent `PredictBroker` (the only outbound HTTP), the sandbox socket plumbing, `probe_endpoint_sandboxed` | `redsim/ml/targets/endpoint.py`, `redsim/ml/endpoint_broker.py`, `redsim/ml/sandbox.py`, `redsim/ml/sandbox_worker.py` | built. The broker uses the `endpoint-v1` encode and validate helpers |
| `MLModelManifest.endpoint: EndpointSpec` and `format: "endpoint"` | `redsim/ml/schema.py` | built |
| The six HTTP codes (`endpoint_url_invalid`, `endpoint_not_allowlisted`, `egress_refused`, `endpoint_schema_mismatch`, `auth_profile_kind_unsupported`, `endpoint_unreachable`) | `redsim/api/errors.py` | built |
| `auth_profile_required` (422), `auth_profile_in_use` (409), `query_budget_exceeded` (429) | `redsim/api/errors.py` | built. `DELETE /v1/auth-profiles/{id}` answers `409 auth_profile_in_use` with `target_ids` while a live endpoint or LLM target references the profile, after a `success=False` `auth_profile.delete` row |
| `attestation_required` (422), `endpoint_auth_failed` (502) | `redsim/api/errors.py` | built: a registration without `evaluation_instance_attestation: true` is refused with `attestation_required`, and a synchronous call answered `401` or `403` to the credential has its code; inside validate or a campaign that condition stays the target's `refusal_reason: load_failed` |
| `POST /v1/models` with `source: "endpoint"`, the `model.register` audit row, validate through the broker, the `ml_model_endpoint` projections, delete | `redsim/api/v1/models.py`, `redsim/services/ml_models.py`, `redsim/workers/tasks/ml_model.py` | built. The typed transport-failure mapping in the campaign task is still open. Gate `target.manage` (admin). Route table in [the API reference](v1.md#register-an-endpoint) |
| Campaign admission for endpoint targets (white-box attacks refused with `attack_requires_gradients`, the query-budget estimate, `POST /v1/models/{id}/attacks` on an `ml_model_endpoint` target) and the worker's broker lifecycle | `redsim/services/ml_campaigns.py`, `redsim/workers/tasks/ml_campaign.py` | built: `429 query_budget_exceeded` at admission, the credential resolved at run time in the worker parent, the broker tally on the record and the audit rows |
| End-to-end evidence through the tiny server | `tests/e2e/test_ml_endpoint.py` | registration through `POST /v1/models` with a bearer `AuthProfile`, RBAC, the `endpoint_url_invalid` and `endpoint_not_allowlisted` refusals, `409 auth_profile_in_use` on the profile while the target is live, the audited delete. Its three campaign-path cases (validation to `available`, the HopSkipJump campaign through the broker, `422 attack_requires_gradients`) fail with one attribution: `EndpointTarget.load` (`redsim/ml/targets/endpoint.py`) sends the 8-row probe as unscaled uint8 values, which the `float32_nchw` rule of this contract refuses, so no endpoint reaches `available` through the tiny server (README open items). The unit evidence is `tests/ml/test_endpoint_routes.py` |

The registration path, read from the tree: the route gates on `target.manage`
...
| `name`, `license_statement` | non-blank (no license statement, no registration) |
...
overridable per admission): `REDSIM_ML_ENDPOINT_RPS` (10),
...
`QueryBudgetExceeded` (`query_budget_exceeded`) and no invented rows. The
same limits bound the admission estimate: `POST /v1/models/{id}/attacks`
...
The broker adds its own floor on top: scheme, userinfo, query and
...
remains. The httpx pinned-connect transport is follow-up work.
...
- The child receives a credential-free `target_endpoint` block in
  ...
  environment allowlist is unchanged. The worker tasks build
...
| Code | Kind | Raised by |
|---|---|---|
| `endpoint_url_invalid` | HTTP 422 (`field: url`) | the URL rules above at registration; an empty registration body fails here first |
| `auth_profile_required` | HTTP 422 | a registration with no `auth_profile_id` |
| `endpoint_not_allowlisted` | HTTP 403 | host outside `target_allowlist` at registration, after the `success=False` audit row with `allowlist_check: fail`, `target` the URL |
...
| `query_budget_exceeded` | HTTP 429 (`estimate`, `cap`) at campaign admission; job state only when the broker reaches `max_rows` or `max_requests` at run time | the admission estimate; the broker |
| `auth_profile_in_use` | HTTP 409 (`target_ids`) | `DELETE /v1/auth-profiles/{id}` while a live endpoint or LLM target of the project names the profile |
...
and message under `validation.probe`; widening the vocabulary is deferred.
`endpoint_auth_failed` still has no HTTP
row, by design: it is only ever a target or job state.

## Recorded divergences

- The contract version is `endpoint-v1`. The earlier working name
  `redsim-predict-proba/1` is superseded and a request carrying it is
  refused as a wrong literal.
- The request body has no `encoding` key; `input_format` names the layout.
- Target-ownership verification by DNS TXT is not built for endpoint
  targets. The control is the egress allowlist plus admin-only registration
  plus the audited attestation (`Target.verified` stays `false`).
- `query_budget_exceeded` is a `429`: a budget refusal that sits with
...
gone afterwards, and that the server saw the bearer on every request.
`tests/ml/test_endpoint_routes.py` drives the registration route,
