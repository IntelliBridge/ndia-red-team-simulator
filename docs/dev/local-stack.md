...
- Free ports:
  ...
  - `:5000` mcp-kali · `:4319` redsim-log-ingest
...
|--------------|---------------------------------------------------------------------------------|
| (default)    | api, worker, web, postgres, redis, keycloak, minio, kali, **redsim-log-ingest**  |
| `obs`        | otel-collector, loki, jaeger                                                    |
| `obs-search` | elasticsearch, kibana                                                           |

The `redsim-log-ingest` service runs in the default profile so
...
slow because `redsim-worker` initialises the cai / strix /
...
is `admin@redsim.local` / `adminpass`.
...
export REDSIM_TOKEN=dev:admin@redsim.local        # dev auth mode

redsim --api status              # mode=api, healthy
redsim --api scan http://juice-shop:3000 --use-strix
```

The dev token works only when `REDSIM_AUTH_MODE=dev` **and**
`REDSIM_ENV` is not `prod`. The API hard-fails dev tokens in prod —
...
| Surface                     | How                                                              |
|-----------------------------|------------------------------------------------------------------|
| Postgres                    | `make psql` (drops into the `redsim` DB)                          |
| Application logs            | `SELECT * FROM application_logs WHERE …` or `redsim --api …` + web `/logs` |
| MinIO console               | `http://localhost:9101` (creds in `deploy/.env`)                 |
| Audit chain                 | `redsim audit verify --all`                                       |
...
The Redsim API session cookie (F14a) is keyed by
`REDSIM_API_SESSION_PRIVATE_KEY` (web side) and
`REDSIM_API_SESSION_PUBLIC_KEY` (api side). The compose stack
...
make up
redsim --api status                 # mode=api, api health = healthy

# Audit chain intact
redsim --api audit verify --all     # ✓ across all chains
...
- **API returns 401**: confirm `REDSIM_TOKEN` is set. If Keycloak is
...
- **Worker idle**: `make logs worker`. The most common cause is a
  ...
  (`make rebuild worker`).
- **`/v1/logs` returns 503**: `redsim-log-ingest` not up. Check
  `docker compose ps redsim-log-ingest` and the service's
  `/health` endpoint at `http://localhost:4319/health`.
- **WebSocket immediately closes 1008**: check the browser console
  for the close reason. Most common: stale `redsim_api_session`
  cookie (re-sign in) or a misconfigured `REDSIM_WEB_ORIGIN`.
