...
test tiers and the CI split.

## Running

```bash
.venv/bin/python -m pytest -q -p no:cacheprovider                          # default tier
.venv/bin/python -m pytest -q -p no:cacheprovider -m ml                    # only the ml-marked tests
REDSIM_E2E=1 .venv/bin/python -m pytest -q -p no:cacheprovider -rs -m e2e tests/e2e   # e2e tier, Postgres lane on with REDSIM_E2E_POSTGRES_URL
.venv/bin/python -m pytest -q -p no:cacheprovider -m garak tests          # garak tier (needs the garak extra; the e2e-gated cases of tests/e2e/test_ml_llm.py need REDSIM_E2E=1 as well)
.venv/bin/python -m pytest -q -p no:cacheprovider tests/ml/test_phase_b_stubs.py   # the Phase B surface pin
...
excludes `docker`, `e2e`, `slow`, `auth_required` and `garak`. The report
tests need `reportlab` (worker extra) and `pypdf` (test extra). The export and
consume tests need `pyarrow` (ml extra) and are `ml`-marked.
...
first (`tests/e2e/README.md` "Running from a git worktree").
...
| `ml` | `-m ml`, needs the `ml` extra | the vertical's library layer: targets, attacks, runners, explainers, recommendation rules, sandbox child, endpoint broker, the endpoint registration and validate path, the admission rules, the worker's broker lifecycle, the review workflow, the PDF and snapshot path (`tests/ml/test_endpoint_routes.py`, `test_admission_phase_b.py`, `test_tasks_phase_b.py`, `test_review_workflow.py`, `test_reports_phase_b.py`, `test_llm_routes.py`, `test_llm_core.py` default-tier cases), the Croissant export against a TinyTarget-shaped run with eager Celery (`test_interop_export.py`: a valid manifest whose FileObject digests and shard columns check out, rows equal to the flip matrix, a mutated row refused with a failed job and no artifacts, idempotent re-export, the refusals, a fixture never exported, the template-only card), the consumed-slice admission and the real parse child (`test_interop_consume.py`: the static refusals audited and persisting nothing, the size cap, the child's digest, class, range and row-cap refusals, a substituted blob refused in the parent, the child spawned credential-free, the API and worker modules importing with pyarrow blocked), ATLAS and Foundry (`test_atlas_foundry.py`: the stamp table against the vendored data, the number-free coverage view, the roster with no value leaking, the payload guard refusing a bare MRI and every forbidden content, admission order and the broker rollback, the happy-path push and the 503-on-commit abort against the fake server with no token, JWT or URL in any row), batches (`test_batches.py`: the roll-up vocabulary, a two-model batch through the single boundary, collected refusals, deferral through the capacity service, cancel, RBAC negatives, grouped compare), bulk upload and capacity (`test_bulk_upload_capacity.py`) and the CLI matrix (`test_cli_matrix.py`, one real-child matrix run) | Unit tests (py3.12), ML tier, Coverage gate, E2E tier |
| `e2e` | `tests/e2e/`, stamped `e2e` by its `conftest.py`, run only with `REDSIM_E2E=1` | the product end to end: real API, admission, eager Celery, the real sandbox child and the real CLI over sqlite on a synthetic asset tree; the Postgres RLS lane with `REDSIM_E2E_POSTGRES_URL`. The core campaign path in `test_harness_smoke.py`, `test_ml_campaigns.py`, `test_ml_upload_reports.py` and `test_ml_governance.py`; the endpoint, LLM, text, detection, attack, review, report, interop and bulk scopes in `test_ml_endpoint.py`, `test_ml_llm.py` (also `garak`-marked), `test_ml_text_detection.py`, `test_ml_attacks_harden.py`, `test_ml_review_reports.py`, `test_ml_interop.py` and `test_ml_bulk.py`, each failing with an attribution naming the module when it meets a product defect (`tests/e2e/README.md`); the harness's `ml_campaigns` mirror carries `batch_id` | `E2E tier (python, eager Celery)` on every PR and push (the step fails if the Postgres lane still reports off), and locally |
| `garak` | `-m garak`, needs the `garak` extra (`garak>=0.16,<0.17`) | the LLM domain: the tests of `tests/ml/test_llm_core.py` and `tests/ml/test_llm_routes.py` plus the e2e-gated cases of `tests/e2e/test_ml_llm.py`, all running real garak through `PythiaGenerator` against the in-process fake gateway `tests/ml/fake_openai_server.py`: headers, body keys and ledger against an `httpx.MockTransport`, the key-file mode check, one real child run (exit 0, counts equal garak's eval records, the hard cap, the persona on every request, no `/v1/models` call, token sums), the credential boundary (no `PYTHIA_`/`AWS_`/`KAGGLE` name in the child env, the key in no file, a DAN prompt fragment only in garak's own `report.jsonl`), the scorecard, rules and report from that run, a version mismatch (exit 3, zero requests), offline mode with HF-detector probes (all `not_run`, zero requests), the wall-clock kill, the committed catalog equal to a fresh regeneration, and the route-to-worker end-to-end case; the e2e file adds the catalog with HarmBench excluded, the registration and probe-run gates per role and one end-to-end run whose k/n scorecard carries no MRI key | `garak offline` on every PR and push (`tests/ml`) and `E2E tier (python, eager Celery)` (the e2e file, the lane installs the extra). The garak step fails on pytest exit 5 and on an all-skipped run, so nothing collected is a regression, never a pass |
| browser e2e | Playwright, `workflow_dispatch` with `run_e2e=true` | the web app against the compose stack | on demand only |
...
- The `garak` skip: when `importlib.util.find_spec("garak")` is `None`, every
  ...
  interpreter without the extra reports skips, never errors.
...
| `garak` | Needs the `garak` extra; skipped when absent. Deselected by `addopts` and by every other lane's marker expression, so `garak` tests run only in the `garak offline` job and the e2e job. Stamp it (and `importorskip("garak")`) on every test that imports garak. |
...
- `fakes_text.py::TinyTextTarget`: a seeded TF-IDF plus logistic
...
- `fakes_detection.py::TinyDetector`: a hand-built differentiable
...
- `tiny_endpoint_server.py::TinyEndpointServer`: a stdlib
  ...
  `tests/ml/test_endpoint_routes.py` registers it through `POST /v1/models`
  and validates it through the worker task.
- `fake_openai_server.py`: a stdlib `ThreadingHTTPServer` on
...
- `fake_foundry_server.py`: a stdlib `ThreadingHTTPServer` on
  ...
  is the only test that reaches a real Foundry instance and skips in CI.
- `fixtures/run_record.json`: the frozen `GET /v1/runs/{id}/campaign` shape
  ...
  Any change to the frozen schema is additive and default-valued, and the
  tripwire must stay green.
- `fixtures/run_record_phase_b.json`: the frozen fixture plus the
  Phase B fields, so a partial landing of the Phase B schema shows
...
- `fixtures/cifar10_test_500.npz` (pinned CIFAR-10 slice) and
  ...
  image and tabular CI fixtures.
- `fixtures/sms_spam_sample.tsv`: 300 rows, 150 per class, seed 0,
...
- `_campaign_pre_refactor.py` and `test_campaign_golden.py`: the
  ...
  behaviour-preserving. Delete both together once the golden is no longer
  wanted.
...
- `test_api_process_has_no_ml.py`: builds the app in a subprocess with
  ...
  `xgboost`, `garak`, `openai`, `litellm`, `reportlab`,
...
- `tests/ml/test_schema_compat.py`: the schema tripwire. The
  frozen fixture's sha256, its byte-identical round trip under
  `exclude_unset`, every frozen property present and untyped, every property
  added since default-valued, sixteen vocabularies never narrowed, the
  stage order kept.
...
- `tests/ml/test_error_codes.py`: the error-code table spelled out in four
  dated blocks and matched against `redsim/api/errors.py` both ways, so a
  code added, renamed or moved to another HTTP status fails there rather than
  in the UI, plus the `capacity_deferred` marker semantics and a refusal of
  the spelling `idempotency_in_flight`.
- `tests/ml/test_phase_b_stubs.py`: the Phase B surface pin.
  Every Phase B path and method is in the OpenAPI document; a `viewer`
...
- `tests/ml/test_interop_consume.py::test_api_service_and_worker_modules_import_with_pyarrow_blocked`
  ...
  raise in the test process: the API and worker modules import
...
- `tests/ml/test_atlas_foundry.py::test_payload_guard_refuses_bare_mri_and_every_forbidden_content`:
  the guard on anything that leaves for Foundry (a bare MRI, a
...
- `tests/test_worker_hardening.py::TestQueueRouting`: the routed-set pin is
  the eight `task_routes` entries. The interop, capacity and integration
  tasks register through the Celery `include` list with their queue on the
  decorator or at enqueue.
- `tests/test_idempotency.py`: the `Idempotency-Key` middleware
...
- `tests/test_policy_ml_actions.py` and `tests/test_policy_engine.py`: the
  `dataset.export` bar is `remediator` in the Python table and both mirrors.
...
- `Unit tests (py3.12)` and `(py3.13)`: install `.[api,worker,test,dev]`
  ...
  Redis.
- `ML tier (py3.12, ml extra)`: `pytest -q -p no:cacheprovider -m ml tests`
  on cached CPU torch, offline.
...
- `E2E tier (python, eager Celery)`: `REDSIM_E2E=1 pytest -q -p
  no:cacheprovider -rs -m e2e tests/e2e` with the migrated service Postgres
  as `REDSIM_E2E_POSTGRES_URL`, so the RLS lane runs and the step fails if the
  harness still reports "Postgres lane is off". The `garak` extra is
  installed for `tests/e2e/test_ml_llm.py`. 30 minute timeout.
- `garak offline`: `.[garak]` on CPU torch, `import garak`, then
  `pytest -q -p no:cacheprovider -m garak tests`. The step fails on
  pytest exit 5 and on an all-skipped run.
