...
14. **LLM-track corpora (2026-09-08, later).** The Phase B LLM-assistant domain (item 6: garak probes through Pythia) uses the probe corpora garak ships under `garak/data`: in-the-wild jailbreak prompts, the DAN templates, HarmBench, Do-Not-Answer, RealToxicityPrompts subsets and the payload sets (spec 11.6). garak's probe classes and detectors load these files directly. Nothing is extracted or re-packaged for this tool. The garak package is Apache-2.0 and each subset keeps its upstream terms (HarmBench ships its own LICENSE). Phase B only and probe material only: not a Phase A dataset, not a classifier dataset, never mixed into an MRI. Prompts are untrusted data, sent only to an explicitly entitled Pythia persona with the permission-gate-only guardrail default, never to a production system. The evidence for an LLM assessment is the test context, the observed response, the criterion, the detector result and the reviewer assessment. A third-party jailbreak corpus fetched earlier the same day was withdrawn because its repository declares no licence, so this brief's data-handling rule holds without divergence (D008 closed as moot). *Addendum 2026-09-09 (owner decision TESTS_DOCS-33, Phase B wave B4):* the sentence "nothing is extracted or re-packaged for this tool" stays true of the code (garak loads its own files; the repository commits no prompt text). The public data repository of item 15 carries, since its commit `754fbca`, a copy of garak 0.16.0's `garak/data` directory (`data/garak/`, 132 files, 3.76 MB) with `LICENSE-garak-Apache-2.0.txt`, a `PROVENANCE.csv` recording the upstream terms per subset and `SHA256SUMS.txt`, published by the owner's decision of 2026-09-08 and recorded in spec 11.6's dated addendum. The `inthewild_jailbreak_llms.json` subset is published as part of that copy under garak's Apache-2.0 packaging with its upstream terms recorded as unconfirmed, not asserted. Probing landed in Phase B wave B2 (`redsim/ml/llm/`) and its end-to-end evidence in wave B4 (`tests/e2e/test_ml_llm.py`, against a fake gateway; no run against the live gateway is recorded).
15. **Public dataset repository (2026-09-08, later).** The Phase A datasets were published as the public GitHub repository [IntelliBridge/ai-red-teaming-data](https://github.com/IntelliBridge/ai-red-teaming-data) (commit `ff6a36b`, CC BY 4.0 for the repository's own contents, upstream licences kept per file): `data/military_vehicles.parquet` (9,444 JPEGs as bytes, HF `leibnitz-lab/military_vehicles` at `9f4ab217bda7a1dffbe0ff9c079fb2d0036cdf4a`, MIT), `data/malicious_urls.csv` (651,191 rows, CC0) and `data/malicious_urls_eval_split.csv` (128,224 rows, the seeded 20 percent holdout behind the URL classifier metrics), with `INDEX.csv` (size, sha256, source and licence per file), `MANIFEST.json`, `README.md` and `LICENSE`. No models and no CIFAR-10 are published. The public URL CSVs are redacted copies: credential-shaped query-parameter values (`password`, `token`, `guestaccesstoken`, `sid`, `key`, `access_token`, one AWS-key-shaped string) are replaced with the literal `REDACTED` in 2,346 of 651,191 rows of the full file and 406 of 128,224 rows of the eval split, with row count, order and labels unchanged so row indices still match upstream. The private build trains on the unredacted Kaggle file, so metrics re-derived from the public copy differ slightly on the 0.36 percent of redacted rows (spec 11.7, D001 and D003 addenda). *Addendum 2026-09-09 (Phase B waves B0 to B4):* the repository head is `0dababc`. After `ff6a36b` it gained `754fbca` (the `data/garak/` copy, item 14), `a9ba6ba3` and `4048a209` (Phase B wave B0: the UCI SMS Spam Collection verbatim with a seeded 20 percent eval split under owner default MODALITIES-12, CC BY 4.0; the 300-image military-assets subset published for owner review under MODALITIES-27, CC BY 4.0, removable in one commit; a WordNet 3.0 reference entry, not republished; a MITRE ATLAS `v2026.08` release record; `INDEX.csv` and `MANIFEST.json` rows for each) and `0dababc` (Phase B wave B3: `data/exports/url_trees_sample/`, the first Croissant export the platform built with its own modules from an offline `url_trees` PGD campaign, 600 rows of 16 lexical features and no URL strings). Every publication is by hand; the platform never pushes to this repository. The redaction record and the re-derivation caveat above are unchanged (spec 11.7 addendum).
16. **Verify paradigm removed (2026-09-09).** The product owner decided that every run is a measurement in its own right. The verify-after-harden loop of items 4, 8 and 9 is removed from the product rather than built further: no verify campaign (`POST /v1/findings/{id}/verify`, the bulk form and the retest list), no `verify.replay` action or job, no finding validation state (`validation_state`, `validated_at`, the `poc_passed` vocabulary), no ΔMRI or measured delta, no derived model. The defense catalog (`GET /v1/defenses`, the ART preprocessing defenses and the training defenses) is dropped with it. Findings close by reviewer decision: `resolve` requires a confirmed review and sets the finding `fixed`. Recommendations stay text-only candidates: each is labelled candidate, none is evaluated against the model inside the product, and a rule may cite an ART class or a paper as plain text only. Migration `0012_remove_verify_paradigm` drops the three columns. *Knowing divergence from items 4, 8 and 9 above and from the "Not in the first milestone" note on the loop.* The brief's reporting principles state no verify-after-harden principle. The decision keeps the principle "treat recommendations as candidates until supported by a separate evaluation": that evaluation is now a separate campaign run by the user, never a loop inside the product. The product spec sections that describe the loop (6.4, 15.6, 16 and the verify rows of 17) stay as history under the dated banner at the top of the spec.
...
- Autonomous application of proposed mitigations. The verify-after-harden loop applies a preprocessing defense to a copy of the model for measurement when a user asks; it deploys nothing. *Superseded 2026-09-09 (item 16 below): the loop is removed and the product applies no defense at all.*
