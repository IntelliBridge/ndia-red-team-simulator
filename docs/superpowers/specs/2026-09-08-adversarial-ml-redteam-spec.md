...
| 58 | LLM-track corpus | **Brief, 11.1, D3:** every dataset carries a licence stated on its distribution page. **Owner decision 2026-09-08 (later):** fetch the `idllresearch/malicious-gpt` jailbreak prompts (USENIX Security '24, commit `25be7cc`) as probe material for the Phase B LLM-assistant domain (D6). **Upstream:** no licence declared. | LLM-track corpus added 2026-09-08 (owner decision). Diverges from the licence rule. Open until resolved (D008). | Section 11.6 records the corpus, the combined file and the handling rules. Phase B only, probe material only, never a classifier dataset or an MRI input (D9). 11.5 carries it as "Phase B, licence unresolved", 12.2 and milestone B1 (section 23) name it. Internal research use until the authors clarify redistribution. |
...
- D008 (licence and redistribution of the malicious-gpt corpus, 11.6) has been OPEN since 2026-09-08 (later). It blocks nothing in Phase A.
...
| `idllresearch/malicious-gpt` jailbreak prompts (11.6) | none declared in the repository | Nothing. Redistribution rights are unstated. | Phase B, licence unresolved (D008). Internal research use only, as LLM-track probe material sent through the permission-gate-only Pythia persona. Not shipped, not published, never a Phase A or classifier dataset, never an MRI input. |
...
### 11.6 Phase B LLM-track corpus: malicious-gpt jailbreak prompts (deferred)

Recorded 2026-09-08 (later, owner decision). This corpus is **not** a Phase A dataset, not a classifier dataset, and never an input to an MRI (D9, section 15.8: one MRI per campaign and per modality). It is probe material for the Phase B LLM-assistant domain only (D6: garak-style probes through Pythia, sections 3.2, 12.2, 17.4 and milestone B1 in section 23). Nothing in Phase A reads it.

- **Source.** `idllresearch/malicious-gpt` on GitHub (the USENIX Security '24 paper on real-world malicious LLM applications, "Malla"), commit `25be7cc16ab821c7f7908641d3e433ac81671181`, fetched 2026-09-08 into the gitignored asset cache `assets/cache/github--idllresearch--malicious-gpt/` (731 files, 359 MB, `.git` stripped, `.provenance` and `.source-commit` sidecars).
- **Contents by folder.** `jailbreak/`: three JSON Lines files, `jailbreak-prompts-from-FlowGPT.json` (72 records), `jailbreak-prompts-from-Poe.json` (125) and `jailbreak-prompts-from-XXXGPT+CodeGPT+MakerGPT.json` (3), 200 records in total. The repository card says 182 jailbreak prompts. The discrepancy is unexplained and was not investigated. `mal_prompts/mal_prompts.xlsx` holds 45 malicious prompts. `LLM_responses/` and `malicious_LLM_responses/` hold model and service responses as JSON and are the bulk of the 644 JSON files. `malicious_LLM_name_list/` holds 3 CSVs, `keywords/` 2 text files, and the rest is measurement scripts and screenshots.
- **Combined file, produced by us.** `assets/cache/combined/malicious-gpt_jailbreak-prompts_combined.jsonl`: 200 records, 368,120 bytes, sha256 `98c9e7cf4447d86cb377c1db29f5a07b24a68ffc717b1e84d162f9515afb9b42`, with a `README.md` and a JSON Schema beside it. Each line carries `source` (enum `FlowGPT | Poe | XXXGPT+CodeGPT+MakerGPT`), `source_file`, `source_index` (0-based line in the source file), `source_repo` (const `idllresearch/malicious-gpt`) and `source_commit`, followed by the upstream fields unchanged: `project_name` (string), `hosting_platform` (string or null, null for the 3 underground-service records), `prompt_is_visible` (boolean or null) and `prompt` (string).
- **Licence gap.** The upstream repository declares no licence, so redistribution rights are unstated. The corpus is usable internally for research and is not shippable or publishable until the authors clarify. This diverges from the 11.1 rule (a licence stated on the distribution page) and from the brief's data-handling rule, and it is recorded as an open item (reconciliation row 58, decision register D008), not accepted silently.
- **Handling rules.** Prompts are untrusted data: never executed, never pasted into a production prompt, never sent to a production system. A probe run sends them only to an explicitly entitled Pythia persona with the permission-gate-only guardrail default (`docs/ops/pythia.md`), provisioned as a separate key from the hardening writer's `default` persona. The corpus stays in the gitignored cache and the team's blob store. No prompt text is committed to the repository or quoted in documentation, and a report shows one only as the recorded input of a specific probe result.
- **Evidence model for the LLM track.** An LLM assessment records, per probe, the test context (probe id, the corpus record by `source_file` and `source_index`, persona and model), the observed response, the criterion, the detector verdict and the reviewer assessment. SHAP is not required. Measurements, observations, interpretation and candidates stay separate fields and separate panels (section 14). No MRI, no grade and no cross-modality aggregate is derived from these results.
...
Phase B rows are registered only when their adapter, estimator support, and tests exist; until then `GET /v1/attacks` does not list them and the UI shows the modality as unavailable with the reason (S2 honesty rule; section 18). LLM-domain red-teaming (garak) is Phase B and, if added, points garak's OpenAI-compatible generator at Pythia (D6); it is not an entry in this catalog. Its probe material is the malicious-gpt jailbreak corpus of 11.6 (200 prompts, licence unresolved, D008), treated as untrusted data and sent only to the permission-gate-only Pythia persona.
...
| B1+ | Stretch (Phase B) | Black-box endpoint connector over `AuthProfile` with egress allowlist; Carlini-Wagner, DeepFool, HopSkipJump (image), ZOO; text and detection modalities; KernelSHAP for black-box; adversarial-training defense; garak LLM-domain probes pointed at Pythia (D6) with the malicious-gpt jailbreak corpus of 11.6 as probe material (licence unresolved, D008); pickle override (admin-only, sandboxed, audited); finding review states beyond dismissal (section 6.4); per-project scoring overrides; `Idempotency-Key`; PDF export. | after Phase A | Deferred extensions (specs/README.md) | Each item appears in the UI as not implemented with a reason until it lands. |
