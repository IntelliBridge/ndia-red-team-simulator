...
5. [Domain model mapping onto redsim tables](#5-domain-model-mapping-onto-redsim-tables)
...
The Adversarial ML Red-Team Simulator is a web tool that stress-tests a machine-learning classifier under adversarial evasion attacks before anyone relies on it. A user selects a model (bundled or uploaded), launches an attack campaign, and views the SHAP explanation and the hardening recommendations side by side, with the whole trail recorded on a hash-chained audit log. It is built as one new vertical (`redsim/ml/`) inside the redsim platform: IntelliBridge's aegis security platform as forked into `IntelliBridge/ndia-red-team-simulator` and renamed to the `redsim` namespace (D7 as amended, section 4). It reuses that platform's API, workers, storage, auth, RBAC, tenancy, audit chain and LLM policy layer (section 2).
...
- Measurements (what was counted), observations (per-sample evidence), interpretation (inferred statements) and candidate recommendations are separate fields in the evidence model and separate panels in the UI (section 14, section 18). The labels `candidate`, `not evaluated`, `inferred` and `heuristic` are enforced by `Literal` types in `redsim/ml/schema.py`.
...
The roles are analysts who register models and run campaigns, reviewers who confirm findings and approve configurations independently of their authors, owners/admins who manage projects, members and policy, and viewers who read evidence (section 7). A session follows the redsim lifecycle: register or pick a model → configure and launch a campaign → watch jobs complete → inspect the MRI scorecard, robustness curve and findings → open a finding's three-pane screen (input pair, SHAP, candidate recommendations) → run the verify loop → read the measured delta → export the report → inspect the audit trail (sections 10, 18, 24).

### 1.4 Vocabulary

| Term | Meaning in this spec |
|---|---|
| **Model target** | An redsim `Target` row with `kind` `ml_model_artifact` (Phase A) or `ml_model_endpoint` (Phase B); `value` is the blob location or the inference URL (section 5). |
| **ML target (protocol)** | The loaded model plus its evaluation dataset slice, as `redsim/ml/targets/base.py:Target` (`sample`, `predict_proba`, `art_classifier`, `torch_model`, `manifest`). Distinct from the ORM row above. |
| **Campaign** | One redsim `Run`: one model × one modality × a declared attack set × a declared ε grid × a reference budget × a sample dataset (with revision) × scoring weights. This is also the unit for which an MRI exists. |
| **Campaign configuration** | The approved evaluation profile (F003) stored immutably with the `Run` as `CampaignConfig` (section 5). |
| **Job** | One redsim `Job` of type `model.validate`, `attack.run`, `explain.run`, `harden.recommend` or `verify.replay` (section 10). |
| **Finding** | One redsim `Finding`: an attack family whose success rate crossed the configured threshold at some ε; severity derived per section 15. |
| **Measurement / Observation / Interpretation / Candidate recommendation** | The four separate evidence fields of `redsim/ml/schema.py` (section 14). |
...
| **F001–F008** | The Spec Kit features of `specs/`, mapped onto redsim components in section 19. |
...
|---|---|---|
| FastAPI `/v1` API, Celery workers, Postgres, Redis | `redsim/api`, `redsim/workers` (`celery_app.py`, `bootstrap.py`, `job_state.py`, `tasks/`), `redsim/db`, `deploy/docker-compose.yml` | Validate, attack, explain, harden and verify jobs run asynchronously on workers; the API process never loads a model (section 10). |
| Hash-chained audit log (Postgres-backed, JSONL offline) with WORM export to S3 Object Lock | `redsim/audit/chain.py`, `redsim/audit/forensic.py`, `redsim/storage/worm.py`, `redsim/workers/tasks/worm_export.py`, `redsim audit verify` CLI | Every model upload, attack, explanation, hardening report and verify replay is a chained event; the demo ends on `/audit` (sections 5, 21). |
| RBAC on resource-scoped actions plus pluggable policy engine | `redsim/api/policy.py` (roles `scanner` < `remediator` < `approver` < `admin`, project memberships), `redsim/policy` (static / OPA / Cedar), `deploy/opa`, `deploy/cedar` | William's Owner / Analyst / Reviewer / Viewer matrix maps onto these roles; the independent-approval rule is enforced here (section 7). |
| Postgres row-level security by `org_id` | `redsim/db` models and migrations (`redsim/migrate`), `tests/test_tenant_rls.py` | Model artifacts, findings and artifacts inherit tenant isolation; one tenant never sees another's model or results. |
| `Run` / `Job` / `Finding` / `Artifact` models with typed vocabularies | `redsim/db/models.py`, `redsim/schema.py` (`Severity`, `Status`, `Confidence` literals), `redsim/workers/job_state.py` | Map directly to campaigns, attack jobs, adversarial findings and SHAP / adversarial-example outputs (sections 5, 6). |
| Generic registry with duplicate detection; scanner registry and capability vocabulary | `redsim/registry.py`, `redsim/scanners/registry.py`, `redsim/scanners/severity.py` | ART attack adapters and SHAP explainers register the same way scanner adapters did. New capability tags: `adversarial_ml`, `explainability`. The 14 pentest adapters and the `/tools` page are deleted, so attacks are listed at `GET /v1/attacks` and in the launcher UI instead (sections 12, 17). |
| Out-of-process plugin sandbox (`REDSIM_PLUGINS_SANDBOX`) | `redsim/scanners/sandbox.py`, `redsim/scanners/sandbox_worker.py`, `redsim/supply_chain` | Uploaded model files are loaded only inside this pattern on the worker: separate process, rlimits, wall-clock kill, minimal allow-listed environment, network off by default (section 9). |
| Verify-after-fix pattern (`verify.replay`) | `redsim/workers/tasks/verify.py`, `redsim/services/verify.py`, `redsim/verify.py` (`VerifyStatus` = `verified` / `still_vulnerable` / `inconclusive`, mapped by `_STATE_MAP` onto `Finding.validation_state` = `poc_passed` / `poc_failed` / `inconclusive`) | Re-attack after applying a preprocessing defense; write the measured ΔMRI and flip `validation_state` (sections 6, 10, 15, 16). |
| Admission pattern: audit event before enqueue, cancel service, stale-job reaper | `redsim/services/runs.py`, `redsim/services/scans.py`, `redsim/workers/tasks/reaper.py`, `tests/test_admission_audit_before_enqueue.py` | Campaign start, cancel and time-out semantics come for free (section 10). |
| Per-task LLM routing, budget caps, pricing, guardrails, redaction | `redsim/llm/router.py`, `redsim/llm/budget.py`, `redsim/llm/pricing.py`, `redsim/llm/guardrails.py`, `redsim/audit/redact.py` | Policy layer for the recommendation writer. Transport is Pythia only (`redsim/llm/pythia.py`, D5); no provider keys are held (section 16). |
| Managed authentication: Keycloak OIDC on the API, NextAuth in the web app, dev-token mode | `redsim/api/auth.py`, `redsim/api/session_cookie.py`, `deploy/keycloak`, `web/src/app/login`, `web/src/app/api` | Satisfies F001 and constitution principle IV; dev-token mode is allowed for the demo (D11 D002; section 7). |
...
| Blob storage abstraction (filesystem / S3 / MinIO) and run-state facade with sha256 artifact refs | `redsim/storage/blobs.py`, `redsim/storage/s3.py`, `redsim/state/{facade,filesystem,postgres}.py` | Models, adversarial examples, SHAP PNG/JSON and robustness curves are `Artifact` rows with content hashes (section 5). |
| XSS-safe Markdown → HTML report rendering | `redsim/report.py` (`html_escape`, `render_inline_markdown`, `_md_to_html_min`), `redsim/services/reports.py`, `redsim/workers/tasks/report.py` | Markdown / JSON / HTML campaign reports that preserve configuration and provenance (sections 14, 17). |
| Observability: OpenTelemetry, structured logs, Loki, log ingest | `redsim/observability.py`, `redsim/log_ingest`, `deploy/otel`, `deploy/loki`, `Dockerfile.log_ingest` | Worker and API tracing with secret redaction; `/logs` page. |
...
| `AuthProfile` (Fernet-encrypted credentials) | `redsim/services/auth_profiles.py`, `web/src/app/auth-profiles` | Reserved for black-box endpoint credentials in Phase B (section 5). |
...
| Access and tenancy | Keycloak OIDC + NextAuth (dev-token mode allowed for the demo), redsim RBAC with William's matrix mapped onto redsim roles, Postgres RLS. | D1; D11 D002; section 7 |
...
**3.2 status addendum (2026-09-09, after Phase B waves B0 to B4).** Read from the tree at the wave B4 push, item by item, with the owner decision of `docs/plans/12-phase-b-plan.md` section 2 that bounds it where one does. "Built" means the path runs through the API, the worker and the sandbox child and has end-to-end evidence under `tests/e2e/`; "built with limitations" names the limitation; "non-goal by decision" names the decision.

| Item | Status at the B4 push | Note |
|---|---|---|
| Ingest: black-box endpoint connector | built with limitations (wave B2 `endpoint-admission`, B1 `endpoint-target`) | `POST /v1/models` with `source: "endpoint"`, `AuthProfile` credentials, `Target.kind = ml_model_endpoint`, the `endpoint-v1` contract (`docs/api/endpoint-contract.md`), queries only from the worker-parent broker. Limitations: no DNS-TXT ownership check (non-goal by owner default ENDPOINT-26: allowlist, admin registration and the audited D3 attestation instead); the broker does not pin the connection to the resolved address (ENDPOINT-07 risk note). The B4 e2e file `tests/e2e/test_ml_endpoint.py` registers and refuses correctly; its three campaign-path cases fail on the tree at the B4 push on one attributed defect (`redsim/ml/targets/endpoint.py:228` sends the 8-row probe unscaled), recorded in the README's open items. |
| Modalities: text and detection | built (wave B1 `text-modality`, `detection-modality`; admitted since B2) | `sms_tfidf_lr` with `word_substitution` under the `edit` norm; `assets_frcnn_mnv3` with `dpatch` under `patch_area`. Detection carries a scorecard and never an MRI (owner default MODALITIES-36). Detection has no SHAP explainer (`ExplainerUnavailable` recorded honestly). Evidence `tests/e2e/test_ml_text_detection.py`. |
| Attacks: CW-L2, DeepFool, image HopSkipJump, ZOO, word substitution, DPatch | built (wave B1 `attacks`) | Image HopSkipJump is the Phase A adapter with `IMAGE_DEFAULTS` (divergence from 12.2, recorded). `adv_patch` is a recorded non-build (MODALITIES-32): `dpatch` is the detection attack. Evidence `tests/e2e/test_ml_attacks_harden.py`. |
| Explain: KernelSHAP for black-box models, text token attributions | built with limitations (wave B1 `explain-blackbox`, `text-modality`) | KernelSHAP serves predict-only tabular targets (including endpoints) with query caps; KernelSHAP for images is a recorded non-build (ATTACKS_HARDEN-08, `PartitionExplainer` stays, divergence from 13.2). Text attributions through `shap.maskers.Text`. |
| Harden: adversarial training with the measured delta, defensive distillation | built with limitations (wave B1 `hardening`, B2 worker registration) | Both run as the `defense_apply` stage in the verify child; the derived weights register as a new `Target` with `derived_from` lineage; distillation is native torch with ART's class cited (divergence from 16.5). Limitation at the B4 push: no bundled image target exposes a training slice to the child (`train_sample`), so `apply_training_defense` records the defense unavailable, the verify run succeeds on the undefended model with the score withheld and no derived target or `MeasuredDelta` exists (attributed in `tests/e2e/test_ml_attacks_harden.py`; open item ATTACKS_HARDEN-11/-13). The tabular tree ensemble is a typed refusal by construction. |
| LLM-domain red-teaming: garak through Pythia | built (wave B2 `llm-core`, `llm-api`) | 103 catalogued probes, the offline `redsim-core` set of 76, `PythiaGenerator` at the gateway, a probe key in an `AuthProfile` with its own persona (owner default LLM-26), k/n scorecards that never enter an MRI. HarmBench excluded (owner default LLM-08). Evidence `tests/e2e/test_ml_llm.py` (real garak 0.16.0 against the fake gateway); no run against the live gateway is recorded. |
| Review workflow: the full state set | built (wave B2 `review-workflow`) | `draft`, `in_review`, `confirmed`, `dismissed`, `retest_requested`, `resolved` over one transition table, independence by identity, compare-and-set, `resolve` gated on a compatible retest, analyst drafts. No `reviewer` role (owner default REVIEW_REPORTS-13). Evidence `tests/e2e/test_ml_review_reports.py`. |
| Reports: PDF export and run comparison beyond the verify delta | built (wave B2 `reports-compare-weights`; B4 completion render) | `report.pdf` as a third projection of the record, immutable snapshots, `GET /v1/runs/compare?ids=` with no mean or rank, per-project weights never renormalised. Since wave B4 the campaign completion path renders all four formats and records the run's first snapshot; a record whose PDF the renderer cannot typeset degrades to the three text formats with `pdf_unavailable` on the `report.render` row (open item: `redsim/ml/pdf.py` wide tables). |
| Trust override for pickles | non-goal by decision (REVIEW_REPORTS-33) | Pickles stay refused; no override exists. |
| Interoperability (section 27) | built with limitations (wave B3 `interop-contribute`, `interop-consume`, `atlas-foundry`) | Croissant/Parquet export, the consume side, ATLAS tags with the coverage view, the Foundry push (opt-in, off by default, proven against a fake server only: owner default INTEROP-26). Lattice is text only (non-goal by decision D3, INTEROP-27 / TESTS_DOCS-41). Limitations at the B4 push: the worker parent does not yet materialise a consumed slice for the target loader (INTEROP-16 remainder), the dataset push to Foundry is not built (INTEROP-23), imagery exports stay in the artifacts bucket (INTEROP-34). Evidence `tests/e2e/test_ml_interop.py`. |
| Bulk operations (the owner's requirement, plan 12) | built (wave B3 `bulk-service-routes`, `bulk-upload-capacity-cli`; B4 single-run capacity and worker projection) | Batch campaigns, bulk upload, bulk verify as one defended run projected onto every selected finding (owner default BULK-16, the worker projection since B4), per-project capacity on every admission (BULK-20/-21 since B4), the CLI matrix. Limitation: a `state_dict` upload of a `small_cnn` for a dataset with other than 10 classes is refused at validation because no route derives `architecture_kwargs` from the dataset binding (open item, attributed in `tests/e2e/test_ml_bulk.py`). |
...
- Agent applications and tool-using agents (a separate scope per the brief; redsim's agent runtime is deleted).
...
| D5 | All LLM calls go through Pythia; redsim routing and budgets remain the policy layer; env `PYTHIA_BASE_URL`, `PYTHIA_API_KEY`, `PYTHIA_PERSONA`, `REDSIM_ML_LLM_MODEL`; text-only inputs; one plain non-streaming completion. |
...
| D7 (amended 2026-09-08, later) | Python namespace and all identifiers renamed to redsim: package `redsim/` and vertical `redsim/ml/`, console script `redsim`, environment variables `REDSIM_*` (including `REDSIM_ML_LLM_MODEL`), API title "Redsim API", session and CSRF cookies `redsim_api_session` / `redsim_csrf` with the `X-Redsim-CSRF` header, compose services and images `redsim-*`, chart `deploy/helm/redsim`, web packages `@redsim/web` and `@redsim/design-system`, config `redsim.yaml`, workflow `redsim-ci.yml`. `aegis` remains only as the upstream fork name. Supersedes the "namespace `aegis`" clause of D7 and the "no code identifier is renamed" clause of row 56 in 4.2 (see row 57). |
...
| D10 | William's Spec Kit tree becomes the feature-level layer beneath this spec; F001–F008 mapped onto redsim components; Replit paths replaced by redsim paths; redsim state machines canonical. |
...
Source keys: **S1** = John Sasser's product spec (`docs/adversarial-ml-redteam-spec.md`); **S2** = the lean design (`docs/superpowers/specs/2026-09-08-redsim-design.md`) and the code contracts in `redsim/ml/`; **S3** = William Yiu's Spec Kit tree (`.specify/`, `specs/`); **Brief** = `docs/project-brief.md`; **Const.** = `.specify/memory/constitution.md`.

| # | Topic | Positions in the sources | Decision | Consequence in this spec |
|---|---|---|---|---|
| 1 | Platform and stack | **S1:** full redsim (API, Celery, Postgres, Redis, S3, auth, RBAC, RLS, audit). **S2:** strip redsim to `redsim/`; no DB, queue, auth, tenancy or audit; `ThreadPoolExecutor` in the API process, `run.json` state, SWR polling. **S3:** reuse the Replit pnpm monorepo (`artifacts/api-server`, `lib/api-spec`, `lib/db`, `artifacts/ai-assurance`); no worker runtime selected. **Brief:** no implementation implied. | D1 | Sections 2, 8, 20. S2's in-process runner and `run.json` are replaced by Celery jobs and Postgres; S3's paths are replaced by redsim paths (row 30). |
| 2 | Product, package and env naming | **S1:** aegis fork, `aegis/ml/`. **S2:** `redsim`, `redsim/` package, `@redsim/design-system`, `REDSIM_LLM_MODEL`, `redsim_version`, footer title "redsim". **S3:** "AI Assurance". | D7 (as amended), D5 | Product is "Adversarial ML Red-Team Simulator" (short name redsim, row 56). D7 first kept the `aegis` namespace and was amended later on 2026-09-08 (4.1, row 57): namespace `redsim`, vertical `redsim/ml/`, web `@redsim/web`, `Provenance.redsim_version`, and `aegis` remains only as the upstream fork name. `redsim/llm/pythia.py` and the `RunConfig.llm_narrative` comment still read `REDSIM_LLM_MODEL` and change to `REDSIM_ML_LLM_MODEL` at M0 (sections 20, 22, 23). "AI Assurance" survives only as the constitution's title and the Spec Kit tree's name for the process layer. The short product name is settled in row 56. |
...
| 26 | Run states | **redsim (S1):** `Job` machine `queued → running | cancelled`, `running → succeeded | failed | cancelled | queued` (retry requeue), terminal sinks; `Run.status`. **S2:** `queued / running / succeeded / failed / not_implemented` with a `stage` field and `STAGES`. **S3:** `queued / running / cancel_requested / completed / failed / cancelled / timed_out` with ordering rules for racing completion. | D10 | redsim's `Job` machine (`redsim/workers/job_state.py`) is canonical. S3's states map onto it in section 6 (`completed` ↔ `succeeded`; `timed_out` ↔ `failed` with a reaper or timeout reason; `cancel_requested` ↔ the cancel service's transient). S2's `not_implemented` is an admission-time 501, never a `Job`; S2's `stage` becomes per-job progress on the stage table. |
| 27 | Finding review states | **redsim:** `Finding.status` `open / fixing / fixed / failed / false_positive`; `validation_state` `unvalidated / poc_passed / poc_failed / inconclusive`. **S2:** a free-text `reviewer_notes` field. **S3:** `draft / in_review / confirmed / dismissed / retest_requested / resolved`, independent approval, revisions, retest links. | D10 | redsim enums are canonical; the mapping and the Phase A / Phase B decision on the review workflow are stated in section 6 (dismissal in Phase A; confirm / resolve Phase B). Phase A keeps `reviewer_notes`. |
| 28 | Roles | **redsim:** `scanner` < `remediator` < `approver` < `admin` plus project memberships (`redsim/api/policy.py`). **S3:** Owner / Analyst / Reviewer / Viewer with "approve, except own authored item", last-owner protection, invitation lifecycle. **S2:** none. | D1, D10 | William's matrix is mapped onto redsim roles in section 7 (read ↔ membership; register / start / cancel ↔ `scanner` and `remediator`; approve / confirm ↔ `approver`; manage ↔ `admin`); the independent-approval rule is enforced for profile approval and finding confirmation. |
...
| 30 | Implementation locations | **S3:** `artifacts/api-server/src/{routes,services}/assurance/`, `lib/api-spec`, `lib/api-client-react`, `lib/api-zod`, `lib/db/src/schema/`, `artifacts/ai-assurance/`. **S1:** `redsim/ml/{loaders.py,attacks/,explain/,harden.py,eval.py}`, `redsim/workers/tasks/{attack,explain,harden}.py`. **S2:** `redsim/…`. | D7, D10 | redsim paths only (section 8 layout); S3's paths are replaced in section 19 and in the feature plans. |
...
| 37 | Limitations | **S2:** `limitations` always non-empty; standing sentences include "CIFAR-10 is not a proxy for any operational domain". **S1:** none. **S3:** F003 requires explicit limitations; F005 shows explanation limitations. | Retained; D3 | Limitations always present on the run page and in every report; the dataset sentence is parameterised by the campaign's dataset and its licence; `STANDING_LIMITATIONS` in `redsim/ml/schema.py` changes accordingly (section 14). |
...
| 40 | Deployment | **S1:** ECS Fargate + RDS + S3 + ElastiCache; Terraform or Copilot; Keycloak or Cognito. **S2:** docker-compose with two services (api, web). **S3:** Replit. | D1 | redsim compose dev stack (api, worker, web, postgres, redis, MinIO, Keycloak) now; Fargate at M7; `ml` extra only in the worker image. Section 20. |
...
| 42 | Listing attacks in `/tools` | **S1:** attacks register "so they list in `/tools`". **Repo state:** `/tools` page and `tool.invoke` deleted. | D1 (as restored) | Attacks and explainers register through `redsim/registry.py` with the `adversarial_ml` / `explainability` tags and are listed at `GET /v1/attacks` and in the launcher (sections 12, 17). |
...
| 47 | Agents and tool-using agents | **Brief:** agent applications are a separate scope. **redsim:** agent runtime deleted. **S3:** deferred extension. | Non-goal | Section 3.3. |
| 48 | Retention, export redaction, licence restrictions | **S3:** D006 OPEN; minimise retained content; block destructive purge until approved. **S1:** WORM Object Lock; redaction on the LLM path. | D11 (stays OPEN) | Phase A uses redsim redaction on LLM inputs and audit exports and adds no retention or purge operation; the row stays open in the register with no invented owner. Sections 19, 21. |
...
| 56 | Product short name | **D7:** "redsim" retired except as history. **S1 §14 (added 2026-09-08):** uses "redsim" as the product's short name throughout. **S2:** "redsim" was both the package and the product name. **Repo:** the branch is `redsim-implementation`. | D7 as amended by this merge: short name accepted | The short name **redsim** is accepted for the product, and the long form stays "Adversarial ML Red-Team Simulator". The title of this spec reads "Adversarial ML Red-Team Simulator (redsim)". This row renamed no code identifier; the later amendment of D7 (4.1, row 57) did: the Python namespace is `redsim`, the vertical is `redsim/ml/`, environment variables are `REDSIM_*`, compose services and images are `redsim-*`, and `REDSIM_LLM_MODEL` still becomes `REDSIM_ML_LLM_MODEL` at M0 (row 2). The web header brand keeps the long form (section 18). |
| 57 | Namespace and identifier rename | **D7 (original):** namespace `aegis`, vertical `aegis/ml/`, web app `@aegis/web`, compose services and images `aegis-*`, environment variables `AEGIS_*`. **Repo (commit `b39d933`, 2026-09-08):** package `redsim/`, console script `redsim`, `REDSIM_*`, "Redsim API", `redsim_api_session` / `redsim_csrf` / `X-Redsim-CSRF`, `redsim-*` services and images, `deploy/helm/redsim`, `@redsim/web`, `@redsim/design-system`, `redsim.yaml`, `redsim-ci.yml`. | D7 as amended (2026-09-08, later) | Every identifier in this spec reads redsim. `aegis` appears only as the upstream fork name and in history (sections 2.1 and 2.3, the recorded D1 and D7 rows in 4.1, and source columns in this table). The M0 rename is `REDSIM_LLM_MODEL` to `REDSIM_ML_LLM_MODEL` (row 2). No section is renumbered. |
...
| 58 | LLM-track corpus: malicious-gpt withdrawn, garak bundled data adopted | **Brief, 11.1, D3:** every dataset carries a licence stated on its distribution page. **Owner decision 2026-09-08 (later):** fetch a third-party jailbreak corpus (no licence declared upstream) as probe material for the Phase B LLM-assistant domain (D6). **Owner decision 2026-09-08 (later still):** withdraw it and use the probe corpora garak ships and loads itself (package licence Apache-2.0). | Corpus withdrawn 2026-09-08 for lack of a licence. garak's bundled data adopted for the Phase B LLM track. D008 closed as moot. | Section 11.6 lists garak's bundled corpora and the handling rules. Nothing is extracted or re-packaged for this tool: garak's probe classes and detectors load their own files. Phase B only, probe material only, never a classifier dataset or an MRI input (D9). 11.5 carries the row (Apache-2.0 packaging, upstream licences per subset, HarmBench ships its own LICENSE), 12.2 and milestone B1 (section 23) name it. The local download and the combined JSONL were deleted from the asset cache. |

### 4.3 Constraints carried forward unchanged

These items did not conflict and are binding throughout the spec:

- Measurements, observations, interpretation and candidate recommendations are separate fields (`redsim/ml/schema.py`) and separate UI panels; the labels `candidate`, `not evaluated`, `inferred`, `heuristic` are `Literal` types (S2; Brief; Const. III).
...
- Provenance for rerun: model sha256, dataset id and revision, split, slice indices and seed, attack and explainer parameters, library versions (`torch`, `art`, `shap`, `numpy`, `redsim_version`), host and device, nondeterminism sources (S2; Const. V; Brief).
...
- Audit event before enqueue for every mutating call (S1 §10; redsim admission pattern).
...
- D008 closed as moot (2026-09-08, later): the corpus it concerned was withdrawn for lack of a licence and the Phase B LLM track uses the probe corpora garak ships (11.6, row 58). Nothing is blocked.
...
### 4.5 P0 freeze resolutions (2026-09-08)

P0 / M0 (PR #18, branch `P0`, merged into `main` as `4350d38` on 2026-09-08, 868 passed and 30 skipped offline per the PR body) froze `redsim/ml/schema.py`, migration `0010_ml_vertical`, the seven `Action` members of 7.4 with the `viewer` rank, the `REDSIM_ML_LLM_MODEL` rename, the `/v1/scans` unmount and the `redsim ml build-assets` skeleton. Where the frozen tree and this text disagreed, the PR resolved the point in favour of the tree. The rows below record each resolution the way 4.2 records the source divergences, numbered as in the PR body. A later change to the frozen contract follows the protocol in `docs/plans/01-p0-contracts-api-skeleton.md` section 8: announce it first and prefer additive fields with defaults. Sections 5, 12, 13, 14 and 15 were corrected in place where they stated the earlier form. Sections 8.3, 11.5, 16.3, 18.3, 20.3, 22.3 and 23 still describe some of these items as pending M0 work and are read as done.

| # | Topic | Spec text before P0 | Frozen on `main` | Consequence in this spec |
|---|---|---|---|---|
| 1 | LLM model env var | The PR body notes the spec read `AEGIS_ML_LLM_MODEL`. After the D7 amendment the text names `REDSIM_ML_LLM_MODEL` (D5, 16.3, 20.3), but rows 2, 56 and 57 and sections 8.3, 16.3, 20.3 and 23 still describe the rename from `REDSIM_LLM_MODEL` as a pending M0 task. | `REDSIM_ML_LLM_MODEL`, read by `redsim/llm/pythia.py`, `tests/test_llm_pythia.py` and `.env.example`, named in the `redsim/ml/schema.py` comment. | The rename is done. Every "at M0" mention of it reads as complete. |
| 2 | `RunStatus` | 5.3 said `RunStatus` is unchanged, while 6.2 makes `cancelled` a run status. | `RunStatus = Literal["queued", "running", "succeeded", "failed", "cancelled", "not_implemented"]`. | 5.3 corrected. `not_implemented` stays an admission-time 501 that is never persisted (5.4, row 26). |
| 3 | `AttackInfo` extras | 17.2 listed `phase`, `access`, `requires_gradients`, `status` and `reason` as what `GET /v1/attacks` returns beside `AttackInfo`, not as fields of it. | `AttackInfo` carries the five with defaults: `phase="A"`, `access="white-box"`, `requires_gradients=True`, `status="available"`, `reason=None`. | Additive. `GET /v1/attacks` returns the model as is (12.1, 17.2). `AttackInfo` has no ATLAS fields: the tag stays at `MLFindingDetail.atlas_technique` for Phase B2 (5.7, 27.2). |
| 4 | `Measurement.params` value type | 12.3 puts `norm` in `params`, while the scaffold typed the values as `float \| int \| bool`. | `params: dict[str, float \| int \| bool \| str]`. | 12.5 states the value type. |
| 5 | Explanation noise-floor fields | 13.5 records `expl_shift_noise_floor` with its `n` and `expl_shift_n_excluded` on the reference row, but 5.3 did not list them. | `Measurement` gains `expl_shift_n_excluded`, `expl_shift_noise_floor` and `expl_shift_noise_floor_n`. | 5.3 and 13.5 list them. The noise floor stays outside the MRI (13.5, 15.1). |
| 6 | `RunSummary` attack field | The scaffold's `RunSummary` carried a single `attack_id`. No section of this spec described it. | `RunSummary.attack_ids: list[str]`, matching the campaign shape. | None in the text. Recorded so any run-list projection uses the frozen name. |
| 7 | Where `finding_asr_threshold` lives | 15.3 listed it as a key of the `ml.scoring` block, which is copied into `CampaignConfig.scoring`. | `CampaignConfig.finding_asr_threshold` (the request field of `POST /v1/models/{id}/attacks`, default 0.2), copied onto `MRIRecord.finding_asr_threshold` at scoring time. `ScoringConfig` does not carry it. | One source of truth. 15.3 corrected. 5.6, 12.3, 12.6, 15.7 and 17.2 already placed it on the campaign. |
| 8 | Standing limitations | 14.5 described `STANDING_LIMITATIONS` "retained with two changes" (dataset template, conditional budget sentence), and 5.3 said its first entry is parameterised by dataset id. | `STANDING_LIMITATIONS` keeps the five dataset-independent sentences. `standing_limitations(dataset_name, eps_grid)` returns the dataset sentence (`DATASET_LIMITATION_TEMPLATE`), then the budget sentence (`SWEEP_LIMITATION_TEMPLATE` when the grid has more than one member, else `SINGLE_BUDGET_LIMITATION`), then the five. | 5.3, 5.6 and 14.5 corrected. Where 11.5 and 18.3 name `STANDING_LIMITATIONS` as the per-run list, and 22.3 as the home of the dataset template, read the output of `standing_limitations(...)`. |

Two further facts of the frozen module that no earlier section contradicted, recorded here so the shape can be read from one place: `Measurement` has no `severity` field (severity is finding-level, derived per 15.5 with `SeverityThresholds`), and `CampaignRecord`, the `GET /v1/runs/{id}/campaign` response of 17.2, extends `RunRecord` with `kind`, `completed_at`, `settings_hash`, `baseline_run_id`, `parent_run_id`, `curve`, `completeness`, `missing` and `score_status`. Beyond the eight, two type columns were aligned to the frozen module without a change of meaning: 5.5 reads `MLModelManifest` as frozen (`dataset_revision`, `license`, `source_url` and `manifest_sha256` nullable, and `surrogate.agreement_clean` an `AccuracyPoint` with `n`, `n_correct` and `accuracy`), and 14.4 reads `Provenance.sample_indices_sha256` and `settings_hash` as `str \| None`.

## 5. Domain model mapping onto redsim tables

### 5.1 Ground rules

- The ML vertical reuses the redsim tables in `redsim/db/models.py` (`Organization`, `Project`, `User`, `ProjectMembership`, `Target`, `AuthProfile`, `Run`, `Job`, `Finding`, `Artifact`, `LLMUsage`, `AuditEvent`, `AuditChainHead`, `ApplicationLog`). Every row keeps its `project_id`, and the eight RLS-scoped tables keep their trigger-backfilled `org_id` (section 7.6).
...
- The redsim idiom for typed JSONB payloads is a `TypedDict` or Pydantic model next to the column (`FixJobDetail`, `VerifyJobDetail`, `RedsimFinding` for `findings.schema_blob`). The ML payloads follow that idiom and are validated **at write time on the worker** with the Pydantic models in `redsim/ml/schema.py`; reads stay lenient the way `RedsimFinding.from_dict` is.
- The single source of truth for one campaign's evidence is the `RunRecord` (S2, `redsim/ml/schema.py`) written as an immutable, sha256-addressed Artifact (kind `ml.run_record`). Everything that lands in `Finding.schema_blob` or `ml_campaigns.score` is a **projection** of that record, never a second computation. A projection that disagrees with the record is a bug; the testing plan (section 22) asserts equality.
...
| `Target.value` (`String(1024)`) | Blob-store location of the model file for uploads (`redsim_output/blobs/...` or `s3://bucket/<project>/models/<target>/<sha256>`), the bundled asset id (`bundled:<id>`) for sample models, or the inference URL for endpoints. | D2 |
...
| `Target.verified`, `allowlist_until`, `installation_id` | Untouched. Ownership verification returns `501` in this build (`redsim/api/v1/targets.py`) and is not needed for model artifacts; sandbox validation status lives in `detail.status`, not in `verified`. | D2 |
...
| `Run.stage_table` (JSONB) | Campaign progress in the `STAGES` vocabulary of `redsim/ml/schema.py` (`load_target, sample, clean_eval, attack, control, explain, score, interpret, recommend, report`; `score` is added by this spec), plus per-job status (section 6.5). Not the place for scores. | S2 |
...
| `Finding.schema_blob` (`RedsimFinding` dict) | Carries the measured summary fields in the existing `RedsimFinding` fields (title, severity, description, evidence, references) and the full ML detail in a new optional `ml` field (`MLFindingDetail`, section 5.7): the attack's eps-sweep measurements with `n`, the control rows at the same eps, observation ids and artifact ids, the interpretation rows that rest on them, and the candidate recommendations they trigger. Phase B2 adds the MITRE ATLAS technique tag inside the same block (`ml.atlas_technique`, section 27.2). | S2; S1 §14 (B2) |
...
| `Finding.status` | redsim vocabulary `open / fixing / fixed / failed / false_positive` (canonical, section 6.4). | D10 |
| `Finding.validation_state` | redsim vocabulary `unvalidated / poc_passed / poc_failed / inconclusive` (canonical; written by `verify.replay`, section 6.4). S1 §4's "`verified` / `still_vulnerable`" are the worker's `VerifyStatus` values that map onto these through the existing `_STATE_MAP`. | D10 |
...
| Scanner registry vocabulary (`redsim/scanners/registry.py`) | New capability tags `adversarial_ml`, `explainability`. Attack adapters register there so they list the way scanner adapters did. | S1 §4 |

### 5.3 Schema changes in migration `0010_ml_vertical` and in `redsim/ml/schema.py`

| Change | DDL / code | Why |
|---|---|---|
| `targets.detail` | `ADD COLUMN detail JSONB NULL` | Model manifest (5.5). |
| `ml_campaigns` | New table, columns in 5.6. `run_id` is PK and FK to `runs.id`; `project_id` NOT NULL FK; `org_id` nullable FK with the `redsim_set_org_id_ml_campaigns` BEFORE INSERT trigger, the `redsim_check_org_id_ml_campaigns` BEFORE UPDATE guard, `ENABLE` + `FORCE ROW LEVEL SECURITY`, and the `redsim_tenant_isolation` policy copied verbatim from `0006_tenant_rls.py` / `0009_tenant_org_id_guard.py`. | Campaign/score record; tenant isolation parity with the other eight scoped tables. |
...
| `RedsimFinding.finding_type` | Add `"adversarial_ml"` to the `FindingType` Literal in `redsim/schema.py`. | New writes are validated at construction; an out-of-vocabulary type fails at the source. |
| `RedsimFinding.ml` | Add `ml: dict[str, Any] | None = None` to `RedsimFinding`. The dict is the `model_dump()` of `MLFindingDetail` (defined in `redsim/ml/schema.py`, validated by the worker before `to_dict()`). Keeping the field a plain dict in `redsim/schema.py` avoids an import from `redsim.ml` in the core schema and keeps `extra="ignore"` semantics: without this field the ML detail would be **silently dropped** by `RedsimFinding.model_validate`. | Field placement (5.7). |
| `redsim/ml/schema.py` additions | New models `CampaignConfig`, `ScoringConfig`, `DefenseConfig`, `MRIInputRow`, `MRIRecord`, `MRIDelta`, `MeasuredDelta`, `MLFindingDetail`, `MLModelManifest`. `RunConfig` is generalised into `CampaignConfig` (5.6: `attack_ids`, `attack_params`, `norm`, `eps_grid`, `reference_eps`, `finding_asr_threshold`, `scoring`, `defense`, dataset binding, plus the existing `n_samples`, `seed`, `include_control`, `explain_k`, `llm_narrative`); `RunRecord.config` becomes `CampaignConfig` and `RunRecord.attack` becomes `attacks: list[AttackInfo]`. `Provenance` gains `dataset_revision`, `sample_indices_sha256`, `settings_hash`, `baseline_run_id`, `parent_run_id`, `defense`, `llm`, `thread_env`, `onnxruntime`, `sklearn`, `xgboost` (sections 11, 14.4). `Measurement` gains the scoring inputs of sections 12.5, 13.5 and 15.1 (`n_clean_correct`, `attack_success_rate`, `pert_first_success_mean`, `pert_first_success_n`, `conf_gap_mean`, `conf_gap_n`, `expl_shift_mean`, `expl_shift_n`, `expl_shift_n_excluded`, `expl_shift_noise_floor`, `expl_shift_noise_floor_n`, `queries_mean`), and its `params` values may be strings because `norm` travels in `params` (4.5 rows 4 and 5). `Observation` gains `expl_shift`, `top_features_clean`, `top_features_adv` (section 13.3); `Observation.artifacts` values become `Artifact.id`s (not run-relative paths). `CandidateRecommendation.validation` becomes `Literal["not evaluated", "measured"]` with a paired `measured: MeasuredDelta | None` (section 16.4). `RunRecord` gains a model validator rejecting dangling citations (section 14.1). `STAGES` gains `score` after `explain`. `RunStatus` gains `cancelled` (6.2, 4.5 row 2). `AttackInfo` gains `phase`, `access`, `requires_gradients`, `status` and `reason` with defaults (4.5 row 3). `RunSummary.attack_id` becomes `attack_ids` (4.5 row 6). `Domain` is unchanged. | Sections 5.4 to 5.7, 12, 13, 14, 15, 16. |
| `STANDING_LIMITATIONS` and `standing_limitations()` | `STANDING_LIMITATIONS` drops the CIFAR-10 sentence and keeps the five dataset-independent sentences. `standing_limitations(dataset_name, eps_grid)` builds the per-campaign list (section 14.5): the dataset sentence "`<dataset name>` is an open, unclassified public benchmark; it is not a proxy for any operational domain, sensor, or deployment condition.", then the budget sentence, then the five. D3 replaces CIFAR-10 as the demo dataset, so no dataset name is hard-coded (4.5 row 8). | D3 |
...
`RunRecord` (S2 §3, `redsim/ml/schema.py`) is generalised from one attack per run to one campaign per run: `config` becomes `CampaignConfig`, `attack` becomes the list of attacks in the declared set, and measurements carry one row per (family, attack, eps). The record itself is always written whole as the `ml.run_record` Artifact. The table says where each field is **also** projected so the API and UI can query it without parsing the record.
...
| `architecture_id` | `str | None` | required for `torch_state_dict` / `safetensors_state_dict`: the id of a bundled architecture definition (`redsim/ml/targets/architectures.py`); free-form code is never accepted |
...
| `surrogate` | `{"kind", "sha256", "agreement_clean": {"n", "n_correct", "accuracy"} | None} | None` (`SurrogateInfo` with an `AccuracyPoint`) | tabular tree ensembles only: the build-time differentiable surrogate used for PGD by transfer (section 12.2) |
| `dataset_id`, `dataset_revision`, `dataset_split` | `str`, `str | None`, `str` (default `"test"`) | the evaluation dataset the model is bound to (section 11) |
...
| `license`, `source_url` | `str | None` | for bundled models and datasets (D3 open/unclassified rule, section 11) |
| `manifest_sha256` | `str | None` | sha256 of the canonical manifest JSON, copied into `Provenance.model_manifest` |

Phase A bundled targets carried by this manifest (seeded by `redsim ml build-assets`, sections 11 and 20): the image CNN on the vehicle-imagery dataset (11.3.1), also exported to ONNX; the **URL maliciousness classifier (sklearn/XGBoost on lexical URL features), trained by the asset script; clean metrics recorded in the asset manifest** (dataset 11.3.3; features and perturbability in 12.9); and the CIFAR-10 small CNN, which is the CI fixture and is never registered in the demo catalog.
...
| `limitations` | JSONB NOT NULL default `[]` | `standing_limitations(dataset_name, eps_grid)` (section 14.5) plus run-specific entries |
...
- `score.weights` and `scoring_version` are frozen into the record at scoring time. S1 §8.3's "overridable per project" is implemented as the deployment default in `RedsimConfig` (`ml.scoring`) copied into `config.scoring` at admission; a per-project override UI is Phase B.
...
Field use inside the existing `RedsimFinding`:

| `RedsimFinding` field | ML value |
|---|---|
...
| `source_tool` | `redsim.ml/<attack_id>` (e.g. `redsim.ml/fgsm`, `redsim.ml/pgd`, `redsim.ml/hopskipjump`) so the findings list filter works per attack and the worker's `redsim.ml/` prefix check (section 9.4) holds |
...
| `ml.adv_slice` | full `x_adv` for one (attack, eps), float32 `.npz`, when under `REDSIM_ML_MAX_ADV_ARTIFACT_MB` (section 12.8) | `application/octet-stream` | `attack.run` |
...
| `harden.recommend` | `HardenJobDetail` | `finding_id` (or `null` for the campaign-wide pass), `llm_narrative` | `model` (the canonical Pythia model id from `REDSIM_ML_LLM_MODEL`, recorded for provenance; never a key) |
...
The chain shape is unchanged (`chain_id`, `seq`, `actor`, `action`, `target`, `allowlist_check`, `override`, `success`, `detail`, `prev_hash`, `this_hash`). Chain ids follow `redsim/audit/chain.py::_chain_id`: `run:<run_id>` when a run exists, else `project:<project_id>`, else `system`. Every ML event goes through `redsim.safety.authorize`, which is the only emitter. For `ml_model_artifact` targets the `target` argument is `None`, so `allowlist_check = "n/a"`; the target id goes in `detail`. For `ml_model_endpoint` targets (Phase B) the inference URL is the `target`, so the host allowlist applies exactly as it does for scans. `detail` is passed through `redsim.audit.redact.redact_audit_detail`. Action names are ≤ 64 characters (`AuditEvent.action` is `String(64)`). This table is the canonical list; section 10.5 states the order in which the tasks emit them.
...
`redsim audit verify` and `GET /v1/audit/verify` prove the trail of a whole campaign because every event above shares the campaign's `run:<run_id>` chain (the admission events sit on the project chain and carry the resulting `run_id` in `detail`).
...
### 6.1 Job lifecycle (canonical, from `redsim/workers/job_state.py`)
...
- The beat reaper (`redsim.reap_stale_jobs`) flips `running` jobs older than `RedsimConfig.job_max_runtime_seconds` (default 3600) to `failed` with `error = "reaped: exceeded max runtime TTL"`.
- Lifecycle events are published to the run's WebSocket channel on `running`, `failed`, `succeeded` (`redsim/workers/events.py`); the UI uses them for progress, never as the source of truth.
...
| William state | redsim canonical | How it is realised |
|---|---|---|
...
`Finding.status` uses the `Status` Literal in `redsim/schema.py`: `open / fixing / fixed / failed / false_positive`. `Finding.validation_state` uses the values `redsim/workers/tasks/verify.py` writes: `unvalidated` (default) / `poc_passed` / `poc_failed` / `inconclusive`. Both vocabularies are already understood by the findings API, the run page chips (`web/src/app/runs/[id]/page.tsx`), and the report renderer.
...
### 7.2 William's roles mapped onto redsim roles (D10, F001)

| William role | redsim role | Divergence to record |
|---|---|---|
...
| (none) | `scanner` (rank 1) | redsim-only tier between Viewer and Analyst; may launch attacks and explanations but not harden, verify, cancel, or review. Harmless; documented. |

### 7.3 Access matrix (William's rows) realised as redsim gates

Columns show the William role, its redsim role in parentheses, and the concrete gate. "Yes*" marks the independence rule (7.7). Section 17 uses these `Action` names and minimum roles verbatim.

| Action (William) | Owner (admin) | Analyst (remediator) | Reviewer (approver) | Viewer (viewer) | redsim gate |
|---|---|---|---|---|---|
...
| Manage members | Yes (in Keycloak) | No | No | No | not an redsim API; see 7.8 |
...
| `DATASET_EXPORT` | `dataset.export` | `remediator` | `POST /v1/runs/{id}/dataset`: enqueues a worker job that writes the run's adversarial slice into a bucket other teams read, a mutation with an egress footprint rather than a read projection; remediator per the 17.4 table and 27.4 (INTEROP-02, -10). Wave B0 set scanner from plan 12's brief wording; wave B2 (`codes-b2`, 2026-09-09) aligned `redsim/api/policy.py`, the Rego bundle and the Cedar bundle to this table |
...
Migration `0006_tenant_rls.py` enables and forces RLS with the `redsim_tenant_isolation` policy on `projects` and the eight scoped tables `targets, runs, jobs, findings, llm_usage, artifacts, remediation_attempts, application_logs`; `0010_ml_vertical` adds `ml_campaigns` with the identical trigger pair and policy. The predicate reads the transaction-local GUC `app.current_tenants`, set by `redsim/api/middleware/tenant.py` from the distinct `org_id`s of the caller's member projects; an empty GUC is the system path (worker, migrations) and bypasses the filter. `org_id` is trigger-backfilled on insert and guarded against drift on update (`0009`), so ML code never sets it. Not RLS-scoped: `organizations`, `users`, `project_memberships`, `auth_profiles` (app-scoped by `project_id` checks), `audit_events` / `audit_chain_heads` (append-only, chain-scoped, `project_id` nullable), `finding_tickets`. Production must connect as the non-superuser `redsim_app` role; `redsim/db/session.py` logs a warning once if a tenant-scoped session runs as a superuser, because a superuser bypasses RLS. RLS is defence in depth for the API read path; the application-layer gates in 7.3 and 7.5 remain the primary control, exactly as documented in `redsim/db/session.py`.
...
- Roles are claims. Adding, suspending, or removing a member and enforcing "a project keeps at least one Owner" are done in Keycloak (realm roles and the `redsim_project_roles` mapper), not in redsim. The app enforces the result on every request and lists membership read-only. William's invitation flow (`pending / accepted / expired / revoked / delivery_failed`) is the identity provider's invitation, outside redsim; redsim never mints invitation or auth tokens (F001 FR-001).
...
`RoleGated minRole=...` in the web app is cosmetic (`redsim/api/v1/projects.py` docstring: "Mutation routes still enforce RBAC server side; the UI gating is cosmetic"). ML pages use: `scanner` for "Run attack" and "Explain", `remediator` for "Add model" (bundled or upload), "Recommend hardening", "Verify fix", "Cancel", and notes, `approver` for "Dismiss", `admin` for delete and endpoint registration. Controls hidden by role are also absent from the DOM, and the server still returns `403` if called directly (F001 FR-011).

### 7.10 What no role can reach

- Provider API keys: none exist in redsim; Pythia holds them (D5). The only secret is `PYTHIA_API_KEY`, read on the worker from the environment.
...
The lean single-process design in S2 §2 (FastAPI + `ThreadPoolExecutor`, `run.json` on disk, no database, no queue, no auth) is **retired by D1**. The product runs on the full redsim platform, restored from aegis into this repository and renamed (D7 as amended): FastAPI `/v1` with RBAC and Postgres row-level security, Celery workers on Redis, Postgres for runs/jobs/findings/artifacts/audit, S3/MinIO for bytes, Keycloak/NextAuth for identity, the hash-chained audit log, per-task LLM routing with budget caps, and the OTel/structlog observability path. The adversarial-ML work is one new vertical, `redsim/ml/`, added to that platform (D7). Two things that S2 got right survive as constraints rather than as architecture: the evidence model (measurements, observations, interpretation, candidate recommendations kept apart; section 14) and the Pythia-only LLM path (D5).

S2 §2.1's "modules copied from aegis nearly verbatim" table (registry, redact, guardrails, state facade/filesystem, `report.html_escape`) is void: those modules are used in place from the `redsim` package (the namespace rename of D7 as amended is the only change). S2's `redsim/jobs.py` thread pool is replaced by Celery `Job` rows and `task_context`; S2's `run.json` stage persistence is replaced by `Run.stage_table` plus the Redis run-event channel; S2's `setup_assets.py` becomes the `redsim ml build-assets` CLI command (section 11, section 20); S2's `recommend/pythia_client.py` is `redsim/llm/pythia.py`.

### 8.2 System diagram

```
                ┌───────────────────────────────┐          ┌─────────────────────────────────┐
  Browser ─────►│ @redsim/web  (Next.js 14)      │◄────────►│ Keycloak (OIDC)  ↔  NextAuth     │
                │ /models /models/[id] /runs/[id]│          │ dev-token mode allowed for demo  │
                │ /findings/[id] /audit /cost    │          └─────────────────────────────────┘
                └───────────────┬───────────────┘
                                │ redsim_api_session cookie + CSRF   (CLI/CI: Authorization: Bearer)
                                ▼
                ┌──────────────────────────────────────────────────────────────────────────────┐
                │ redsim-api   FastAPI /v1  — RBAC (policy.check), tenant GUC → Postgres RLS      │
   ...
   admission order:    │ 1. authorize() → audit row │ 2. Run + Job rows           │ 3. task.delay()
            ...
            └─────────▲──────────┘        │ WORM bucket (audit)  │   │ redsim-worker  (Celery; `ml` extra) │
                      ...
                      └──────────────────────────────┼──────────────►│  -Q default harden.recommend       │
                              ...
                              │ python -m redsim.ml.sandbox_worker                │   │ Pythia gateway            │
                              │  own session/process group, POSIX rlimits,       │   │ POST {PYTHIA_BASE_URL}    │
                              │  wall-clock kill, allowlisted env (no REDSIM_*    │   │  /v1/chat/completions     │
...
- **Pythia egress (D5).** The only outbound network call the platform makes on behalf of a campaign is from the `harden.recommend` task to the Pythia gateway. It carries metrics and a SHAP text summary, never images, model bytes, or dataset rows (section 16, section 21). Provider credentials live in Pythia; redsim holds a `pk_…` key only. The sandbox child has no network configuration at all.
...
- **Tenancy.** The API sets the `app.current_tenants` GUC per request (`redsim/api/middleware/tenant.py`, `redsim/db/session.py`) so RLS filters `targets`, `runs`, `jobs`, `findings`, `artifacts`, `llm_usage`, `application_logs` and `ml_campaigns` by `org_id`. The worker runs in system scope (GUC empty) and is trusted to act only on the `Job` it was handed (section 10.4).
...
- **Observability.** `configure_otel` / `configure_structlog` in both API and worker propagate `request_id`, `run_id`, `job_id`, `project_id`; the `redsim-log-ingest` sidecar mirrors logs into `application_logs`; `/metrics` serves Prometheus.

### 8.3 Package layout (`redsim/ml/` and neighbours)
...
|---|---|---|---|
| `redsim/ml/loaders.py` — load artifact/endpoint into an ART estimator, detect framework by file signature | `redsim/ml/targets/base.py` (`Target` protocol, `Sample`), `redsim/ml/targets/bundled.py` (bundled model catalog + asset manifest reader), `redsim/ml/targets/artifact.py` (ONNX / state_dict / safetensors / XGBoost-JSON loaders), `redsim/ml/targets/architectures.py` (in-tree architecture catalog for state_dict uploads), `redsim/ml/targets/tabular.py`, `redsim/ml/targets/endpoint.py` (Phase B stub, `status=not_implemented`) | `base.py` exists; rest new | Loaders are imported **only inside the sandbox child** (section 9). File-signature detection happens at admission (cheap sniff) and again in the child (full parse). |
| `redsim/ml/attacks/` — one module per ART attack, registered through the existing registry | `redsim/ml/attacks/base.py` (`AttackAdapter`, `AttackOutput`), `fgsm.py`, `pgd.py`, `noise_control.py`, `hopskipjump.py`, `registry.py` | `base.py` exists; rest new | `registry.py` is an instance of the generic `redsim.registry.Registry` (duplicate detection, protocol check, opt-in entry-point discovery under group `redsim.ml.attacks`, gated by `REDSIM_PLUGINS=1` and the Ed25519 signature gate like scanner plugins). S1's "list in `/tools`" becomes `GET /v1/attacks` (section 17) and `redsim plugins list`. |
| — | `redsim/ml/campaign.py` — `CampaignScannerAdapter` (`name="ml-campaign"`, `capabilities={"adversarial_ml","explainability"}`) plus the stage pipeline (`load_target → sample → clean_eval → attack×eps → control → explain → score → interpret → recommend → report`) | new | Satisfies the S1 §2 row "ART attacks register the same way scanner adapters do": the façade is a built-in `ScannerAdapter` registered in `redsim.scanners.registry`, so `list_scanners()`, `dispatch()`, and the offline CLI see the vertical. `KNOWN_CAPABILITIES` gains `adversarial_ml` and `explainability` (S1 §4). `health_check()` reports whether the `ml` extra imports and the sandbox worker launches. `POST /v1/scans` is **not** the ML entry point (section 17). |
| `redsim/ml/explain/` — SHAP per modality, PNG + JSON artifacts | `redsim/ml/explain/shap_image.py`, `shap_tabular.py`, `stability.py` (`expl_shift`, center-mass heuristic), `summary.py` (the deterministic SHAP text summary, section 13.7) | package exists, empty | Section 13. |
| `redsim/ml/harden.py` — rules → ranked defenses → LLM prose | `redsim/ml/recommend/interpret.py` (interpretation rules, section 14.6), `redsim/ml/recommend/rules.py` (deterministic recommendation rules, cite measurement ids), `redsim/ml/recommend/narrative.py` (Pythia writer, text only), `redsim/ml/defenses.py` (ART preprocessors: feature squeezing, spatial smoothing, JPEG compression — applied only inside the sandbox for the verify loop) | package exists, empty | S2 naming retained. Section 16. |
| `redsim/ml/eval.py` — clean/robust accuracy, ASR, mean perturbation, confidence drop | `redsim/ml/eval.py` (metrics → `Measurement` rows), `redsim/ml/scoring.py` (MRI, subscores, ΔMRI) | new | Sections 12 and 15. |
| `redsim/workers/tasks/{attack,explain,harden}.py` mirroring scan/fix/verify | `redsim/workers/tasks/model_validate.py` (`redsim.model_validate`), `attack.py` (`redsim.attack_run`), `explain.py` (`redsim.explain_run`), `harden.py` (`redsim.harden_recommend`); `verify.py` gains an ML branch for `redsim.verify_replay` | new / modified | Section 10. |
| — | `redsim/ml/sandbox.py`, `redsim/ml/sandbox_worker.py` | new | The model-loading boundary (section 9.4). Reuses `SandboxConfig`, `_rlimit_preexec`, `_child_env`, `_kill_process_group` and the envelope conventions from `redsim/scanners/sandbox.py`; those helpers are promoted to public names in a shared module as part of the code fix-up. |
| — | `redsim/ml/datasets/` | new | Dataset catalog, licences, revision hashes, stratified sampling (section 11). |
| — | `redsim/ml/errors.py` | new | Typed failure classes whose names are the first token of `Job.error` (section 10.6). |
| — | `redsim/ml/schema.py` | exists | `TargetInfo`, `AttackInfo`, `RunConfig`, `Provenance`, `Measurement`, `Observation`, `Interpretation`, `CandidateRecommendation`, `RunRecord`, `STANDING_LIMITATIONS`. The scaffold's `RunConfig` names a single `attack_id`; the campaign configuration in section 5 (`CampaignConfig`) carries `attack_ids`, `eps_grid`, `reference_eps`, scoring weights and the finding threshold. The tasks in section 10 read the section 5 shape. |
| — | `redsim/services/ml_models.py` (`register_bundled_model`, `upload_model_artifact`), `redsim/services/ml_campaigns.py` (`create_attack_campaign`, `create_explain_job`, `create_harden_job`), `redsim/services/ml_findings.py` (`review_finding`, `annotate`) | new | Admission halves, same shape as `services/scans.py::create_scan_job`. |
| — | `redsim/api/v1/{models,attacks,datasets,defenses,ml_capabilities,artifacts,compare,ml_findings}.py` | new | Section 17. `/v1/artifacts/{id}` is new: no artifact-streaming route exists today; reports stream from `/v1/runs/{id}/report.{ext}` with the strict CSP in `redsim/api/security_headers.py`. |
| — | `redsim/llm/pythia.py` | exists | Transport (D5). `PythiaSettings.from_env` reads `REDSIM_LLM_MODEL` today; it changes to `REDSIM_ML_LLM_MODEL` at M0 (D5), with `tests/test_llm_pythia.py` and the comment in `redsim/ml/schema.py` updated in the same change. |
| — | `redsim/cli/ml.py` — `redsim ml build-assets`, `redsim ml attack` (offline) | new | Offline path mirrors `services.scans.start_scan`: filesystem `RunState`, `JsonlAuditWriter` at `<run_path>/audit.jsonl`, same sandbox child. |
| — | `tests/ml/fakes.py` (`TinyTarget`) | exists | Section 22. |

Router registration and queue routing changes: `redsim/workers/celery_app.py` `include` gains the four new task modules; `task_routes` gains `redsim.model_validate` → `scans`, `redsim.attack_run` → `scans`, `redsim.explain_run` → `scans`, `redsim.harden_recommend` → `default` (`redsim.verify_replay` already routes to `scans`). `redsim/api/app.py` mounts the new routers under `/v1`. `Job.type` (`String(64)`) and `Target.kind` (`String(32)`) are unconstrained strings in the ORM, so the new values need no enum migration; section 5 states the migrations for the new column and the campaign/score record.
...
1. **Preferred format is ONNX.** PyTorch `state_dict` is accepted only with an explicit architecture id from the in-tree catalog. Full pickles are refused; there is no override path in Phase A (S1 §13's "I trust this file" checkbox is a Phase B item and, if built, requires the `admin` role, an operator-set worker flag `REDSIM_ML_ALLOW_PICKLE=1`, an audit row with `override=true`, and still runs in the sandbox). TensorFlow SavedModel, listed as preferred in S1 §5, is deferred to Phase B: D2 names ONNX and state_dict only, and the `ml` extra carries no TensorFlow.
...
Bundled models are registered through `register_bundled_model` (same audit action with `source=bundled`, digest from the asset manifest) and seeded directly as `available`; their blobs are seeded by `redsim ml build-assets` at deploy (section 20).

### 9.4 The sandbox mechanism as it exists, and how the ML vertical uses it

`redsim/scanners/sandbox.py` and `redsim/scanners/sandbox_worker.py` implement the out-of-process boundary for third-party scanner plugins discovered from the `redsim.scanners` entry-point group. What it actually does:

- **Wrap point.** During eager discovery `redsim.scanners.registry._sandbox_wrap` wraps each conformant, signature-approved plugin in `SandboxedScanner`, a structurally conformant `ScannerAdapter` whose `scan()` calls `run_scanner_sandboxed(entry_point, run_state, options, adapter_name, config)`. Built-in adapters registered in code never pass through the wrapper; the sandbox is a plugin-only control today.
- **Launch.** `subprocess.Popen([sys.executable, "-m", "redsim.scanners.sandbox_worker", "--entry-point", ep], stdin/stdout/stderr=PIPE, text=True, env=_child_env(cfg), preexec_fn=_rlimit_preexec(cfg), start_new_session=True)` — list argv, never `shell=True`.
- **Limits** (`SandboxConfig`, from `REDSIM_PLUGIN_SANDBOX_*` env or defaults): `timeout_s=600` (wall clock), `cpu_seconds=300` (`RLIMIT_CPU`), `memory_mb=1024` (`RLIMIT_AS`), `file_size_mb=256` (`RLIMIT_FSIZE`), `open_files=256` (`RLIMIT_NOFILE`), `max_processes=256` (`RLIMIT_NPROC`, best-effort), `RLIMIT_CORE=0`, `allow_network=False`. Soft and hard limits are set equal; individual `setrlimit` failures are swallowed so the child still launches (best-effort hardening).
- **Environment.** Built from an empty dict, copying only `PATH, PYTHONPATH, PYTHONHOME, PYTHONHASHSEED, VIRTUAL_ENV, HOME, TMPDIR, TEMP, TMP, LANG, LC_ALL, LC_CTYPE, TERM, TZ, SYSTEMROOT` when present, pinning `REDSIM_PLUGINS=0`. Proxy variables (`HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY`, `NO_PROXY` and lower-case forms) are restored only when `REDSIM_PLUGIN_SANDBOX_NETWORK` opts in. The parent's `REDSIM_DB_URL`, `REDSIM_S3_*`, `REDSIM_WORKER_SIGNING_KEY`, `REDSIM_AUTH_PROFILES_KEY`, `PYTHIA_API_KEY`, and any API tokens are never in the child.
...
The ML vertical builds `redsim/ml/sandbox.py` on the same primitives and conventions, with these deliberate differences:

| Aspect | Plugin sandbox (exists) | ML model sandbox (this spec) |
|---|---|---|
| Child entry | `redsim.scanners.sandbox_worker --entry-point module:factory`; imports arbitrary plugin code | `redsim.ml.sandbox_worker --stage validate|attack|explain|verify`; imports only in-tree `redsim.ml` code; the untrusted input is the model file, not code |
| Enable switch | `REDSIM_PLUGINS_SANDBOX` (default on, can be turned off for trusted plugins) | none — there is no in-process path for model bytes; `REDSIM_PLUGINS_SANDBOX=0` has no effect on ML |
| Config env | `REDSIM_PLUGIN_SANDBOX_{NETWORK,CPU_SECONDS,MEMORY_MB,FILESIZE_MB}` | `REDSIM_ML_SANDBOX_TIMEOUT_S` (default 1200), `REDSIM_ML_SANDBOX_CPU_SECONDS` (900), `REDSIM_ML_SANDBOX_MEMORY_MB` (4096), `REDSIM_ML_SANDBOX_FILESIZE_MB` (1024), `REDSIM_ML_SANDBOX_THREADS` (2); network is never enabled (section 20.3). `RLIMIT_AS` caps virtual address space, and CPU torch maps large virtual regions at import, so the default sits well above resident need; the wall-clock and CPU limits are the operative caps. The wall clock must stay below the Celery soft limit (1800 s) and the reaper TTL (`job_max_runtime_seconds`, 3600 s). |
| Child env additions | — | `OMP_NUM_THREADS`/`MKL_NUM_THREADS` = `REDSIM_ML_SANDBOX_THREADS`, `MPLBACKEND=Agg`, `HF_HUB_OFFLINE=1`, `HF_DATASETS_OFFLINE=1`, `PYTHONHASHSEED` fixed for the run |
| Inputs | run dir path + `ScanOptions` JSON | a per-job work directory (`REDSIM_ML_WORK_DIR`, default `$TMPDIR/redsim-ml/<job_id>`, mode 0700) that the **parent** populates: the model file fetched from S3 and digest-checked against the manifest, `slice.npz` (the seeded, stratified evaluation slice sampled by the parent — section 11), and for explain/verify stages the attack-stage `x_adv` artifacts fetched and digest-checked against their `Artifact` rows. The child never reaches S3, Postgres, Redis, or the dataset source. |
| Output | `{"ok": true, "result": <ScanResult dict>}` | `{"ok": true, "result": {measurements[], observations[], findings[] (RedsimFinding dicts), artifacts[] ({name, kind, path, sha256, content_type}), provenance{}, not_run[] ({attack, reason}), stage_timings{}}}` or `{"ok": false, "error_class": "<redsim.ml.errors name>", "error": "<operator-safe message>"}`. Large arrays go to files in the work dir; the envelope is capped at 16 MiB. |
| Parent post-processing | rebuild `ScanResult` | validate every list element with its Pydantic model (a malformed envelope fails the job, nothing is trimmed silently); recompute sha256 of each listed file and compare; require `finding.source_run_id == run_id` and `source_tool` prefix `redsim.ml/`; then `run_state.record_artifact(name, bytes, content_type)` (→ blob key `{project_id}/{run_id}/{kind}/{sha256}` and an `Artifact` row unique on `(run_id, kind, sha256)`), `run_state.save_findings(...)`, the campaign/score record write (section 5), and the audit rows (section 10.5); finally the work dir is removed unless `REDSIM_ML_KEEP_WORK_DIR=1` |

The stage internals (`Target` implementations, attack adapters, explainers, defenses, scoring) are plain Python called by `redsim/ml/campaign.py` inside the child. They are unit-tested in-process against `tests/ml/fakes.py::TinyTarget` (section 22); the subprocess boundary is tested separately by launching the real child on `TinyTarget` and on a deliberately malformed file. Bundled models take the identical path on every run, so the demo exercises the boundary rather than bypassing it.
...
|---|---|---|---|
| Upload exceeds `REDSIM_ML_UPLOAD_MAX_MB` | API, while streaming | HTTP 413 `model_too_large`; bytes discarded | `model.register` `success=false`, `detail.reason=size_cap` |
...
redsim splits every operation into an **admission** half (API process, request-scoped, cheap) and an **execution** half (worker, long-running). `redsim/services/scans.py::create_scan_job`, `services/verify.py::create_verify_job`, and `services/runs.py::cancel_run` are the templates; `redsim/workers/bootstrap.py::task_context` is the execution wrapper; `redsim/workers/job_state.py::set_job_status` is the only writer of `Job.status`. The ML vertical adds admission functions and tasks of the same shape and changes nothing about the contract.
...
Offline (`redsim ml attack`, CI): the same `authorize()` calls resolve to `JsonlAuditWriter` at `<run_path>/audit.jsonl` (D4(c): "Postgres-backed, JSONL offline"); `verify_chain` walks either.
...
1. `redsim.llm.router.route("ml.harden_narrative", config, project_id=..., org_id=..., budget_checker=DbBudgetChecker())` is the policy layer: `Organization.llm_model_overrides["ml.harden_narrative"]` wins, else `config.task_models["ml.harden_narrative"]` seeded from `REDSIM_ML_LLM_MODEL`; the project daily cap and org monthly cap are enforced (`BudgetExceeded`; with `llm_budget_strict`, an unverifiable budget denies). The resolved id must be a Pythia canonical `<vendor>/<model>` or `pythia/auto`; `RedsimConfig.model`'s provider-style default is not used for this task.
2. `redsim.llm.pythia.chat_text(settings, system, user)` is the transport: one non-streaming `POST {PYTHIA_BASE_URL}/v1/chat/completions` with `Authorization: Bearer pk_…`, optional `X-Pythia-Persona`, `temperature=0.2`, `max_tokens=800`, timeout `PYTHIA_TIMEOUT_S` (60 s); `pythia_sdk.PythiaClient` when importable, otherwise the in-repo `httpx` client. No tools, vision, or structured output; no litellm; no provider keys.
3. Input is the rule-layer output, the measurement table with denominators, the scorecard numbers, the limitations and the SHAP text summary (`ml.shap.summary_text`) — text only (section 16.3). `guard_input` (injection detection) and `guard_output`/`filter_output` (secret scrub) from `redsim.llm.guardrails` wrap the call; the prompt and completion are stored as `Artifact` rows (`ml.harden.prompt`, `ml.harden.completion`) and their digests go on the `harden.execute` audit row; a `LLMUsage(task="ml.harden_narrative")` row records tokens and cost (zero with `unpriced_model` noted when the canonical id is not in `redsim/llm/pricing.py`).
4. Any failure — `PythiaUnavailable` (env unset), `REDSIM_DISABLE_LLM=1` (already set on the compose worker pools; section 20 adds `PYTHIA_*` where the writer should run), `BudgetExceeded`, HTTP error, guardrail block, post-check rejection, malformed response — leaves the deterministic rule output in place with `narrative_source="rules"` and the skip reason recorded; it never fails the job and never invents prose. When it succeeds, `narrative_source="llm"` and the UI labels the text "LLM-generated narrative of rule outputs" (section 16, section 18). The writer may not introduce claims absent from the rule outputs, and it never receives images, model data, or dataset rows.
...
| Gated | Metadata: no. **Download: yes** — the Kaggle API requires an account token (`KAGGLE_USERNAME` / `KAGGLE_KEY`). No token exists on the build machine as of 2026-09-08; the operator who runs `redsim ml build-assets` supplies one for that run only (section 20.3). |
...
- **Where data lives.** Dataset bytes are fetched once at build time by `redsim ml build-assets` (`redsim/cli/ml.py`, the successor of the lean design's `redsim/setup_assets.py`, D7) — hub datasets with `revision=` pinned, the Kaggle tabular dataset through the authenticated download of 11.3.3 with `KAGGLE_USERNAME` / `KAGGLE_KEY` read for that run only and never written anywhere — written to the blob store (`redsim/storage/blobs.py`; `REDSIM_BLOB_BACKEND=s3` for MinIO/S3) under `ml/assets/<dataset_id>/<revision>/`, and read only by the Celery worker (section 10). The API and web containers never hold dataset bytes (section 8, section 20). The asset seeding step of S1 §12 ("seed sample models and datasets into S3 on first deploy") is this CLI.
...
### 11.6 Phase B LLM-track corpora: garak bundled probe data (deferred)

Recorded 2026-09-08 (later, owner decision). The Phase B LLM-assistant track (D6: garak probes through Pythia, sections 3.2, 12.2, 17.4 and milestone B1 in section 23) uses the probe corpora that garak ships under `garak/data` (github.com/NVIDIA/garak, `main`, package licence Apache-2.0). Nothing is extracted or re-packaged for this tool: garak's probe classes and detectors load these files directly. None of it is a Phase A dataset, a classifier dataset or an input to an MRI (D9, section 15.8: one MRI per campaign and per modality). Nothing in Phase A reads it.

Note: the `idllresearch/malicious-gpt` corpus was considered for this track and withdrawn on 2026-09-08 (no licence). The local download and the combined JSONL were deleted from the asset cache.

- **Static corpora under `garak/data`** (verified from the upstream repository):
    - `inthewild_jailbreak_llms.json` (1,339,768 bytes): in-the-wild jailbreak prompts used by `garak.probes.dan`. Upstream is the CCS '24 "Do Anything Now" study. Its upstream licence is to be confirmed before any redistribution outside garak.
    - `dan/`: 14 JSON DAN-family templates (AntiDAN, ChatGPT_Developer_Mode_RANTI, ChatGPT_Developer_Mode_v2, ChatGPT_Image_Markdown, DAN_Jailbreak, DUDE, Dan_6_0, Dan_6_2, Dan_7_0, Dan_8_0, Dan_9_0, Dan_10_0, Dan_11_0, STAN).
    - `harmbench/`: `harmbench_prompts.txt` (17,920 bytes) with its own LICENSE and README.
    - `donotanswer/`: 5 JSONL files by harm category (discrimination_exclusion_toxicity_hateful_offensive, human_chatbox, information_hazard, malicious_uses, misinformation_harms).
    - `realtoxicityprompts/`: 7 category `.txt` subsets.
    - `payloads/`: 17 small JSON payload sets, including harmful_behaviors, sql_injection, xss, python_code_execution and slur_terms_en.
- **Generated at runtime, no static dataset.** The encoding-smuggling, GCG-suffix, TAP, AutoDAN, BEAST, GOAT, agent_breaker and sysprompt_extraction probe families build their prompts when they run.
...
**11.6 addendum (2026-09-09, owner decision TESTS_DOCS-33).** The sentence above that "this tool redistributes none of them" describes the tool and stays true of the code: garak loads its own files from the installed package, redsim re-packages nothing and no probe text is committed to this repository. It is superseded for the public data repository: on 2026-09-08 the owner decided to publish a copy of garak 0.16.0's `garak/data` directory to [IntelliBridge/ai-red-teaming-data](https://github.com/IntelliBridge/ai-red-teaming-data) (commit `754fbca`: `data/garak/`, 132 files, 3.76 MB, with `LICENSE-garak-Apache-2.0.txt`, `PROVENANCE.csv` recording the upstream terms per subset and `SHA256SUMS.txt`), and the Phase B wave B0 datasets commit (`4048a209`) added the reference entry to `INDEX.csv` and `MANIFEST.json` with the licence note per subset. `PROVENANCE.csv` carries the caveat this section already stated for `inthewild_jailbreak_llms.json` ("confirm upstream terms before further redistribution"); that subset is published as part of the garak copy under garak's Apache-2.0 packaging and its upstream terms are recorded as unconfirmed, not asserted. Nothing else in this section changes: the corpora stay probe material only, never a classifier dataset and never an MRI input, and the handling and evidence rules hold. `docs/project-brief.md` item 14 carries the same dated addendum.
...
**11.7 addendum (2026-09-09, Phase B publications).** The repository head at the wave B4 push is `0dababc`. After `ff6a36b` it gained, in order: `754fbca`, the `data/garak/` copy of garak 0.16.0's probe corpora with `LICENSE-garak-Apache-2.0.txt`, `PROVENANCE.csv` and `SHA256SUMS.txt` (owner decision TESTS_DOCS-33, the 11.6 addendum); `a9ba6ba3` and `4048a209` (Phase B wave B0), the UCI SMS Spam Collection published verbatim with its seeded 20 percent eval split (`data/sms_spam_collection.tsv`, `data/sms_spam_eval_split.tsv`, CC BY 4.0, owner default MODALITIES-12), the 300-image military-assets subset `data/military_assets_subset/` (CC BY 4.0, published for owner review under MODALITIES-27 and removable in one commit), a WordNet 3.0 reference entry (not republished) and a MITRE ATLAS `v2026.08` release record, every file with an `INDEX.csv` row and a `MANIFEST.json` entry; and `0dababc` (Phase B wave B3), the first Croissant export the platform built with its own modules, `data/exports/url_trees_sample/` (`croissant.json`, `README.md` and three Parquet shards from an offline `url_trees` PGD campaign, 600 rows of 16 lexical features, no URL strings, the adversarial family only because the source run retained no clean or control slices). The platform never pushes to this repository; every publication is by hand. The redaction and re-derivation statements above are unchanged. `tests/ml/fixtures/public_index.csv` snapshots the repository's `INDEX.csv` at `4048a209`, before the export rows, and `tests/ml/test_datasets.py` checks every code-named dataset has its rows.

## 12. Attack catalog

### 12.1 Adapter contract and registration

Every attack is an adapter over the Adversarial Robustness Toolbox (ART). S1's interface `run(estimator, data, budget) -> AttackResult` is realised by the protocol already in `redsim/ml/attacks/base.py`:
...
- `AttackInfo.family` is the Literal `evasion | control`; `AttackInfo.domain` is `image | tabular | llm`. Since P0 the model also carries `phase`, `access`, `requires_gradients`, `status` and `reason` with defaults (4.5 row 3), so `GET /v1/attacks` (17.2) returns `AttackInfo` as is. Phase B rows for text and detection require adding `text` and `detection` to the `Domain` literal in `redsim/ml/schema.py`; that change is not made in Phase A.
...
- Adapters register in the redsim `Registry[T]` (`redsim/registry.py`, duplicate-id detection) under the new capability tags `adversarial_ml` and `explainability` (S1 §4). They are listed by `GET /v1/attacks` (section 17). S1's remark that attacks "list in `/tools`" is superseded: the `/tools` pages and the agent/tool registry were deleted with the pentest vertical (section 2).
...
Phase B rows are registered only when their adapter, estimator support, and tests exist; until then `GET /v1/attacks` does not list them and the UI shows the modality as unavailable with the reason (S2 honesty rule; section 18). LLM-domain red-teaming (garak) is Phase B and, if added, points garak's OpenAI-compatible generator at Pythia (D6); it is not an entry in this catalog. Its probe material is the set of corpora garak ships and loads itself (11.6, Apache-2.0 packaging, upstream licences per subset), treated as untrusted data and sent only to the permission-gate-only Pythia persona.
...
S1 §8.1's inputs are recorded as follows. Existing `Measurement` fields are reused; the additions marked **new** are stated explicitly, landed with P0 (4.5 row 5) and live in `redsim/ml/schema.py` (storage placement is fixed in section 5.3; the scoring definitions in section 15.1 govern).
...
Also recorded on every evasion and control row: `wall_time_s`, `params` (resolved, with values typed `float | int | bool | str` because `norm` is a string, 4.5 row 4), `notes` (surrogate transfer, frozen features, "not run" reasons, artifact-retention notes), and — through `AttackOutput.library_versions` — the `art`, `torch`, `numpy`, and `scikit-learn`/`xgboost` versions, which are merged into `Provenance` (section 14).
...
- **Identity:** `Finding.scanner_finding_id = "ml.<attack_id>"`, `source_tool = "redsim.ml/<attack_id>"`, `dedup_key = "ml:<model_sha256[:16]>:<attack_id>:<settings_hash[:16]>"` (section 5.2), so a rerun at identical settings dedups against the earlier Finding and a different grid or dataset does not.
...
- The worker enforces a per-Job wall-clock budget (`REDSIM_ML_SANDBOX_TIMEOUT_S`, section 9.4); exceeding it fails the Job (`failed` with `SandboxTimeout`; William's `timed_out` maps onto it, section 6.3). Partial Measurements written before the timeout are kept and labelled partial; no curve point is interpolated.
- Adversarial arrays: the full `x_adv` slice per (attack, ε) is stored as an `Artifact` (`adv_slice.npz`, float32, kind `ml.adv_slice`) when it is at most `REDSIM_ML_MAX_ADV_ARTIFACT_MB` (default 64 MB); otherwise only the explained samples' arrays are stored (section 13.6) and `notes` records that the full slice was not retained. Retention is never load-bearing: `verify.replay` regenerates the attack from configuration and seed rather than replaying stored arrays.
...
5. **Per-sample record.** One `Observation` per explained sample: `sample_index`, `true_label`, `pred_clean`, `pred_adv`, `flipped`, `confidence_clean`, `confidence_adv`, `artifacts` (name → `Artifact.id`), `artifact_sha256`, `center_mass_ratio_clean` / `center_mass_ratio_adv` (image only; 13.4), `metric_kind = "heuristic"`, `metric_note`, plus the **new** field `expl_shift: float | None` (13.5) and, for tabular, **new** `top_features_clean` / `top_features_adv: list[str]` (feature names ranked by \|SHAP\|). These additions to `Observation` in `redsim/ml/schema.py` landed with P0 (section 5.3); existing fields are unchanged.
...
`redsim/ml/explain/` writes to the blob store and registers `Artifact` rows (`redsim/db/models.py`: `id`, `run_id`, `project_id`, `org_id`, `kind`, `sha256`, `location`, `content_type`, `size_bytes`; unique on `(run_id, kind, sha256)`) with the kinds fixed in section 5.8. `Observation.artifacts` maps a stable name to the `Artifact.id`; `Observation.artifact_sha256` mirrors `Artifact.sha256`; `GET /v1/artifacts/{id}` streams them with the strict CSP (section 17). S1's "raw SHAP values JSON" is amended: raw arrays are written as `.npz`, with a small JSON meta file beside them, because a 128×128×3 map per class is impractical as JSON.
...
The summary passes through redsim's redaction and guardrails before the Pythia call (section 16, section 21).
...
This section fixes what a run is allowed to say and how it says it. It carries the brief's reporting principles, constitution Principles III (evidence before claims) and V (reproducibility with honest limits), and the evidence requirements of F005 (FR-004, FR-005, FR-006, FR-011), F006 (FR-008, FR-010) and F007 (FR-008, FR-009) into the redsim platform as binding constraints. Every later section that produces a number, a picture or a sentence (12, 13, 15, 16, 17, 18, 24) is bound by it. Nothing in S1 conflicts with this section; where S1 was silent (labels, denominators, controls, limitations) S2 applies, and where S2's lean layout differs from redsim persistence, section 5 governs storage while this section governs meaning.

### 14.1 Four kinds of statement, four fields, four panels

A run produces exactly four kinds of statement. They are separate Pydantic types in `redsim/ml/schema.py`, separate sub-objects in every report, and separate panels in the UI (section 18). They are never blended in prose.

| Kind | Type in `redsim/ml/schema.py` | What it is allowed to contain | Label enforced by | UI panel (section 18) |
|---|---|---|---|---|
...
`Provenance` (already in `redsim/ml/schema.py`) records `redsim_version`, `python`, `torch`, `art`, `shap`, `numpy`, `model_sha256`, `dataset`, `dataset_split`, `model_manifest`, `started_at`, `finished_at`, `hostname`, `device`, `nondeterminism`. P0 added the fields below (section 5.3 lists the full set):

| Field | Type | Meaning |
|---|---|---|
| `dataset_revision` | `str \| None` | Content hash or hub revision of the dataset (section 11) |
| `sample_indices_sha256` | `str \| None` (`None` on the frozen model until the worker writes it) | Hash of the evaluated index list, so a rerun can prove it used the same slice |
| `settings_hash` | `str \| None` (`None` on the frozen model until the worker writes it) | The campaign settings hash defined in section 5.6; identical hashes are the precondition for any comparison |
...
`RunRecord.limitations` is non-empty on every succeeded run; the existing validator (`_limitations_required_when_done`) enforces it and `tests/ml/test_schema.py` covers it. P0 realised the two changes below as `standing_limitations(dataset_name, eps_grid)` in `redsim/ml/schema.py` (4.5 row 8): `STANDING_LIMITATIONS` itself keeps only the five dataset-independent sentences, and the function returns the dataset sentence, then the budget sentence, then those five.

1. The CIFAR-10 sentence became `DATASET_LIMITATION_TEMPLATE`, filled with the dataset name from the manifest (section 11.5): `"<dataset name> is an open, unclassified public benchmark; it is not a proxy for any operational domain, sensor, or deployment condition."` CIFAR-10 remains the CI fixture and the sentence is exercised in tests with that name.
2. The budget sentence is chosen by the grid: `SWEEP_LIMITATION_TEMPLATE` (`"Results come from a single seed; the ε grid was <grid>."`) when the grid has more than one member, otherwise `SINGLE_BUDGET_LIMITATION` (the original text).
...
Interpretations are produced by a deterministic rule set in `redsim/ml/recommend/interpret.py`, run as the `interpret` stage of the `harden.recommend` job (section 10.2), so that every inferred sentence has a reproducible basis. The initial set:
...
| Distinguish observed behaviour from inferred causes (brief; constitution III) | Four types, four panels, `Literal` labels (14.1) | `redsim/ml/schema.py`; `tests/ml/test_schema.py`; vitest label assertions (22) |
...
The Model Robustness Index (MRI) is adopted from S1 §8 (D9) as the per-campaign summary of robustness under the in-scope attacks. It is a 0–100 integer, higher is more robust. It drives the scorecard, the derived Finding severities (15.5) and the ranking of candidate recommendations (16.1). It is deterministic and recomputable from the stored measurements: `redsim.ml.scoring.compute_mri(measurements, observations, settings)` is a pure function, the stored score record carries its inputs, and `tests/ml/test_scoring.py` asserts that recomputation reproduces the stored value.
...
For each in-scope attack `a` and each ε in the declared grid the engine records the following, all derived from `Measurement` and `Observation` rows of the same run (14.2). The reference budget `reference_eps` is written ε_ref below. Every field named here and in section 12.5 is on the frozen `Measurement` since P0 (section 5.3, 4.5 row 5).
...
Weights, thresholds and defaults live in the `ml.scoring` config block on `RedsimConfig` and are copied into `CampaignConfig.scoring` at admission (section 5.6). S1's "overridable per project" is a Phase B UI; in Phase A the deployment default applies to every project. The block:

| Key | Default | Used by |
|---|---|---|
| `weights` | `{acc: 0.35, asr: 0.25, eps: 0.20, conf: 0.10, expl: 0.10}` | 15.3 |
| `finding_asr_threshold` | 0.2 (ASR at which an attack "succeeds" and creates a Finding; section 12.6). Not a `ScoringConfig` key since P0 (4.5 row 7): it is `CampaignConfig.finding_asr_threshold`, a request field of `POST /v1/models/{id}/attacks` (17.2), copied onto `MRIRecord.finding_asr_threshold` at scoring time, and it enters `settings_hash` beside the `scoring` block (5.6). | 12.6, 15.5 |
...
This writes the existing `Finding.severity` column, whose vocabulary `Severity = Literal["critical","high","medium","low"]` (`redsim/schema.py`) matches exactly, so the findings table, filters, dashboard counts and audit trail work unchanged. The other required `RedsimFinding` fields are filled deterministically (section 5.7): `title` = `"<attack> flips predictions at ε=<first-success ε> (<norm>; ASR <n_flipped>/<n_correct_clean>)"`; `finding_type` = `adversarial_ml` (the new `FindingType` literal registered in section 5, matching S1's `adversarial_ml` capability tag); `description` = measurement text only; `confidence` = `high` when `m.clean.n_correct ≥ confidence.n_high`, `medium` when `≥ confidence.n_medium`, else `low` (sample-size based, not judgement based); `source_tool` = `redsim.ml/<attack_id>`; `status` = `open`; `evidence` = the measurement ids. The `ml` sub-object of `Finding.schema_blob` (section 5.7) stores the full derivation: attack id and resolved params, grid, per-ε `asr` and `acc_adv` with denominators, first-success ε, `pert`, `conf_gap`, `expl_shift`, `queries`, SHAP artifact ids, and the thresholds used, so severity is recomputable and auditable. Scoring writes `severity` and (via verify, 15.6) `validation_state`; it never writes `Finding.status`, whose transitions are in section 6.
...
**Transport (D5).** Every call goes through Pythia, IntelliBridge's OpenAI-compatible agent gateway: `POST {PYTHIA_BASE_URL}/v1/chat/completions`, header `Authorization: Bearer pk_…`, optional `X-Pythia-Persona`, model id in canonical `<vendor>/<model>` form or `pythia/auto`. The implementation is `redsim/llm/pythia.py` (`PythiaSettings`, `chat_text`, `make_backend`, which prefers the official `pythia_sdk` and falls back to `httpx`). redsim holds only the Pythia key; provider keys live in Pythia (section 21). There is no litellm path and no direct provider client. One plain, non-streaming chat completion per narrative (`chat_text`: one system message, one user message, `temperature=0.2`, `max_tokens=800`); no tools, no vision, no structured-output mode.

**Configuration.** `PYTHIA_BASE_URL`, `PYTHIA_API_KEY`, `PYTHIA_PERSONA` (optional), `PYTHIA_TIMEOUT_S` (default 60) and `REDSIM_ML_LLM_MODEL` (section 20.3). `redsim/llm/pythia.py` currently reads `REDSIM_LLM_MODEL`; renaming it to `REDSIM_ML_LLM_MODEL` is an M0 task (D5, D7; section 23). `CampaignConfig.llm_narrative` defaults to `false`; the launcher (section 18) offers the checkbox only when `/v1/ml/capabilities` reports Pythia as configured, and the demo profile turns it on.

**Policy layer.** The model id is resolved through the existing router: `redsim.llm.router.route(task="ml.harden_narrative", …)` reads `config.task_models["ml.harden_narrative"]` (seeded from `REDSIM_ML_LLM_MODEL`), honours the per-organisation override (`Organization.llm_model_overrides`), and applies the per-project daily and per-organisation monthly budget caps through `DbBudgetChecker`; `enforce_budget_for_run` runs as a fail-closed pre-flight when `llm_budget_strict` is set. The resolved id MUST be a Pythia canonical id; the router rejects anything else for this task. Token usage from Pythia's `usage` block and cost via `redsim.llm.pricing` are written to `LLMUsage` (`task = "ml.harden_narrative"`, section 5.12) exactly as for every other routed call, so the cost page (section 18) shows narrative spend.

**Payload: text only.** The user message is assembled by `redsim/ml/recommend/narrative.py` from exactly:
...
**Guardrails.** `guard_input` runs on the assembled user message (model, dataset and class names are user-supplied strings and could carry an injection); `guard_output` scrubs the response with the same token signatures the audit chain trusts (`redsim/llm/guardrails.py`, `redsim/audit/redact.py`). Two further post-checks are specific to this writer: a **numeric-consistency check** rejects a narrative containing a number token absent from the payload, and a **banned-word check** rejects a narrative containing any word from the list above. A rejected narrative is discarded, `narrative_source` stays `"rules"`, and the UI shows "LLM narrative: rejected by post-check (<reason>)".
...
| `model_too_large` | 413 | Upload exceeds `REDSIM_ML_UPLOAD_MAX_MB`. |
...
| `endpoint_schema_mismatch` | 422 | A synchronous endpoint call made on the caller's behalf answered outside the `endpoint-v1` contract (`{contract, input_format, inputs}` in, `probabilities` or `logits` out; shape, class count, dtype); the same condition inside validate or a campaign is the target's `refusal_reason`, not an HTTP code. The register's working name `redsim-predict-proba/1` is superseded (wave B2 divergence, corrected 2026-09-09 by wave B4). |
...
**17.3 addendum, second table (Phase B wave B2, `codes-b2` track, 2026-09-09).** The codes the register of 2026-09-09 named that wave B0 left out, added before the wave B2 and B3 routes so those tracks import constants from `redsim/api/errors.py` instead of spelling strings. Same envelope, same rules as the two tables above, and `tests/ml/test_error_codes.py` parses this table too. Every row is a refusal: none is a 2xx marker and none joins the plain-string set, so `reviewer_not_independent` is the structured spelling of the independence refusal that the Phase A table folds into `forbidden`; the Phase A dismissal alias `PATCH /v1/findings/{id}/status` keeps its string `forbidden` for byte-compatibility and the Phase B decisions of `POST /v1/findings/{id}/review` emit this code. Where the register spelled a status or a name differently, this table wins: `query_budget_exceeded` is `429` (ENDPOINT-06 wrote 422; it is a budget refusal and sits with `daily_budget_exceeded`), `idempotency_key_reused` is `409` (REVIEW_REPORTS-32 wrote 422), `fixture_not_exportable` is `422` (INTEROP-03 wrote 409), and REVIEW_REPORTS-32's `idempotency_in_flight` is `idempotency_conflict`. Worker-side conditions with the same names (the broker's `QueryBudgetExceeded`) stay job failures with partial evidence kept; only the admission-time estimate is an HTTP refusal.

| Code | HTTP | Raised when |
|---|---|---|
| `reviewer_not_independent` | 403 | A review decision, submit or confirmation by an actor the independence rule excludes: the campaign creator (`runs.created_by`), the author of the latest revision, the requester of the verify run being resolved, or a system principal; an `admin` rank never bypasses it because the check compares identities, not ranks (REVIEW_REPORTS-12). Written after the `success=False` `finding.review` row; the envelope carries `relation` naming which exclusion applied. |
| `auth_profile_in_use` | 409 | `DELETE /v1/auth-profiles/{id}` while a live (non-deleted) endpoint or LLM target in the project references the profile, so a campaign can never pick up a dangling credential; the envelope carries `target_ids` (ENDPOINT-18). |
| `idempotency_key_reused` | 409 | An `Idempotency-Key` presented again by the same principal on the same project with a different request: the canonical-body `request_sha256` or the route differs from the stored one. Nothing is replayed and nothing is admitted; the envelope carries the stored `request_sha256` (REVIEW_REPORTS-32, BULK-11). |
| `idempotency_conflict` | 409 | An `Idempotency-Key` whose original request is still reserved and unfinished: neither a replay nor a fresh admission is possible until it completes, or a 5xx releases the reservation (REVIEW_REPORTS-32's `idempotency_in_flight`). |
| `review_state_conflict` | 409 | A review decision whose `expected_status` or `expected_review_state` no longer matches the finding's row: the compare-and-set loser of REVIEW_REPORTS-06, written with a `success=False` chain row. The envelope carries the current `status` and `review_state`; `run_terminal` stays the code for a run or finding that is already terminal. |
| `snapshot_archived` | 409 | A report fetch with `?snapshot=<id>`, a render from, or an archive request against a report snapshot whose `archived_at` is set, by a caller who is not admin; restore is the only transition out, and bytes are never deleted (REVIEW_REPORTS-22). |
| `bulk_too_large` | 413 | `POST /v1/models/bulk` whose `Content-Length` exceeds `REDSIM_ML_BULK_UPLOAD_MAX_MB`, checked before any byte reaches the blob store (BULK-13). A single file over `REDSIM_ML_UPLOAD_MAX_MB` is a per-item refusal in the response body, not a request failure. |
| `auth_profile_required` | 422 | Endpoint registration (`POST /v1/models` with `source: "endpoint"`) that names no `auth_profile_id`: endpoint targets never carry an inline credential (ENDPOINT-06). |
| `probe_key_required` | 422 | LLM target registration (`endpoint_kind: llm`) or `POST /v1/models/{id}/probes` whose `AuthProfile` is not a `bearer` profile carrying the probe persona's key, or whose target names none; probe traffic never borrows the narrative writer's key or persona (LLM-03, LLM-04, LLM-26). |
| `batch_member_refused` | 422 | `POST /v1/campaigns/batch` where at least one member fails the per-member admission pre-pass; the batch is refused whole, nothing is admitted, and the envelope carries `members: [{target_id, code, message}]` after the `success=False` `batch.create` row (BULK-03). |
| `bulk_too_many_files` | 422 | `POST /v1/models/bulk` with more `files` parts than `REDSIM_ML_BULK_UPLOAD_MAX_FILES` (BULK-13). |
| `fixture_not_exportable` | 422 | `POST /v1/runs/{id}/dataset` on a run whose target is a CI fixture (the CIFAR-10 fixture, `TinyTarget`): fixture runs are never exported as adversarial datasets (INTEROP-10; D3). |
| `query_budget_exceeded` | 429 | Admission of a campaign or explain job on an endpoint target whose worst-case query estimate exceeds `REDSIM_ML_ENDPOINT_MAX_ROWS` or `REDSIM_ML_ENDPOINT_MAX_REQUESTS`; the envelope carries `estimate` and `cap`, after the `success=False` admission row (ENDPOINT-08). |

**17.3 addendum, third table (Phase B wave B4, `fix-api-services` track, 2026-09-09).** The codes the wave B2 routes needed and resolved with `getattr` fallbacks onto a documented neighbour because the table lacked them: the endpoint route's D3 attestation (ENDPOINT-31, stood in by `license_required`), the project scoring override's weight vector (REVIEW_REPORTS-28, stood in by `params_out_of_range`), the LLM-12 registration and probe-run set (stood in by `params_out_of_range`, `probe_set_unknown` and `rate_limited`, with the planned name in `reason`), and the endpoint credential refusal on a synchronous call. Same envelope, same rules as the three tables above, and `tests/ml/test_error_codes.py` parses this table too. Every row is a refusal: none is a 2xx marker and none joins the plain-string set. `endpoint_auth_failed` is `502`, beside `endpoint_unreachable`, because the upstream endpoint produced the answer; inside validate or a campaign it stays the target's `refusal_reason: load_failed` with the class and code under `validation.probe` (ENDPOINT-28 widening still deferred). The `reason` field the routes carried while the codes were off the table is kept for one release so a client keyed on it keeps working.

| Code | HTTP | Raised when |
|---|---|---|
| `attestation_required` | 422 | `POST /v1/models` with `source: "endpoint"` whose `evaluation_instance_attestation` is absent or not `true`: only a non-operational evaluation instance is registered (D3, ENDPOINT-31); `field` names it, after the `success=False` `model.register` row. |
| `scoring_weights_invalid` | 422 | `PUT /v1/projects/{slug}/ml-scoring` (or the settings route) with a weight vector that is partial, non-numeric or does not sum to 1 within 1e-9; nothing is filled in or renormalised (REVIEW_REPORTS-28); `field` is `ml_scoring.weights`. |
| `model_id_invalid` | 422 | LLM target registration (`endpoint_kind: llm`) whose `model_id` is not a canonical Pythia id (`<vendor>/<model>` or `pythia/auto`) (LLM-12). |
| `model_not_chat` | 422 | LLM target registration naming an embedding model: probes need a chat completion endpoint (LLM-12). |
| `gateway_url_required` | 422 | LLM target registration without `gateway_url` while `PYTHIA_BASE_URL` is not set on the API (LLM-12). |
| `unknown_probe` | 422 | `POST /v1/models/{id}/probes` naming a `probe_ids` entry outside the committed catalog (LLM-12). |
| `probe_excluded` | 422 | `POST /v1/models/{id}/probes` naming a probe the catalog lists as excluded (HarmBench and the other recorded exclusions) (LLM-07, LLM-12). |
| `probe_detector_unavailable` | 422 | `POST /v1/models/{id}/probes` with `detector_mode: "hf"` while `REDSIM_LLM_PROBE_HF_DETECTORS` is not `1`, or naming a probe whose only detectors need a model this deployment does not permit offline (LLM-09, LLM-12). |
| `llm_probe_quota_exceeded` | 429 | The project's daily LLM probe-run quota (`REDSIM_LLM_PROBE_MAX_RUNS_PER_PROJECT_PER_DAY`) is spent; the envelope carries `limit`, `used` and `retry_after` (LLM-21). |
| `endpoint_auth_failed` | 502 | A synchronous endpoint call made on the caller's behalf was answered `401` or `403` to the `AuthProfile` credential (none exists yet); inside validate or a campaign the same condition is the target's `refusal_reason: load_failed`, never an HTTP code. |
...
**17.4 status addendum (2026-09-09, after Phase B waves B0 to B4).** The two paragraphs and the table above are kept as written on 2026-09-08. Wave B0 (`0b0981b`, INTEROP-01) mounted every route named here as a gated `501 not_implemented` stub with `phase: "B"` and a reason; wave B2 replaced the LLM probe routes, `report.render` and the snapshots list with their handlers and added the review, compare, ml-scoring, retests and analyst-draft paths; wave B3 replaced the last 14 stubs (dataset export and consume, ATLAS coverage, the integrations roster and the Foundry push, batch campaigns, bulk upload, bulk verify and the capacity view). At the B4 push no route on this surface answers `501 not_implemented` as a stub: `tests/ml/test_phase_b_stubs.py` pins every path to a real handler and `tests/test_docs_phase_b_consistency.py` checks the status column of the Phase B table in `docs/api/v1.md` against the app's OpenAPI document. What still answers `501` does so by decision, with a reason: the `llm` modality on `/attacks` (probe runs have their own route), `text` and `detection` uploads and consumed slices, an `endpoint_kind` other than `predict` or `llm`, the Lattice roster entry (D3), and `integration_disabled` on the Foundry push while it is unconfigured (a configuration state). The Phase A dismissal route keeps its shape byte for byte; the review transitions live at `POST /v1/findings/{id}/review[/{transition}]` per section 6.4. Departures from the table above, recorded in `docs/architecture/ml-vertical.md` "Accepted divergences": the export refuses a fixture run with `422 fixture_not_exportable` (not `409`), a non-terminal or slice-less run with `409 export_unavailable` and a second concurrent export with `409 export_in_flight`, and no `export_blocked_pending_d006` code exists because imagery exports stay in the artifacts bucket by owner default INTEROP-34; `GET /v1/datasets/{id}` answers a plain `404` for an unexported run rather than `dataset_not_exported`; `POST /v1/runs/{id}/dataset` takes no `include_card` flag (the template card is always written); the export of a run whose slices were not retained is `409 export_unavailable` rather than regenerated inside the sandbox child (INTEROP-07, recorded); policy-gated export redaction (F007/F008) stays unbuilt while D006 is open, and the reports say so.

## 18. Web UI

The web app is `@redsim/web` (D7): the Next.js 14 app router under `web/`, Tailwind, `@redsim/design-system` from `packages/design-system`, the `api()` client in `web/src/lib/api.ts` (cookie or bearer, CSRF header, request id), NextAuth with the Keycloak provider plus the dev-token path (hidden when `NEXT_PUBLIC_REDSIM_ENV=prod`), and the hooks `useRequireAuth`, `useRoles`, `useRunEvents`. `RoleGated` remains UX-only; the API is the authorization boundary (section 7.9). The header brand reads "Adversarial ML Red-Team Simulator". Every page carries the footer line from S2: *"Proof of concept on open, unclassified public data. Results are evidence for human review, not a safety, readiness, or certification determination."*
...
9. **Provenance** — redsim / Python / torch / ART / SHAP / numpy versions, model `sha256`, dataset id + revision, seed, `sample_indices_sha256`, device, hostname, `nondeterminism[]`, and a **Rerun with same config** button that starts a new linked campaign (`parent_run_id`).
...
| Slice 3 — human review and accountable reporting (F006 + F007 + F008 tools) | Candidate recommendations, dismissal, verify, report, compare, audit trail | M3 (rules + Pythia writer), M6 (verify-after-harden, ΔMRI), M5b (upload dialog), M7 (Fargate) | Steps 3 (verify-after-harden), 5 (ONNX upload), 6 (Fargate) | Verify produces a measured ΔMRI and a `validation_state` change on the chain; md / json / html reports carry limitations and provenance; compare blocks incompatible runs; `redsim audit verify` passes on the campaign chain |
...
William's plans proposed Replit monorepo paths. D10 replaces them with redsim paths. Feature `plan.md` files keep their FR and verification content; their path tables are read through this mapping.
...
| `lib/db/src/schema/*.ts` | `redsim/db/models.py` and Alembic revisions under `redsim/db/migrations/versions/` (`0010_ml_vertical`, after `0009_tenant_org_id_guard`, adds `targets.detail`, `ml_campaigns` and the section 5 vocabularies) |
| `artifacts/api-server/src/routes/assurance/*.ts` | `redsim/api/v1/{models,datasets,attacks,defenses,ml_capabilities,artifacts,compare,ml_findings}.py` (new) and the existing `runs.py`, `runs_cancel.py`, `findings.py`, `verify.py`, `reports.py`, `audit.py`, `projects.py`, `auth_profiles.py` |
| `artifacts/api-server/src/services/assurance/*.ts` | `redsim/services/{ml_models,ml_campaigns,ml_findings}.py` (new), `redsim/services/{runs,verify,targets,reports,evidence}.py` (existing), and the domain code in `redsim/ml/{schema,targets,attacks,explain,recommend,scoring,datasets,defenses,sandbox}` |
| "No worker runtime selected" (F004) | `redsim/workers/celery_app.py`, `redsim/workers/tasks/{model_validate,attack,explain,harden,verify}.py`, `redsim/workers/bootstrap.py`, `redsim/ml/sandbox.py` (on the `redsim/scanners/sandbox.py` primitives) |
...
| `specs/008-audit-governance/contracts/audit-event.yaml` | `redsim/audit/chain.py` record shape (`to_record`) and `docs/architecture/audit-chain.md` |
...
|---|---|---|---|---|
| `postgres` | `redsim-postgres:dev` (postgres:16 + pgaudit, `deploy/Dockerfile.postgres`) | 5432 | Runs, Jobs, Findings, Artifacts, campaign/score records (`ml_campaigns`, section 5), RLS-protected tenant tables, Postgres audit chain | None. The Alembic migration `0010_ml_vertical` (section 5.3) runs through the API image's existing `alembic upgrade head` entrypoint. |
| `redis` | `redis:7-alpine` | 6379 | Celery broker (`/0`) and result backend (`/1`) | None |
| `keycloak` | `quay.io/keycloak/keycloak:24.0 start-dev --import-realm` | 8080 | OIDC issuer for the `redsim` realm (`deploy/keycloak/realm-export.json`) | Add the `viewer` realm role (section 7.2). `REDSIM_AUTH_MODE=dev` (dev tokens, `make token-for`) is permitted for the demo per D11 (D002). |
| `minio` | `quay.io/minio/minio` | 9100 (S3), 9101 (console) | Blob store for uploaded models, bundled assets, adversarial examples, SHAP PNG/npz, robustness-curve artifacts; container-internal `http://minio:9000` | Create a second bucket with Object Lock enabled when exercising WORM export locally (`REDSIM_WORM_BUCKET`, default `redsim-worm`). |
| `redsim-api` | `redsim-api:dev` (`deploy/Dockerfile.api`, `pip install -e ".[api,worker]"`) | 8000 | `/v1` routes incl. the new `/v1/models`, attack/explain/harden/verify endpoints (section 17); admission (audit event → Run/Job rows → enqueue) | None to the image. The `ml` extra is **never** installed here; a test asserts the API process does not import `torch`, `art`, `shap`, or `onnxruntime` (section 22). |
| `redsim-worker` | `redsim-worker:dev` (`deploy/Dockerfile.worker`), `celery … worker -Q scans` | — | Runs `model.validate`, `attack.run`, `explain.run`, `verify.replay` (section 10) on the long-running `scans` queue; the only process that loads a model, and only inside the sandboxed loader subprocess (section 9) | Install `.[worker,ml]`; drop the `project_repos/` `COPY` and editable installs (the submodules were deleted, so the current `COPY project_repos/` line fails the build); mount a named volume for the dataset cache (`REDSIM_ML_DATASET_CACHE`) so section 11's dataset fetch happens once. |
| `redsim-worker-default` | same image, `-Q default` | — | `harden.recommend` (rules + the Pythia writer; section 8.3 routes it here so the LLM call never shares a pool with model loading), report rendering, stale-job reaper, WORM export, tenant-integrity checks | Add the Pythia env (20.3) and unset `REDSIM_DISABLE_LLM` on this service when the narrative should run; it never loads a model. |
| `redsim-beat` | same image, `celery … beat` | — | Fires `redsim.reap_stale_jobs` (turns a crashed `running` job into `failed`, section 6) and `redsim.export_chains_to_worm` | None |
| `redsim-web` | `redsim-web:dev` (`deploy/Dockerfile.web`, pnpm workspace `@redsim/web` + `@redsim/design-system`) | 3300 → 3000 | Pages in section 18 | None to the image; new pages are ordinary Next.js routes. |
| `redsim-log-ingest` | `redsim-log-ingest:dev` | 4319 | Postgres mirror of application logs (`application_logs`), queryable by `run_id` | None |
...
All existing `REDSIM_*` variables keep their meaning (`REDSIM_DB_URL`, `REDSIM_BROKER_URL`, `REDSIM_RESULT_BACKEND`, `REDSIM_BLOB_BACKEND=s3`, `REDSIM_S3_ENDPOINT/BUCKET/REGION/ACCESS_KEY_ID/SECRET_ACCESS_KEY`, `REDSIM_AUTH_MODE`, `REDSIM_OIDC_ISSUER`, `REDSIM_OIDC_JWKS_URL`, `REDSIM_CORS_ORIGINS`, `REDSIM_WORKER_SIGNING_KEY`, `REDSIM_AUTH_PROFILES_KEY`, `REDSIM_PLUGINS_SANDBOX`, `REDSIM_PLUGIN_SANDBOX_CPU_SECONDS/MEMORY_MB/FILESIZE_MB`, `REDSIM_PLUGIN_SANDBOX_NETWORK`, `REDSIM_WORM_EXPORT`, `REDSIM_WORM_BUCKET`, `REDSIM_WORM_RETENTION_DAYS`, `REDSIM_DISABLE_LLM`, `REDSIM_LLM_BUDGET_STRICT`, `REDSIM_LLM_GUARDRAILS`).
...
| `REDSIM_ML_LLM_MODEL` | worker | Canonical model id (`<vendor>/<model>` or `pythia/auto`) for the hardening narrative. This **renames** the scaffold's `REDSIM_LLM_MODEL` (read today in `redsim/llm/pythia.py`, referenced in `redsim/ml/schema.py` and `tests/test_llm_pythia.py`); the rename is an M0 task and the test changes with it (section 22). redsim's per-task router (`redsim.llm.router.route`, task `ml.harden_narrative`) and budget checker still decide whether and at what budget the call is made; Pythia is the transport. |
| `REDSIM_ML_UPLOAD_MAX_MB` | api | Hard cap on uploaded model size (default 512); exceeding it returns `413 model_too_large` (section 17.3). |
| `REDSIM_ML_DATASET_CACHE` | worker | Directory (volume) for the dataset cache used by `build-assets` and the loaders in section 11. |
| `REDSIM_ML_WORK_DIR` | worker | Root of the per-job work directories the sandbox parent populates (default `$TMPDIR/redsim-ml/<job_id>`, mode 0700; section 9.4). |
| `REDSIM_ML_KEEP_WORK_DIR` | worker, optional | `1` keeps work directories after a job for debugging; default removes them. |
| `REDSIM_ML_SANDBOX_TIMEOUT_S`, `REDSIM_ML_SANDBOX_CPU_SECONDS`, `REDSIM_ML_SANDBOX_MEMORY_MB`, `REDSIM_ML_SANDBOX_FILESIZE_MB`, `REDSIM_ML_SANDBOX_THREADS` | worker | Resource ceilings of the ML sandbox child (defaults 1200 s, 900 s, 4096 MB, 1024 MB, 2 threads; section 9.4). There is no network switch: the ML child never gets network configuration. |
| `REDSIM_ML_MAX_ADV_ARTIFACT_MB` | worker | Size above which the full adversarial slice is not retained as an artifact (default 64; section 12.8). |
| `KAGGLE_USERNAME`, `KAGGLE_KEY` | `redsim ml build-assets` one-off run only (worker image) | Kaggle API credentials used solely to download `malicious_phish.csv` (section 11.3.3). Never set on the API, web, steady-state worker or beat services, never in the sandbox child environment (section 9.4), never logged or written to a manifest; CI does not need them because the committed sample is used. |

If any of the required Pythia variables (`PYTHIA_BASE_URL`, `PYTHIA_API_KEY`, `REDSIM_ML_LLM_MODEL`) is missing the narrative is skipped, not faked: `PythiaSettings.from_env()` returns `None`, the recommendation keeps `narrative = None`, `narrative_source = "rules"`, and the UI says so (section 18.5).
...
|---|---|---|
| API (`redsim-api`) | ECS Fargate service behind an ALB | Reuse `deploy/Dockerfile.api`. Run `alembic upgrade head` as a one-off ECS task before each rollout rather than from every API task's entrypoint, so concurrent tasks never race the migration. |
| Worker (`redsim-worker`) | ECS Fargate service | Reuse `deploy/Dockerfile.worker` with the `ml` extra (20.2). The default image CMD consumes `scans,default`, so one service suffices for the demo; split into two services (`-Q scans` / `-Q default`) when long attacks starve bookkeeping, mirroring compose. Size the task for torch + ART + SHAP on CPU and raise Fargate ephemeral storage above the 20 GiB default if the dataset cache and uploaded models require it. The sandboxed loader's rlimits (section 9.4) must fit inside the task's memory limit. |
| Beat (`redsim-beat`) | ECS Fargate service, one task | Reaper and WORM export schedule. Exactly one task; `celery beat` is not safe to scale. |
| Web (`redsim-web`) | ECS Fargate service behind the ALB, or Amplify | Reuse `deploy/Dockerfile.web`. |
| Log ingest (`redsim-log-ingest`) | ECS Fargate service, optional | Postgres log mirror; skip for the demo if time is short. |
...
| Object store | S3 bucket (artifacts) + a second S3 bucket created with Object Lock (WORM audit export) | Models, adversarial examples, SHAP outputs, robustness curves, reports in the first; `redsim.export_chains_to_worm` writes chains to the second with `COMPLIANCE` retention (`REDSIM_WORM_RETENTION_DAYS`, default 2555). Object Lock must be set at bucket creation. |
| Secrets | AWS Secrets Manager → task-definition secrets | `PYTHIA_API_KEY`, `REDSIM_WORKER_SIGNING_KEY`, `REDSIM_AUTH_PROFILES_KEY` (Fernet), `NEXTAUTH_SECRET`, DB password. **No model-provider keys exist anywhere in the deployment** (D5); Pythia holds them. Prefer an IAM task role over static `REDSIM_S3_*` keys. |
| Auth | Keycloak on Fargate (realm import from `deploy/keycloak/`), or Cognito via OIDC (`REDSIM_OIDC_ISSUER`, `REDSIM_OIDC_JWKS_URL`) | Dev-token mode (`REDSIM_AUTH_MODE=dev`) is acceptable for the hackathon demo (D11/D002) only when the ALB is restricted to the team's addresses; it is never acceptable on an open listener, and it is refused when `REDSIM_ENV=prod`. |
...
- Uploaded models and datasets are retained under the same lifecycle as other redsim artifacts; deletion is an explicit, audited operation, and the retention/redaction policy remains open in the decision register (D006, section 19).

### 21.6 Web and API hygiene (inherited)

CSRF double-submit on cookie sessions, the WebSocket Origin gate, strict CSP on served HTML reports and artifacts (`default-src 'none'`), `X-Content-Type-Options: nosniff`, per-route RBAC checked server-side (the web `<RoleGated>` is cosmetic), rate limiting, and JWKS caching are all restored redsim behavior and apply to the new routes without change. Upload endpoints add: a size cap (`REDSIM_ML_UPLOAD_MAX_MB`), content sniffing by magic bytes rather than by extension or client `Content-Type`, and a filename that is never used as a storage key.

### 21.7 Phase B connector (not built)

The black-box endpoint connector (D2 Phase B) will reuse `AuthProfile` (Fernet-encrypted bearer/header credentials, key rotation via `REDSIM_AUTH_PROFILES_KEY_PREVIOUS`) and must add an egress allowlist and private-address blocking before any request leaves the worker; redsim's existing target-ownership verification (DNS TXT) gates `verified` targets. Until then `POST /v1/models` with `source: "endpoint"` returns `501 not_implemented` (section 17) and the UI shows the reason.
...
The plan keeps redsim's existing suite green, adds an offline `ml` tier that needs no dataset download and no network, and asserts the reporting principles as code. Commands: `pytest -q` (default; `addopts` excludes `docker`, `e2e`, `slow`, `auth_required`), `pytest -m ml`, `pytest -m integration`, `make check` (ruff → mypy → pytest), `pnpm --filter @redsim/web test` (vitest), `pnpm --filter @redsim/web typecheck`.
...
| `e2e` | yes | live compose stack, `REDSIM_E2E` set | One browser campaign against the bundled image model (Playwright, `web/tests/`). |

Budget: the default `pytest -q` stays under a minute with no services and no LLM (the redsim "offline path stays sacred" rule); the `ml` tier targets a few minutes on a laptop CPU because every test uses 8×8×3 inputs.
...
| `tests/ml/test_url_features.py` | unit | The lexical feature extractor of 11.3.3 (`redsim/ml/datasets/url_features.py`) is a pure function of the string: identical output for identical input; defined output for empty, malformed and non-ASCII strings; feature values inside the manifest's declared ranges on the committed sample `tests/ml/fixtures/malicious_urls_sample.csv`; with `socket.socket` patched to raise, extraction over the whole sample and the tabular target's `predict_proba` on it make no network call (the "URLs are data" rule); the report renderer emits no anchor element for a URL string (`redsim/report.py` escaping). |
...
| `tests/ml/test_sandbox_loader.py` | ml | The loader is invoked through the sandbox seam (list argv, `start_new_session=True`, `preexec_fn` present on POSIX); the child environment contains no `REDSIM_*` secret, `PYTHIA_API_KEY`, or proxy variable; a loader that sleeps past the timeout is killed as a process group and the Job records `SandboxTimeout`, distinct from `ModelLoadRefused`; loader stdout is a single JSON line. |
| `tests/test_api_process_has_no_ml.py` | unit | Importing `redsim.api.app` and building the app leaves `torch`, `art`, `shap`, `onnxruntime` absent from `sys.modules`. |
...
- `web/tests/ml_campaign.spec.ts` (Playwright, `e2e`): sign in with a dev token, pick the bundled image model, launch FGSM + PGD with the default grid, wait for `succeeded`, open the finding, run "Verify fix", open `/audit`. Gated by `REDSIM_E2E`; not part of the merge gate.
...
### 22.6 Phase B testing addendum (2026-09-09, waves B0 to B4)

This addendum records the tiers, doubles and evidence files as they exist at the wave B4 push, beside the Phase A plan above (which is kept as written; where the two differ, this addendum and `docs/dev/testing.md` describe the tree). Every file named here is on the tree; `tests/test_docs_phase_b_consistency.py` fails when one is not.

**Tiers and lanes.** Five tiers, four of them with a CI lane:

| Tier | Selected by | Lane | What it proves |
|---|---|---|---|
| default (unit and sqlite integration) | no `-m` flag; `addopts` deselects `docker`, `e2e`, `slow`, `auth_required` and `garak` | `Unit tests (py3.12)` and `(py3.13)`, `Coverage gate`, `API integration` | the platform and the vertical's pure units, the route tests over the sqlite harness, the P0 schema tripwire `tests/ml/test_schema_compat.py`, the API-process import guard `tests/test_api_process_has_no_ml.py`, the error-code table test `tests/ml/test_error_codes.py` (the spec 17.3 table and all three dated addenda), the policy mirrors `tests/test_policy_ml_actions.py`, the migration tests `tests/test_migration_0011.py` and `tests/test_tenant_rls.py` (Postgres cases in the Coverage gate), the Phase B surface pin `tests/ml/test_phase_b_stubs.py`, and the docs half of the gate `tests/test_docs_phase_b_consistency.py` |
| `ml` | `-m ml`, needs the `ml` extra | `Unit tests (py3.12)`, `Coverage gate` | the library layer and the Phase B services and workers on the tiny doubles: `tests/ml/test_campaign_golden.py` (the frame-plus-runner split byte for byte against the frozen pre-refactor copy), `tests/ml/test_campaign.py`, `tests/ml/test_text_modality.py`, `tests/ml/test_detection_modality.py`, `tests/ml/test_attacks_phase_b.py`, `tests/ml/test_explain_blackbox.py`, `tests/ml/test_endpoint_contract.py`, `tests/ml/test_endpoint_egress.py`, `tests/ml/test_endpoint_target.py`, `tests/ml/test_hardening.py`, `tests/ml/test_endpoint_routes.py`, `tests/ml/test_admission_phase_b.py`, `tests/ml/test_tasks_phase_b.py`, `tests/ml/test_review_workflow.py`, `tests/ml/test_reports_phase_b.py`, `tests/test_idempotency.py`, `tests/ml/test_interop_export.py`, `tests/ml/test_interop_consume.py`, `tests/ml/test_atlas_foundry.py`, `tests/ml/test_batches.py`, `tests/ml/test_bulk_upload_capacity.py`, `tests/ml/test_cli_matrix.py`, `tests/ml/test_datasets.py` |
| `garak` | `-m garak`, needs the `garak` extra (`garak>=0.16,<0.17`); skipped at collection without it | `garak offline` (job `garak-offline`) for `tests/ml/`; `E2E tier (python, eager Celery)` (job `e2e-python`, which installs the extra since wave B4) for the e2e-gated file | real garak 0.16.0 through `PythiaGenerator` against `tests/ml/fake_openai_server.py` on the loopback interface, never a live gateway: `tests/ml/test_llm_core.py`, `tests/ml/test_llm_routes.py` and `tests/e2e/test_ml_llm.py`. Since wave B4 the gate's garak step fails on pytest exit 5 (nothing collected), on an all-skipped run and on a missing extra; the wave B0 rule that let an empty collection pass is gone from the script and the workflow |
| `e2e` | `tests/e2e/`, stamped by its `conftest.py`, runs only with `REDSIM_E2E=1`; `REDSIM_E2E_POSTGRES_URL` turns the Postgres RLS lane on | `E2E tier (python, eager Celery)`, through `scripts/phase_b_gate.sh --only e2e` | the completion criteria end to end (real API, admission, eager Celery, the real sandbox child, the CLI, over sqlite on a synthetic asset tree): the wave 3 smoke file `tests/e2e/test_harness_smoke.py`, the wave 4 files `tests/e2e/test_ml_campaigns.py`, `tests/e2e/test_ml_verify_upload_reports.py`, `tests/e2e/test_ml_governance.py`, and the wave B4 files `tests/e2e/test_ml_endpoint.py`, `tests/e2e/test_ml_llm.py`, `tests/e2e/test_ml_text_detection.py`, `tests/e2e/test_ml_attacks_harden.py`, `tests/e2e/test_ml_review_reports.py`, `tests/e2e/test_ml_interop.py`, `tests/e2e/test_ml_bulk.py`. A B4 file that meets a product defect fails with `pytest.fail(..., pytrace=False)` naming the module, never with a weaker assertion |
| browser e2e | Playwright, `workflow_dispatch` with `run_e2e=true` | `Stack E2E`, on demand only | the web app against the compose stack; not part of the Phase B waves (the web UI is the plan's one deferral) |

**Phase B doubles** (beside `TinyTarget` and `TinyTabularTarget` of 22.2): `tests/ml/fakes_text.py` (`TinyTextTarget`, a seeded TF-IDF plus logistic regression on a synthetic corpus with a fixture lexicon), `tests/ml/fakes_detection.py` (`TinyDetector`, a differentiable anchor detector honouring the torchvision contract, wrapped in ART's `PyTorchFasterRCNN`), `tests/ml/tiny_endpoint_server.py` (`TinyEndpointServer`, a stdlib HTTP server over `TinyTarget` speaking `endpoint-v1` with bearer or header auth and misbehaviour switches), `tests/ml/fake_openai_server.py` (an OpenAI-compatible gateway on the loopback interface with a low-entropy fake token), `tests/ml/fake_foundry_server.py` (the Foundry Datasets v2 create, upload, commit and abort paths with a JWT-shaped fake token assembled from segments at import), and the fixtures `tests/ml/fixtures/run_record_phase_b.json`, `sms_spam_sample.tsv`, `synonyms_tiny.json` and `public_index.csv`. `tests/ml/_campaign_pre_refactor.py` is the frozen pre-refactor `run_campaign` (sha256 asserted) the golden test compares against. No double is ever listed, registered or served as a result.

**The gate.** `make check-phase-b` runs `scripts/phase_b_gate.sh`: ruff, mypy, the default tier, the `ml` tier, the `garak` tier, the `e2e` tier, `mkdocs build --strict`, `tests/test_docs_phase_b_consistency.py` and, when `REDSIM_API_URL` and `REDSIM_API_TOKEN` are set, HTTP probes against a running stack followed by `redsim audit verify --all`. It stops at the first failure and names the section 26 criterion (26.27, 26.20, 26.2 to 26.24, 26.11, 26.9, 26.17, 26.22). The `e2e-python` and `garak-offline` jobs call the same script (`docs/dev/ci.md` "Phase B gate").

**Counts of record.** The last counts labelled with their commit are the B3 integration's on `main` at `703f8f6`: default tier 2634 passed, 35 skipped, 13 deselected; `ml` 433 passed, 1 skipped; `garak` 12 passed; `e2e` 22 passed; ruff and `mypy redsim` (253 files) clean. The B4 tree adds the seven e2e files and the gate; its counts are the B4 assembler's to record with its commit, and the e2e tier at the B4 push is not claimed green: the attributed product defects the B4 files report are listed in the README's open items.
...
**Timeline statement.** The product owner's original estimate was 1–2 days. Phase A as decided — image and tabular paths, ONNX/state_dict upload with sandboxed loading, eps sweep, verify-after-harden, MRI, full audit coverage, on the full redsim platform — exceeds a 1–2 day build. S1 §16's risk stands: freeze Phase A first, execute it in the D8 order below, and do not start any Phase B item until the image path runs end to end in the browser. If time runs out, the milestones after the cut line are reported as not implemented in the UI and README (section 26), never simulated.

| # | Milestone | Deliverable (S1 §14, updated) | D8 order | Spec Kit slice / features | Exit check |
|---|---|---|---|---|---|
| M0 | Scaffold | `redsim/ml/` package (schema, target/attack protocols, and Pythia client already exist as contracts); Alembic migration `0010_ml_vertical` adding `targets.detail`, `ml_campaigns`, and the `Target.kind` / `Job.type` / `Run.scanner` values of section 5; `redsim/ml/schema.py` additions of section 5.3 (`CampaignConfig`, `MRIRecord`, `MLFindingDetail`, `MLModelManifest`, new `Measurement`/`Observation`/`Provenance` fields, `score` stage); the seven `Action` members and `viewer` rank (section 7); `ml` dep group (exists) plus `onnx2torch`/`safetensors` installed in `Dockerfile.worker` with the `project_repos` lines removed; `REDSIM_LLM_MODEL` → `REDSIM_ML_LLM_MODEL`; `/v1/scans` unmounted and stale `Action` members pruned; `redsim ml build-assets` CLI skeleton; decision register rows D001–D005 recorded (D11); constitution amendment proposals appended (D12). | 1 | Gate 0 + Slice 1 (F001 and F008 event foundation are already redsim; F002 schema) | `alembic upgrade head` on a fresh DB; `pytest -q` green; API process imports no ML library. |
...
| M7 | Deploy | Fargate services + RDS + ElastiCache + two S3 buckets + Secrets Manager; one-off migration and `build-assets` tasks; Pythia egress from the worker only; demo run end to end with `redsim audit verify --all`. | 9 | Slice 3 (F008 WORM export) | Section 20.4 smoke test passes; README records the measured clone-to-first-run time (section 26). |
| B1+ | Stretch (Phase B) | Black-box endpoint connector over `AuthProfile` with egress allowlist; Carlini-Wagner, DeepFool, HopSkipJump (image), ZOO; text and detection modalities; KernelSHAP for black-box; adversarial-training defense; garak LLM-domain probes pointed at Pythia (D6) with garak's bundled probe corpora of 11.6 as probe material; pickle override (admin-only, sandboxed, audited); finding review states beyond dismissal (section 6.4); per-project scoring overrides; `Idempotency-Key`; PDF export. | after Phase A | Deferred extensions (specs/README.md) | Each item appears in the UI as not implemented with a reason until it lands. |
...
8. **Audit.** Open `/audit` for the campaign: register/validate, attack, explain, score, harden, verify, and report events in one hash chain. Run `redsim audit verify --all` in a terminal and show the result.
...
| **Eps sweep multiplies cost.** Three eps × two attacks × n samples × explain. | Runs exceed the reaper's `job_max_runtime_seconds` (3600 default) or the sandbox wall clock. | Default slice size chosen so an image campaign completes on a laptop CPU; one job per attack with per-stage progress; `REDSIM_ML_SANDBOX_TIMEOUT_S` below the Celery soft limit; raise the ceilings for the worker if measured runtimes require it. |
| **Platform restore in flight.** The restored redsim code is being made import-clean concurrently; `deploy/Dockerfile.worker` still copies the deleted `project_repos/`. | M0 cannot start until the suite is green. | The ML tier is not merged while the default tier is red (section 22.5); the Dockerfile change is an M0 task. |
...
1. A teammate can follow `README.md` from clone to a finished bundled-image-model FGSM + PGD campaign visible in the browser, on a laptop CPU with Docker, in one sitting; the measured clone-to-first-run time is recorded in the README at M7 (S2's 15-minute target applied to the lean stack; the full redsim stack targets 30 minutes including image builds). [S2, Brief]
...
9. Limitations are non-empty on every succeeded run, visible on every run page, and included in every report; they always include the standing limitations of `redsim/ml/schema.py`, the dataset caveats of section 11, and the D3 bounds statement. [S2]
...
20. Every LLM call goes through Pythia, with only metrics, rule outputs, limitations and a SHAP text summary as input, under redsim routing and budgets; no provider key exists anywhere in the repository, images, or deployment. [D5]
...
### 26.7 Phase B completion criteria (addendum 2026-09-09)

Added after Phase B waves B0 to B4, one merged list like the Phase A list above. Each criterion is observable and tagged with the owner decision (`docs/plans/12-phase-b-plan.md` section 2) or the spec section that bounds it; `**[open]**` marks a criterion that does not hold on the tree at the wave B4 push, with the reason. Phase B is complete when every item holds; anything that does not is listed in the README as open. The tags `[Dn]` are the decisions of section 4.

30. Every Phase B route is mounted behind its real gate and no route answers `501 not_implemented` as a stub; what answers `501` does so by decision with a reason (17.4 addendum). Pinned by `tests/ml/test_phase_b_stubs.py` and the status column check of `tests/test_docs_phase_b_consistency.py`. [S2, Const. IV, INTEROP-01]
31. `redsim/ml/schema.py` was extended once, additively and default-valued, with the frozen fixture validating byte-identical (`tests/ml/test_schema_compat.py`); the migration head moved once (`0011_phase_b_platform`) with RLS parity on every new table; every Phase B `Action` and error code is on the table with its policy mirrors (`tests/test_policy_ml_actions.py`, `tests/ml/test_error_codes.py`). [Const. V, plan 01 section 8]
32. A black-box endpoint registers with its credential in an `AuthProfile`, is queried only from the worker-parent broker under the egress allowlist, and the child, the audit rows, the job detail and the reports never carry the secret. [D2, D5, section 21.7; owner default ENDPOINT-26: no DNS-TXT ownership check]. **[open]** for the campaign path at the B4 push: the probe rows leave the child unscaled (`redsim/ml/targets/endpoint.py:228`), so no endpoint reaches `available` through the tiny server; the registration, refusal and RBAC halves of `tests/e2e/test_ml_endpoint.py` hold.
33. An LLM target carrying a Pythia model id runs garak probes through the gateway with its own persona and key (owner default LLM-26), HarmBench excluded (owner default LLM-08), and reports a k/n scorecard that never enters an MRI (`tests/e2e/test_ml_llm.py`). No provider key exists anywhere. [D5, D6, D9]
34. Text and detection campaigns run on bundled models over open licensed datasets (owner defaults MODALITIES-12 and MODALITIES-27) with their own scorecards; detection carries a scorecard and never an MRI (owner default MODALITIES-36). [D3, D9] (`tests/e2e/test_ml_text_detection.py`)
35. Carlini-Wagner L2, DeepFool and ZOO run with their controls under declared norms enforced at admission (`tests/e2e/test_ml_attacks_harden.py`). [12.2 as amended by the recorded divergences]
36. Adversarial training and distillation run as the `defense_apply` stage inside the verify child, register a derived target with `derived_from` lineage and record a `MeasuredDelta`; where the defense cannot be applied the run says so with the score withheld and nothing is claimed. [15.6, 16.4, D9 iv]. **[open]** at the B4 push: no bundled image target exposes a training slice to the child, so every training verify records the defense unavailable and no derived target or delta exists (ATTACKS_HARDEN-11/-13).
37. The review states `draft`, `in_review`, `confirmed`, `dismissed`, `retest_requested` and `resolved` work through one transition table with independence by identity and `resolve` gated on a compatible retest; there is no `reviewer` role (owner default REVIEW_REPORTS-13). [S3, 6.4] (`tests/e2e/test_ml_review_reports.py`)
38. `report.pdf` is a projection of the record and never a recomputation, every render is an immutable snapshot, N-run comparison carries no mean or rank, per-project weights are never renormalised, and `Idempotency-Key` replays a stored answer without a second row. [D9 ii, 15.3, 15.6 to 15.8] The completion path renders the PDF and records the first snapshot since wave B4; a PDF the renderer cannot typeset degrades to the text formats with `pdf_unavailable` recorded (open item: `redsim/ml/pdf.py` wide tables).
39. A terminal run exports as a Croissant dataset over Parquet whose manifest sha256 is the version, the projection-equality guard holds against the flip matrix, fixture runs are never exported, and imagery exports stay in the artifacts bucket (owner default INTEROP-34). [27.1, D3, D9] (`tests/e2e/test_ml_interop.py`)
40. Another team's Parquet slice is admitted with static checks in the API and parsed only in the sandbox child, refused without a licence statement, and can be bound to a model and a campaign. [27.1, 11.1, D2]. **[open]** at the B4 push: the upload admission and the target loader bind a consumed slice, but the worker parent does not yet materialise it for the loader, so a campaign on a consumed-bound model does not run end to end (INTEROP-16 remainder).
41. Every new finding carries its ATLAS technique, a campaign has a number-free coverage view, and the pinned release is cited. [27.2]
42. The Foundry push is opt-in and off by default, holds no standing credential, and sends nothing without its subscores, denominators, eps grid, `settings_hash` and the grade sentence; the Lattice push stays text only. [27.3, D3, D9; owner defaults INTEROP-26 (proven against the fake server only), INTEROP-27]
43. Batch campaigns, bulk upload and bulk verify run through the API and a CLI matrix with one audit chain per run and no aggregate score; bulk verify is one defended run projected onto every selected finding (owner default BULK-16); per-project capacity defers an over-cap admission and refuses a spent daily budget on the single routes as well as batch members (BULK-20, -21); bulk upload answers per file (BULK-13 divergence). [owner requirement 5] (`tests/e2e/test_ml_bulk.py`). **[open]** at the B4 push: a `state_dict` upload for a dataset with other than 10 classes is refused at validation because no route derives `architecture_kwargs` from the binding.
44. Every new dataset is published with its licence and a recorded caveat where one exists (TESTS_DOCS-33, MODALITIES-12, MODALITIES-27), every new action is on the hash chain, RBAC and RLS cover every new route and table, and `redsim audit verify --all` passes on a completed Phase B run. [D3, D4c, S3 F001/F006/F008]
45. `make check-phase-b` exits 0 locally against a running stack and CI is green on every lane; the plan and the docs match the tree (`tests/test_docs_phase_b_consistency.py`, `mkdocs build --strict`). [S3, Const. VI, TESTS_DOCS-36]. **[open]** at the B4 push: the e2e tier carries the attributed failures of items 32, 36, 40 and 43, the gate has not been run against `make up`, and no CI run on `main` after `58461cc` has been read.
