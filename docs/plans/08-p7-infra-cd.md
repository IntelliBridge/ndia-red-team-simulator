> **STATUS (2026-09-09, Phase B wave B4 documentation pass).** The runtime this phase plans exists on `main`: PR #19 (`b40f7e1`) is the code-only Terraform foundation under `deploy/terraform/` and PR #23 (`feat/p7-fargate-runtime`, merged as `10650da` on 2026-09-08) adds `deploy/bootstrap/` and `deploy/runtime/`, a public HTTPS Fargate demo runtime at https://redsim.ndia.agiledefense.xyz that its author reports applied to the AWS account (health, login and OIDC discovery answering 200, unauthenticated API 401, migrated through `0010` at the time of the PR; the next rollout has to migrate to `0011`) with workers at zero tasks until a pinned asset bundle exists, demo users, project memberships and real model assets outstanding and automatic ECS rollout disabled. `deploy-aws.yml` builds and pushes the three images under GitHub OIDC and skips its deploy job while `ECS_CLUSTER` is unset. No campaign has been run on that runtime. Its completion is package E of `docs/plans/10-remaining-work-brief.md`, not a Phase B wave, and the operational posture of the Phase B integrations (S3 prefix for exports, HTTPS egress from the default pool, owner decision INTEROP-30) is a runbook item of that package. **The body below is the v2 plan and is not rewritten**: read `deploy/runtime/README.md`, `docs/ops/deploy.md` and master plan section 4.1 for what exists.
>
