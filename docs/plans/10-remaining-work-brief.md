...
| Id | Requirement | State on 2026-09-09 | Acceptance check | Size |
|---|---|---|---|---|
| B1 | Do not redo the three `main` fixes wave 4 landed in `d8a9f15` and `e73dea0`: `python-multipart` in the `api` extra (coverage job), the `redsim.ml.datasets.sampling` to `redsim.ml.targets` import cycle and the lazy torch imports behind `redsim ml build-assets` (torch-less lane), and the Next.js advisory baseline in `.trivyignore`. Confirm the run for `e73dea0` or later is green on those jobs before B2. | Closed 2026-09-09 (package B PR). The fixes are on `main` (`d8a9f15`, `e73dea0`); the run could not be read green because every Redsim CI run from the wave B0 push (`934838e`) to the B3 push (`703f8f6`) failed at workflow parse in 0 s: `garak-offline` set `${{ runner.temp }}` in a job-level `env`, where the `runner` context is unavailable. Fixed in `.github/workflows/redsim-ci.yml` (XDG variables moved to the step `env`), `actionlint` clean. Green is proven only by the run for this PR. | `gh run list --branch main --workflow redsim-ci.yml --limit 1` shows success on every job except, until B5, the Next.js build. | S |
| B5 | One vitest test fails on `main`: `web/src/app/findings/[id]/page.test.tsx` "submits selected defense and editable params". Its `useDefenses` mock has no `art_class` and expects a three-argument `verifyFinding` call, while the page enables Verify only when a candidate recommendation references the selected defense's `art_class` and calls `verifyFinding` with a fourth argument (the recommendation id). Align the test with the page behaviour (add `art_class` to the mock, expect the fourth argument). This is a test fix, not UI work. (void since 2026-09-09: the verify paradigm was removed) | Closed 2026-09-09 with evidence: fixed on `main` by `b93d9a9` (#24). `pnpm vitest run` in `web/` on `703f8f6`: 43 files, 313 tests passed. | `pnpm --filter @redsim/web test` reports 274 passed; the Next.js CI job is green. | S |
| B2 | A dedicated `ml` CI job: installs `.[test,dev,ml]` with the CPU torch wheel cached (`actions/cache` keyed by the torch version), runs `pytest -q -m ml` offline, and is a required check. Today the py3.12 unit job installs torch inline and runs the ml tests with the default tier. | Closed 2026-09-09 (package B PR): job `ml-tier`, name `ML tier (py3.12, ml extra)`, in `redsim-ci.yml`; `actions/cache` keyed by the torch version `pip index versions` reads from the CPU index, wheels installed `--no-index` from the cache. Installs `api,worker` too because the ml-marked route and worker tests build the app. Making it a required check is a repository setting (`main` has no branch protection or ruleset): open for the repository owner, command in `docs/dev/ci.md` "The ML tier job". | A job named `ML tier (py3.12, ml extra)` passes and its torch install hits the cache on the second run. | M |
| B3 | `make check` mirrors CI: `ruff check --select E4,E7,E9,F,I redsim tests`, `mypy redsim`, `pytest -q`, and `pnpm --filter @redsim/web typecheck && pnpm --filter @redsim/web test`. | Closed 2026-09-09 (package B PR): `lint-py` now runs the CI selection, `check` runs lint, typecheck (Python and web) and test (pytest then vitest); documented in `Makefile`, `docs/dev/ci.md` "Reproduce locally", `README.md` and `CLAUDE.md`. | `make check` exits 0 on a clean checkout with pnpm installed and fails when a vitest test fails. | S |
| B4 | `docs/dev/ci.md` describes the lanes, what gates the downstream jobs (API integration, Next.js build, image build, Playwright), and the honest current state. | Closed 2026-09-09 (package B PR): the ML tier row and section, the parse-failure state section for `703f8f6`, the `make check` paragraph. | The page matches the workflow file. | S |
...
| Id | Requirement | State on 2026-09-09 | Acceptance check | Size |
|---|---|---|---|---|
| C1 | Each `specs/00N-*` gets `checklists/readiness-checklist.md` copied from `specs/_shared/readiness-checklist.md`: Specify, Clarify, Plan and Tasks sections present, all boxes unchecked, reviewer names blank. Boxes are checked only by the named reviewers, never by generation. | Closed 2026-09-09 (package C PR): `specs/00N-*/checklists/readiness-checklist.md` for F001 to F008, copied from the shared template with the feature name filled in, 23 unchecked boxes each and no `[x]`. | `ls specs/00*/checklists/readiness-checklist.md` lists eight files; `grep -c '\[x\]'` prints 0 for each. | S |
| C2 | Each feature gets `checklists/approval.md`: product owner, engineering reviewer, security or data reviewer where relevant, dates blank, remaining non-blocking limitations listed, decision `Draft`. D006 and D007 stay open with no invented owners. | Closed 2026-09-09 (package C PR): `specs/00N-*/checklists/approval.md` for F001 to F008, reviewer rows blank, `Decision: Draft`, the remaining limitations listed per feature, D006 and D007 recorded as OPEN with no owners. | Eight files exist; each reads `Decision: Draft`. | S |
| C3 | Each feature gets `checklists/evidence.md` mapping every success criterion in its `spec.md` to the test ids that prove it (unit, integration, `ml`, `e2e`) or to "no automated evidence" where true. "Done" is recorded separately from approval. | Closed 2026-09-09 (package C PR): `specs/00N-*/checklists/evidence.md` for F001 to F008. Every SC-001 to SC-003 row maps to `tests/...::test_...` ids by tier or says "no automated evidence". The rows follow the per-SC dispositions already in the specs (001, 004, 008 tables, the "Also affected" prose of the rest). Every cited id was checked against the tree. The Done gate is blank. | Every `SC-` id in the eight `spec.md` files appears in the matching evidence file. | M |
| C4 | `specs/_shared/decisions.md` carries RESOLVED rows D001 to D005 in its own format with approver `product owner (hackathon), 2026-09-08`; D006 and D007 open; a new row D014 for the poisoning decision (see F1). | Closed 2026-09-09 (package C PR): `specs/_shared/decisions.md` row D014 (RESOLVED 2026-09-09) and the "### D014" record under "Resolutions recorded 2026-09-09", approver `product owner, 2026-09-09`, the module marked not built (package F). D001 to D005 unchanged (RESOLVED, `product owner (hackathon), 2026-09-08`), D006 and D007 OPEN. | The five rows read RESOLVED with that approver; D006 and D007 read OPEN; D014 present. | S |
| C5 | `.specify/memory/constitution.md` carries "Amendment proposals (2026-09-08)" with status `proposed, pending named approval`; ratification is not claimed anywhere. | Closed 2026-09-09 (evidence only, no edit): `.specify/memory/constitution.md` line 62 `## Amendment proposals (2026-09-08)`, line 64 `**Status:** proposed, pending named approval` and "Ratification of the constitution, and approval of these proposals, is not claimed", lines 3 and 4 `draft for team ratification`, `**Ratified:** Not yet`. | `grep -n "Amendment proposals (2026-09-08)" .specify/memory/constitution.md` matches; no line claims ratification. | S |
| C6 | `docs/project-brief.md` is the single brief (D13). Every reference to `docs/brief.md` under `docs/`, `specs/` and `.specify/` is updated; references remaining in code or `README.md` are reported in the master plan, not silently edited. | Closed 2026-09-09 (evidence only, no edit): no live link remains. The grep hits are historical mentions only: `docs/project-brief.md:119` (the D13 record), `specs/_shared/analysis.md:50` ("was folded into"), `docs/superpowers/specs/2026-09-08-redsim-design.md:11` and `:298` (superseded, history), the product spec's D13 row, reconciliation rows 33 and 39 and completion item 29, and this row. Nothing in code or `README.md` references `docs/brief.md`. | `grep -rn "docs/brief.md" docs specs .specify` prints only historical mentions (the D13 records, "was folded into" notes, the superseded design spec) and this row. No live link, no mention in code or `README.md`. | S |
| C7 | `SECURITY.md` "Known gaps" is current: the Next.js advisories baselined in `.trivyignore` (including CVE-2026-75604 and GHSA-2xp9-vwfh-vxw4 added in wave 4) with the tracked Next 15 upgrade, and the sandbox and upload status after waves 1 to 3 (typed sandbox config, rlimits, no network jail, ONNX and state_dict only, pickles refused). | Closed 2026-09-09 (package C PR): `SECURITY.md` "ML vertical boundaries" re-headed as enforced on `main` with the sandbox described from `redsim/ml/sandbox.py` (rlimits, the four-name allowlist, process-group kill, no jail), and "Known gaps (tracked)" listing all twelve `.trivyignore` ids with the Next 14 to 15 upgrade as the fix. The "not on main yet" bullet is gone. | The section names every `.trivyignore` id and describes the sandbox as `redsim/ml/sandbox.py` implements it. | S |
| C8 | Illustrative numbers labelled illustrative everywhere (spec 26.2 item 11): local asset accuracies (0.7687, 0.9087, 0.6872) and demo-script figures. Wave 4 labels README and docs; sweep `specs/` and `docs/plans/06` to `08`. | Closed 2026-09-09 (package C PR): the two unlabelled hits, `docs/plans/11-phase-b-register-2026-09-09.md` lines 308 and 335, now read "(illustrative, one local build)". Every other hit already had "illustrative" in its paragraph. | `grep -rn "0\.7687\|0\.9087\|0\.5151" docs specs README.md` shows "illustrative" in the same paragraph for every hit. | S |
| C9 | A scripted walkthrough of the spec 24 demo (no UI): a shell script or `make demo` target that seeds, registers the bundled models, starts the image campaign, waits, starts verify, downloads the report and runs `redsim audit verify --all`, printing the ids at each step. | Closed 2026-09-09 (package C PR): `scripts/demo.sh` and the `demo` Makefile target. `bash -n` passes. NOT yet run against a live `make up` stack (the script header says so). Run it and record the ids here before quoting it as evidence. | `make demo` (or `scripts/demo.sh`) exits 0 against `make up` and prints the run id, finding id, verify run id and report path. | M |
| C10 | Docs wording drift left by wave 4: about ten places (`README.md`, `CLAUDE.md`, `docs/plans/00-master-plan.md` section 4.1, `docs/plans/EXECUTION-CONTEXT.md`, `docs/architecture/ml-vertical.md`) still say "open PR #23"; it merged as `10650da` with two review fixes. The same pages should record wave 4 as landed (`e73dea0`, the counts in section 1) and the CI state once the run for `e73dea0` has concluded (B1). Also `tests/e2e/README.md` should say `REDSIM_E2E_POSTGRES_URL` is driver-qualified (`postgresql+psycopg://`) in a venv without `psycopg2` (the governance fixture normalises a bare scheme; the harness fixture alone does not), and the docstring of `_attack_plugins` in `redsim/api/v1/attacks.py` still describes the pre-idempotent "already registered" behaviour. | Closed 2026-09-09 (package C PR): the three live mentions (`docs/plans/EXECUTION-CONTEXT.md:204`, `docs/plans/00-master-plan.md:349` and `:1191`) now read `PR #23 (... merged as 10650da with two review fixes)`. `docs/plans/09-gap-register-2026-09-08.md:9` keeps its mention because that register declares itself frozen at audit time. `tests/e2e/README.md` "Postgres lane" carries the driver-qualified URL note. The `_attack_plugins` docstring in `redsim/api/v1/attacks.py` describes the idempotent loader. `README.md`, `CLAUDE.md` and `docs/architecture/ml-vertical.md` already said merged. | `grep -rn "open PR #23\|Open PR #23" README.md CLAUDE.md docs` prints only the frozen `docs/plans/09-gap-register-2026-09-08.md` and this row (its requirement quotes the phrase). `mkdocs build --strict` passes. | S |
...
| F7 | Routes: `POST /v1/models/{id}/poisoning` (body: attack id, defense ids, poison fraction, trigger, subsample size, seed; 202 with the run id; 501 with reason for endpoint targets and unbuilt attacks; 422 for bounds), `GET /v1/runs/{id}/poisoning` (the card), catalog rows on `GET /v1/attacks?family=poisoning` and `GET /v1/defenses?family=poisoning` (the defenses row is void since 2026-09-09: the verify paradigm was removed); a new `Action` `POISON_RUN` at the `scanner` rank in `redsim/api/policy.py` (platform code, not the frozen ML schema) with tests; the spec 17.3 codes reused. | Missing. | Route tests for 202, 501, 422, 403 (viewer) and the card shape. | M |
...
- Phase B proper continues in the redsim workflow waves (plan at
  ...
  adversarial training and distillation with measured deltas (void since
  2026-09-09: the verify paradigm was removed), the full review
  ...
  verify (void since 2026-09-09: the verify paradigm was removed), CLI
  matrix).
...
`redsim/api/v1/{models,attacks,compare,reports,ml_findings}.py`,
