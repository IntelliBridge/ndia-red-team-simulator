...
Status (2026-09-09): landed. Four track commits written in the worktree `wt/waveb4` on the B3 integration and rebased onto `main` at `703f8f6` (`e2e(endpoint,llm): endpoint connector and garak LLM probe evidence on the shared harness`, `e2e(text,detection,attacks,harden): modality, norm, ZOO and training-defense evidence`, `e2e(review,reports,interop,bulk): review workflow, PDF/snapshots, compare, Croissant, ATLAS, Foundry, batches, capacity, CLI matrix evidence`, `ci(phase-b-gate): scripts/phase_b_gate.sh, make check-phase-b, CI jobs, docs consistency test`), then the fix pass the B4 e2e files demanded (three tracks: `fix-api-services`, `fix-worker-runners`, `fix-gate-ci`, closing the carried B2 and B3 items listed under "Closed in B4" in `docs/architecture/ml-vertical.md`) and this documentation pass (master plan v2.8, the plan-07 rewrite onto the section 27 vocabulary, `EXECUTION-CONTEXT.md`, the spec 3.2, 11.6, 11.7, 17.4, 22.6 and 26.7 addenda, `docs/api/v1.md` against the OpenAPI document, the garak supply-chain paragraph), pushed together. The gate exists (`make check-phase-b`, nine steps, the first failure naming its spec 26 criterion) and its garak step fails on an empty collection since this wave. What the B4 files found and left open by attribution (each a `pytest.fail` naming the module, never a weakened assertion): the endpoint campaign path (`redsim/ml/targets/endpoint.py:228` sends the 8-row probe unscaled, three cases of `test_ml_endpoint.py`), the training-defense verify (no bundled target exposes a training slice, `test_ml_attacks_harden.py`), the consumed slice on a campaign (the worker parent does not materialise it, `test_ml_interop.py`), the bulk `state_dict` upload (`architecture_kwargs` not derived from the binding, `test_ml_bulk.py`), and three stale report-format pins the completion render made stale (`tests/ml/test_audit_campaign.py`, `tests/e2e/test_ml_verify_upload_reports.py`, `tests/e2e/test_ml_review_reports.py`). The README's "Open items and not implemented" lists them; the e2e tier and the default tier are not claimed green at the B4 push and their counts are the assembler's to record.
...
Status of these checks at the wave B4 push (2026-09-09, read from the tree,
not claimed): ruff and mypy hold; `pytest -q tests/ml/test_schema_compat.py`
holds; `pytest -q tests/test_docs_phase_b_consistency.py` and `mkdocs build
--strict` hold after this documentation pass; the default, `ml` and `garak`
tiers were last counted green on `main` at `703f8f6` (2634 / 433 / 12) and the
B4 tree carries one stale pin in `tests/ml/test_audit_campaign.py` the
assembler moves; the `e2e` tier carries the seven B4 files and the attributed
failures named in the wave B4 status line, so it is not green; `make
check-phase-b` has not been run against `make up` (the stack has not been
brought up end to end, brief package A); no CI run on `main` after `58461cc`
has been read.
