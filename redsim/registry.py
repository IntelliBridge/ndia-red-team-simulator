...
(:meth:`maybe_load_entry_points`) gated by ``REDSIM_PLUGINS=1`` so third-party
plugins can never perturb the offline test path.

Discovery is now a *single* code path. :meth:`scan_entry_points` walks the
entry points once and yields a :class:`~redsim.plugins.PluginInfo` per item,
...
(``redsim.plugins.discover_all``) consume that one generator, so there is no
...
def _allowlist() -> set[str] | None:
    """Parse ``REDSIM_PLUGINS_ALLOW`` into a set of distribution names.
    ...
    """
    raw = os.environ.get("REDSIM_PLUGINS_ALLOW")
    if raw is None:
        return None
...
class Registry(Generic[T]):
    ...
    def scan_entry_points(
        ...
        *,
        ...
        ``REDSIM_PLUGINS_REQUIRE_SIGNATURE`` enforcement is on), a conformant
        ...
        Caller is responsible for the ``REDSIM_PLUGINS=1`` gate; this method
        ...
        if allow is None:
            logger.warning(
                "REDSIM_PLUGINS=1 but no REDSIM_PLUGINS_ALLOW allowlist configured; "
        ...
        for ep in eps:
            ...
            if allow is not None and (dist_name is None or dist_name not in allow):
                yield _info(ep.name, "skipped",
                            "distribution not in REDSIM_PLUGINS_ALLOW")
                continue
    ...
    def maybe_load_entry_points(
        ...
        """Discover and register third-party items, gated by REDSIM_PLUGINS=1.
        ...
        (``REDSIM_PLUGINS_REQUIRE_SIGNATURE``), a verifier is wired in so unsigned
