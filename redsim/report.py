"""Redsim report generation — markdown and JSON outputs from scan findings."""
...
def _sort_findings(findings: list[RedsimFinding]) -> list[RedsimFinding]:
...
def _resolve_target(findings: list[RedsimFinding]) -> str:
...
def generate_markdown_report(run_state: RunStateAPI, findings: list[RedsimFinding]) -> str:
    ...
    # Header
    _a("# Redsim Security Assessment Report")
    _a("")
...
def generate_json_report(run_state: RunStateAPI, findings: list[RedsimFinding]) -> dict[str, Any]:
...
def generate_html_report(run_state: RunStateAPI, findings: list[RedsimFinding]) -> str:
    md = generate_markdown_report(run_state, findings)
    body = _md_to_html_min(md)
    return (
        "<!doctype html><html><head><meta charset=\"utf-8\">"
        f"<title>Redsim Report — {run_state.run_id}</title>"
        f"<style>{_HTML_CSS}</style></head><body>{body}</body></html>"
    )


def save_reports(run_state: RunStateAPI, findings: list[RedsimFinding],
