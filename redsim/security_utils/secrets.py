...
The key comes from ``RedsimConfig.auth_profiles_key`` (env
``REDSIM_AUTH_PROFILES_KEY``). Encryption is symmetric and process-local:
...
def _fernet(config: RedsimConfig | None = None) -> MultiFernet:
    """Return the configured Fernet, wrapped for key rotation.

    Always a ``MultiFernet``: the current key first, then the previous
    key (``REDSIM_AUTH_PROFILES_KEY_PREVIOUS``) when configured. Encryption
    ...
    if not key:
        raise AuthProfilesKeyError(
            ...
            "REDSIM_AUTH_PROFILES_KEY environment variable to a Fernet key "
            "(generate one with: python -c \"from cryptography.fernet import "
            "Fernet; print(Fernet.generate_key().decode())\")"
        )
    fernets = [_build_fernet(key, env_var="REDSIM_AUTH_PROFILES_KEY")]
    previous = cfg.auth_profiles_key_previous
    if previous:
        fernets.append(
            _build_fernet(previous, env_var="REDSIM_AUTH_PROFILES_KEY_PREVIOUS")
        )
    return MultiFernet(fernets)


def encrypt_secret(plaintext: str, *, config: RedsimConfig | None = None) -> bytes:
...
def decrypt_secret(token: bytes, *, config: RedsimConfig | None = None) -> str:
    """Decrypt a ``secret_ciphertext`` token back to the plaintext secret.

    Tries the current key then the previous key
    (``REDSIM_AUTH_PROFILES_KEY_PREVIOUS``) when one is configured, so
