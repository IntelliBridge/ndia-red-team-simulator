...
down_revision: str | None = "0003_application_logs"
...
def upgrade() -> None:
    ...
    op.execute(sa.text("""
        CREATE OR REPLACE FUNCTION redsim_audit_events_immutable()
        RETURNS trigger
        LANGUAGE plpgsql
        AS $fn$
        BEGIN
    ...
    op.execute(sa.text("""
        CREATE TRIGGER trg_audit_events_no_update_delete
            ...
            FOR EACH ROW
            EXECUTE FUNCTION redsim_audit_events_immutable();
    """))
    ...
    op.execute(sa.text("""
        DO $$
        BEGIN
            IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'redsim_owner') THEN
                ALTER TABLE audit_events OWNER TO redsim_owner;
                ALTER FUNCTION redsim_audit_events_immutable() OWNER TO redsim_owner;
            END IF;
        END
        $$;
    """))
    ...
    op.execute(sa.text("""
        ...
        BEGIN
            IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'redsim_app') THEN
                REVOKE ALL ON TABLE audit_events FROM redsim_app;
                GRANT INSERT, SELECT ON TABLE audit_events TO redsim_app;
                ...
                GRANT USAGE ON ALL SEQUENCES IN SCHEMA public TO redsim_app;
            END IF;
        END
        $$;
    """))
    ...
    op.execute(sa.text("""
        ...
        BEGIN
            IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pgaudit') THEN
                IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'redsim_owner') THEN
                    ALTER ROLE redsim_owner SET pgaudit.log = 'ddl, role';
                END IF;
                IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'redsim_app') THEN
                    ALTER ROLE redsim_app SET pgaudit.log = 'ddl, role';
                END IF;
            END IF;
        END
        $$;
...
def downgrade() -> None:
    ...
    op.execute(sa.text("""
        ...
        BEGIN
            IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pgaudit') THEN
                IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'redsim_owner') THEN
                    ALTER ROLE redsim_owner RESET pgaudit.log;
                END IF;
                IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'redsim_app') THEN
                    ALTER ROLE redsim_app RESET pgaudit.log;
                END IF;
            END IF;
        END
        $$;
        ...
        BEGIN
            IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'redsim_app') THEN
                GRANT SELECT, INSERT, UPDATE, DELETE
                    ON TABLE audit_events TO redsim_app;
            END IF;
        END
        $$;
    """))
    ...
    op.execute(sa.text(
        ...
        "ON audit_events;"))
    op.execute(sa.text(
        "DROP FUNCTION IF EXISTS redsim_audit_events_immutable();"))
