...
installed a ``BEFORE INSERT`` trigger (``redsim_set_org_id_<table>``) that
...
scoped table (``redsim_check_org_id_<table>``) that **raises** whenever the new
...
TRIGGER IF EXISTS``) is copied from ``redsim_set_org_id_<table>`` in
...
down_revision: str | None = "0008_finding_tickets"
...
def upgrade() -> None:
    ...
    for table in _SCOPED_TABLES:
        fn = f"redsim_check_org_id_{table}"
...
def downgrade() -> None:
    ...
    for table in _SCOPED_TABLES:
        op.execute(sa.text(
            f"DROP TRIGGER IF EXISTS trg_check_org_id_{table} ON {table}"))
        op.execute(sa.text(
            f"DROP FUNCTION IF EXISTS redsim_check_org_id_{table}()"))
