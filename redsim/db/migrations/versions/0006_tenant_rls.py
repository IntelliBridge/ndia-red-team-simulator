...
down_revision: str | None = "0005_auth_profiles"
...
def upgrade() -> None:
    ...
    for table in _RLS_TABLES:
        ...
        op.execute(sa.text(
            f"DROP POLICY IF EXISTS redsim_tenant_isolation ON {table}"))
...
def downgrade() -> None:
    ...
    for table in _SCOPED_TABLES:
        op.execute(sa.text(
            f"DROP TRIGGER IF EXISTS trg_set_org_id_{table} ON {table}"))
        op.execute(sa.text(
            f"DROP FUNCTION IF EXISTS redsim_set_org_id_{table}()"))
        op.execute(sa.text(
