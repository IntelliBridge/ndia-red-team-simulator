...
marketplace loader (:meth:`redsim.registry.Registry.scan_entry_points`). It is
entirely opt-in: when ``REDSIM_PLUGINS_REQUIRE_SIGNATURE`` is enabled an operator
...
Signature file format + discovery
    ...
    ``REDSIM_PLUGINS_SIG_DIR`` (colon/comma-separated), then the trusted-keys
...
Trusted keys
    Ed25519 **public** keys in PEM form, named by ``REDSIM_PLUGINS_TRUSTED_KEYS``
...
# Env var names (kept in sync with redsim.config for discoverability/docs).
ENV_REQUIRE_SIGNATURE = "REDSIM_PLUGINS_REQUIRE_SIGNATURE"
ENV_TRUSTED_KEYS = "REDSIM_PLUGINS_TRUSTED_KEYS"
ENV_SIG_DIR = "REDSIM_PLUGINS_SIG_DIR"

_PAYLOAD_PREFIX = b"redsim-plugin"
...
class SignatureResult:
    ...
    suitable for logs and the ``redsim plugins list`` report.
    """
...
class KeyringVerifier:
    ...
    ``REDSIM_PLUGINS_TRUSTED_KEYS`` / ``REDSIM_PLUGINS_SIG_DIR`` env vars (read at
...
def load_plugin_verifier(config: object | None = None) -> PluginVerifier | None:
    ...
    Enforcement is enabled by ``REDSIM_PLUGINS_REQUIRE_SIGNATURE`` (``1``/truthy)
    ...
    if not _CRYPTOGRAPHY_AVAILABLE:  # pragma: no cover - misconfig: enforcement on, no crypto
        raise RuntimeError(
            ...
            "plugin signatures (or unset REDSIM_PLUGINS_REQUIRE_SIGNATURE)"
        )
    trusted_raw = None
    sig_raw = None
    if config is not None:
...
# ---------------------------------------------------------------------------
# Signer (used by `redsim plugins sign`, tests, and to sign the example)
# ---------------------------------------------------------------------------
