"""Supply-chain security primitives for Redsim.

Currently this package houses optional cryptographic *signature verification*
for third-party marketplace plugins (:mod:`redsim.supply_chain.signing`). The
signing seam layers onto the existing entry-point loader: when enforcement is
enabled (``REDSIM_PLUGINS_REQUIRE_SIGNATURE=1``) only plugins whose distribution
