...
``config.task_models`` (an optional dict on ``RedsimConfig``), falling back
...
def _resolve_model(task: str, config: RedsimConfig) -> str:
...
def route(
    ...
    *,
    ...
    otherwise the RedsimConfig default (``_resolve_model``). The override is
