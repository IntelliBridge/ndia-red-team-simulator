...
``redsim.llm.router``: ``remaining(project_id)`` returns the project's
...
from redsim.llm.router import BudgetChecker, BudgetExceeded
...
class DbBudgetChecker:
    ...
    ``redsim.db.session.get_session`` shape). Defaulting to ``get_session``
    ...
    def model_override(self, org_id: str | None, task: str) -> str | None:
        ...
        before falling back to the RedsimConfig default.
        """
        if not org_id:
            return None
        with self._session_factory() as sess:
