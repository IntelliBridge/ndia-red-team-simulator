"""Redsim-signed API session cookie (Phase 4 v0.4.0 F14a).

NextAuth owns the browser session. After Keycloak login, the NextAuth
callback mints an additional cookie — ``redsim_api_session`` — signed
with an Redsim-managed RSA key. FastAPI verifies that cookie against
the Redsim public key on every request; we do **not** try to verify
...
def _ensure_key(value: str | None, *, label: str) -> str:
    if not value:
        raise SessionCookieError(
            f"{label} is not configured. Set the Redsim API session keypair "
            "env vars (REDSIM_API_SESSION_PRIVATE_KEY / "
            "REDSIM_API_SESSION_PUBLIC_KEY) before serving cookie auth."
        )
    return value
...
def mint_session_cookie(
    *,
    ...
    private = _ensure_key(settings.api_session_private_key,
                          label="REDSIM_API_SESSION_PRIVATE_KEY")
    ...
    payload = {
        ...
        "email": email,
        "name": display_name,
        "redsim_project_roles": project_memberships or {},
        "iat": now,
        "exp": now + ttl,
        "jti": uuid.uuid4().hex,
    }
...
def verify_session_cookie(value: str, settings: APISettings) -> SessionClaims:
    """Verify the JWT against the Redsim public key.

    Tries the current ``REDSIM_API_SESSION_PUBLIC_KEY`` first, then the
    previous key (``REDSIM_API_SESSION_PUBLIC_KEY_PREVIOUS``) if one is
    ...
    public = _ensure_key(settings.api_session_public_key,
                         label="REDSIM_API_SESSION_PUBLIC_KEY")
    candidates = [public]
    ...
    iss = claims.get("iss")
    if iss != "redsim-api-session":
        raise SessionCookieError("session cookie issuer mismatch")

    memberships_raw = claims.get("redsim_project_roles") or {}
...
def generate_keypair() -> tuple[str, str]:
    ...
    Used by the test helpers and by ``redsim api-session-keys generate``
