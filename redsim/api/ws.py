...
- Programmatic callers may attach a bearer token via the WebSocket
  subprotocol channel ``Sec-WebSocket-Protocol: redsim.bearer.<token>``
  (RFC 6455). The server echoes the chosen subprotocol back; the
  token itself never appears in the URL or in a query parameter.
- Browser callers use cookie auth — the ``redsim_api_session`` cookie
...
_BEARER_SUBPROTOCOL_PREFIX = "redsim.bearer."
...
def _extract_bearer_subprotocol(websocket: WebSocket) -> tuple[str | None, str | None]:
    ...
    ``Sec-WebSocket-Protocol`` when an ``redsim.bearer.<token>`` entry
...
async def _resolve_user_for_ws(
    ...
    Resolution order:
    1. ``Sec-WebSocket-Protocol: redsim.bearer.<token>`` (programmatic).
    2. ``Authorization: Bearer …`` header (works for some clients).
    3. ``redsim_api_session`` cookie (browser path, F14a).
    """
...
async def _enforce_upgrade_policy(websocket: WebSocket, run_id: str) -> bool:
    ...
    from redsim.api.policy import has_project_access
...
async def _redis_pubsub_iter(channel: str) -> AsyncIterator[dict[str, Any]]:
    ...
    url = os.environ.get("REDSIM_BROKER_URL")
    if not url:
