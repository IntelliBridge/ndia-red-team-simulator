"""Redsim run state persistence (filesystem backend).

Phase 3 introduces ``redsim.state.RunStateAPI`` as the shared Protocol;
...
from redsim.state.facade import ArtifactRef

if TYPE_CHECKING:
    from redsim.schema import Status
...
class FilesystemRunState:
    ...
    def save_findings(self, findings: list) -> None:
        """Save list of RedsimFinding dicts to findings.json."""
