...
When ``REDSIM_DB_URL`` is set the Postgres backend is used; otherwise the
...
if TYPE_CHECKING:
    from redsim.state.facade import RunStateAPI


def open_run_state(config: RedsimConfig, *,
    ...
    ``REDSIM_DB_URL`` (or a pre-initialised engine).
    """
    db_url = os.environ.get("REDSIM_DB_URL")
    if not db_url:
        from redsim.state.filesystem import FilesystemRunState
