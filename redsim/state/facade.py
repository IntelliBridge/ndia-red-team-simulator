...
Phase 2's ``redsim.state.RunState`` is filesystem-only. Phase 3 introduces a
Postgres backend (``redsim.state.PostgresRunState``) so the API and the
...
service layer (``redsim.services``) emits audit events at operation
...
if TYPE_CHECKING:
    from redsim.schema import RedsimFinding, Status
...
class RunStateAPI(Protocol):
    ...
    def save_findings(self, findings: list[RedsimFinding]) -> None: ...
