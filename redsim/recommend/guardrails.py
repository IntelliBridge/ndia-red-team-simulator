...
1. **Secret scrubbing** (:func:`scrub_secrets` / :func:`filter_output` /
   ...
   *same* ones the audit chain already trusts
   (:mod:`redsim.redact`); we reuse them rather than maintain a second
...
Everything is gated by :class:`GuardrailConfig` and is **secret-free
...
from redsim.redact import _TOKEN_PATTERNS



@dataclass(frozen=True)
class GuardrailConfig:
    """Switches for the guardrail helpers (all default on)."""

    enabled: bool = True
    detect_injection: bool = True
    injection_block_risk: str = "high"   # low | medium | high | off
    filter_output: bool = True
    scrub_diff: bool = True
...
def _resolve_config(config: GuardrailConfig | None) -> GuardrailConfig:
    return config if config is not None else GuardrailConfig()


def guard_input(text: str, *, config: GuardrailConfig | None = None) -> None:
    ...
    block_risk = (cfg.injection_block_risk or "high").lower()
    if block_risk == "off":
        return
...
def guard_diff(diff: str, *, config: GuardrailConfig | None = None) -> str:
...
def guard_output(text: str, *, config: GuardrailConfig | None = None) -> str:
