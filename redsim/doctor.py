"""Redsim environment validation (doctor) module — provider-aware."""
...
def _report_attack_adapters() -> None:
    ...
    attack adapter (``redsim.ml.attacks``) or a signed plugin registers through
    ``redsim.scanners``.
    """
    try:
    ...
    if names:
        ...
        _warn("Attack adapters",
              ...
              "adapters (redsim.ml.attacks) register via redsim.scanners")
...
def run_doctor(config: RedsimConfig | None = None, *,
    ...
    ok = True
    print("Redsim Doctor — environment check\n")
