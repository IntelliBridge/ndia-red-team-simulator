...
:mod:`redsim.scanners.sandbox`). It is launched as a module —
``python -m redsim.scanners.sandbox_worker`` — with a single ``--entry-point``
...
:class:`~redsim.state.RunState` rooted at ``run_path``. Because the import and
...
clean :class:`~redsim.scanners.registry.ScanResult` error instead of dying.

Resource limits, the wall-clock timeout, and the network-off environment are
imposed by the *parent* (see :mod:`redsim.scanners.sandbox`) before this code
...
def _parse_entry_point(spec: str) -> tuple[str, str]:
    ...
    Mirrors the ``redsim.scanners:example`` form used across the marketplace
...
def main(argv: list[str] | None = None) -> int:
    ...
    """
    parser = argparse.ArgumentParser(prog="redsim.scanners.sandbox_worker")
