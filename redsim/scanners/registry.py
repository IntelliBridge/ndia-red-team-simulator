...
class RunnerResult(Protocol):
    ...
    attack adapters in ``redsim.ml.attacks``) uses to hand its result back.
    """
...
def run_cli_scan(
    ...
    *,
    argv: list[str],
    command_str: str,
    parse: Callable[[subprocess.CompletedProcess, str], list[RedsimFinding]],
...
def run_cli_scan_jsonl(
    ...
    *,
    argv: list[str],
    command_str: str,
    convert: Callable[[dict[str, Any], str], RedsimFinding | None],
    subdir: str | None = None,
    raw_filename: str | None = None,
) -> ScanResult:
    ...
    * ``convert`` — callback ``(record, run_id) -> RedsimFinding | None`` invoked
...
def dispatch(name_or_capability: str,
    ...
    raise KeyError(
        ...
        f"ML attack adapter (redsim.ml.attacks) or a signed plugin. "
...
def _sandbox_wrap(adapter: ScannerAdapter, ep: object) -> ScannerAdapter:
    ...
    (``REDSIM_PLUGINS_SANDBOX=0``) or the entry point cannot be resolved to an
...
def maybe_load_entry_points() -> None:
    """Third-party adapter discovery, gated by REDSIM_PLUGINS=1.

    Conformant third-party scanners are wrapped in a sandboxing proxy
    (:class:`redsim.scanners.sandbox.SandboxedScanner`) so their ``scan()`` runs
    in a resource-limited, network-off subprocess by default.
    """
    _scanner_registry.maybe_load_entry_points("redsim.scanners", wrap=_sandbox_wrap)
