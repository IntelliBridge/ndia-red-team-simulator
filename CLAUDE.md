...
| Exports (2026-09-09, the web Exports page; `docs/api/v1.md#exports`) | `GET /v1/exports?project=&limit=` (authenticated; membership on `?project=`, else the caller's memberships; one row per `ml.campaign` run, each `kind: "campaign"`, newest first, from `services.ml_exports.list_exports`: the run, its `model` block, a `reports` block with the four formats as artifact refs from the newest non-archived snapshot then the newest artifact row, `available`, `missing`, `snapshot_count`, `latest_snapshot` and `render_in_flight`, and a `dataset` block with `status` `not_exported | queued | running | exported | failed`, the manifest digest, `files`, `bytes`, `card`, the follow-up `job_id` and `follow_up_run_id`, `error` and the admission `blockers` `not_terminal | run_failed | fixture_target | no_slices`; `422 params_out_of_range` for `limit` outside 1 to 500; reads only, writes no audit row, carries no score; `tests/ml/test_exports_route.py`; since 2026-09-10 a `foundry` block per row: `status` `not_configured | not_pushed | queued | running | pushed | failed`, `auto_push`, `push_run_id`, `transaction_rid`, `pushed_at`, `error`, `blockers` `not_terminal | run_failed | fixture_target | score_unavailable`, read from the newest `ml.integration_push` run whose `stage_table.parent_run_id` names the row). Project Foundry settings (2026-09-10): `GET /v1/projects/{slug}/integrations/foundry` (membership) and `PUT` (`target.manage`, audited `project.settings` before the write; body keys `dataset_rid`, `auth_profile_id`, `auto_push`, each optional, `null` clears; `auto_push: true` is `501 integration_disabled` while the deployment is not configured and `422 params_out_of_range` with `auto_push_not_ready` and the `blockers` while a push would be refused) over `projects.ml_integrations` through `services.ml_integrations`; the response carries the `deployment` block (status, host, reason, attested, default rid), the `settings` and the `effective` view with its blockers. The push route's body keys are optional since the same day: the project's profile and rid fill an empty body, `422 auth_profile_required` when neither names a profile |
...
- `redsim/workers/tasks/capacity.py` (`redsim.ml_dispatch_deferred`, on
  ...
  backstop.
- `redsim/workers/tasks/foundry_auto_push.py` (2026-09-10): `auto_push_continuation(job_id)`
  is the outermost context of `ml_campaign_run`. After the terminal status is
  committed, a campaign run that `succeeded` in a project whose
  `ml_integrations.foundry.auto_push` is true goes through
  `redsim.integrations.create_foundry_push` with an empty body (the project's
  profile and rid fill it) and actor `worker:auto_push`: the same refusals,
  admission row, follow-up `Run` and `Job` and enqueue on `default` as an
  admin's `POST`. A refused or failed push lives on its own run; the campaign
  run is never touched and the hook never raises.
...
`black-box-endpoint`, `gradients: false`), `endpoint_contract` (wave B0,
`endpoint-v1`, pure Python) and `input_contract` (2026-09-10, pure Python: the
`InputPreprocessing` block of an upload, resize then scale then mean / std
then layout, parsed by the API route and applied by `artifact.py`, which also
maps the bare torchvision / timm state-dict key layout onto `resnet18`
(`state_dict_layout: torchvision`), sniffs or honours a channels-last ONNX
signature (`onnx.input_layout: NHWC`) and folds the contract into the estimator
with `make_input_adapter` so the campaign keeps perturbing `[0, 1]` NCHW pixels
at the evaluation resolution; `scripts/hf_open_weights_fetch.sh` and
`scripts/hf_open_weights_demo.py` register and scan four public CIFAR-10
checkpoints from Hugging Face through the real upload path)), `attacks/` (`fgsm`, `pgd` with surrogate
...
prefetch; since 2026-09-10 also the "Palantir Foundry" panel above the table
(`foundry-panel.tsx`: the deployment line, the per-project dataset rid, the
bearer auth profile and the auto-push toggle through `exports.foundrySettings`
and `exports.updateFoundrySettings`, Save admin-gated) and a Foundry column
per row with a "Push to Foundry" action through `exports.pushFoundry`;
the 2026-09-09 shape: server
