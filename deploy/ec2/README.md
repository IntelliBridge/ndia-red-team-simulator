...
in `web.env` and the web secret JSON (`/opt/redsim/env/web.json`), and
the API already holds the matching public key. `REDSIM_ENV=prod` keeps the
developer-access panel off the page and the `dev:` bearer refused by the API.
